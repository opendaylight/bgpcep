...
class TestSinglePeerChangeCount:
    ...
    @pytest.mark.skip_if_fails(
        ["test_reconfigure_odl_to_accept_connection", "test_data_change_counter_ready"]
    )
    ...
    @pytest.mark.skip_if_fails(
        [
            "test_reconfigure_odl_to_accept_connection",
    ...
    @pytest.mark.skip_if_fails(
        ["test_kill_talking_bgp_speaker", "test_data_change_counter_ready"]
    )
    def test_wait_for_stable_ip_topology_after_talking(self):
        change_counter.wait_for_change_count_to_become_stable(
            minimum_value=self.last_change_count_single + 1, timeout=BGP_EMPTYING_TIMEOUT
        )
    ...
    @pytest.mark.skip_if_fails(
        ["test_kill_listening_bgp_speaker", "test_data_change_counter_ready"]
    )
