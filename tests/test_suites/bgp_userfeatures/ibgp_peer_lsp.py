...
class TestIbgpPeerLsp:
    ...
    def check_example_bgp_rib_does_not_contain(self, substr: str):
        ...
        assert substr not in response.text, f"Did not expect {substr} to be present in example-bgp-rib config:\n{response.text}"
    ...
    def test_ibgp_peer_lsp(self, allure_step_with_separate_logging):
        with allure_step_with_separate_logging("step_tc1_configure_ibgp_peer"):
            ...
            templated_requests.put_templated_request(
                "data/bgpuser/bgp_peer", mapping, json=False
            )

        with allure_step_with_separate_logging("step_tc1_check_example_bgp_rib_is_empty"):
        ...
        with allure_step_with_separate_logging("step_tc1_connect_bgp_peer"):
            ...
            infra.log_message_to_karaf("Error = WELL_KNOWN_ATTR_MISSING is EXPECTED in this test case, and should be thrown when missing mandatory attributes.")
            self.bgp_speaker_process = bgp.start_bgp_speaker(1, bgpls=True, skipAttr=True, my_ip=TOOLS_IP, log_level="debug")
        ...
        with allure_step_with_separate_logging("step_tc1_check_example_bgp_rib"):
            ...
            utils.verify_function_does_not_fail_within_timeout(10, 1, self.check_example_bgp_rib_does_not_contain, JSONKEYSTR)
        ...
        with allure_step_with_separate_logging("step_tc2_check_example_bgp_rib_is_empty"):
            """Check RIB for none linkstate-routes."""
            self.check_example_bgp_rib_does_not_contain(JSONKEYSTR)
        ...
        with allure_step_with_separate_logging("step_tc2_connect_bgp_peer"):
            """Connect BGP peer with advertising the routes without mandatory params like LOC_PREF."""
            infra.log_message_to_karaf("Error = WELL_KNOWN_ATTR_MISSING is EXPECTED in this test case, and should be thrown when missing mandatory attributes.")
            self.bgp_speaker_process = bgp.start_bgp_speaker(1, bgpls=True, my_ip=TOOLS_IP, log_level="debug")
        ...
        with allure_step_with_separate_logging("step_tc2_check_example_bgp_rib"):
            ...
            templated_requests.get_templated_request("data/bgpuser/lsp/effective_rib_in", mapping, verify=True)
