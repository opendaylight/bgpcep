...
class TestBgpFlowspec:
    ...
    def set_bgp_peer_configuration(self):
        ...
        templated_requests.put_templated_request(
            "data/bgpflowspec/bgp_peer", mapping, json=False
        )

    def delete_bgp_peer_configuration(self):
        mapping = {"BGP_RIB_OPENCONFIG": "example-bgp-rib", "IP":TOOLS_IP }
        templated_requests.delete_templated_request("data/bgpflowspec/bgp_peer", mapping)

    def prepare_config_files(self):
        infra.shell("cp data/bgpflowspec/bgp-flowspec-redirect.cfg tmp/")
        infra.shell(f"sed -i -e 's/EXABGPIP/{TOOLS_IP}/g' tmp/bgp-flowspec-redirect.cfg")
        infra.shell(f"sed -i -e 's/ODLIP/{ODL_IP}/g' tmp/bgp-flowspec-redirect.cfg")
        infra.shell("cp data/bgpflowspec/bgp-flowspec.cfg tmp/")
    ...
    def test_bgp_flowspec(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_check_for_rmpty_topology_before_talking"):
        ...
        with allure_step_with_separate_logging("step_flowspec_test_1"):
            ...
            exabgp_process = bgp.start_exabgp_and_verify_connected("tmp/bgp-flowspec.cfg", TOOLS_IP)
            utils.wait_until_function_pass(15, 1, templated_requests.get_templated_request, "data/bgpflowspec/bgp_flowspec", mapping=None, verify=True)
        ...
        with allure_step_with_separate_logging("step_flowspec_test_2"):
            ...
            exabgp_process = bgp.start_exabgp_and_verify_connected("tmp/bgp-flowspec-redirect.cfg", TOOLS_IP)
            utils.wait_until_function_pass(15, 1, templated_requests.get_templated_request, "data/bgpflowspec/bgp_flowspec_redirect", mapping=None, verify=True)
            bgp.stop_exabgp(exabgp_process)
            flowspec.verify_flowspec_data_is_empty()

        with allure_step_with_separate_logging("step_deconfigure_odl_to_accept_connection"):
