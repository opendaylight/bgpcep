...
class BaseManyPeerPrefixCount:
    bgp_speaker_process = None

    def test_many_peers_prefix_count(self, allure_step_with_separate_logging, bgp_peers_count, prefixes_count, insert, withdraw, prefill):
        bgp_filling_timeout = TEST_DURATION_MULTIPLIER * (prefixes_count * bgp_peers_count * 4.0 / 10000 + 80)
        ...
        with allure_step_with_separate_logging("step_start_talking_bgp_managers"):
            ...
            self.bgp_speaker_process = bgp.start_bgp_speaker(
                ammount=prefixes_count, multiplicity=bgp_peers_count,
                insert=insert,
                withdraw=withdraw,
                prefill=prefill,
                update="single", listen=False
            )
