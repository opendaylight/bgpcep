...
class BaseTestManyPeerChangeCount:
    bgp_speaker_process = None
    last_change_count = 1

    def test_many_peers_change_count(
        self,
        ...
        with allure_step_with_separate_logging(
            ...
            it."""
            bgp.set_bgp_peer_group(
                peer_group_name="internal-neighbors", passive_mode=True
            )
        ...
        with allure_step_with_separate_logging("step_start_talking_bgp_speakers"):
            ...
            self.last_change_count = change_counter.get_change_count()
        ...
        with allure_step_with_separate_logging("step_check_talking_ip_topology_count"):
            ...
            ), f"Ipv4 topology does not contain all {prefixes_count} expected " \
               f"advertised prefixes, but only {topology_count}"
