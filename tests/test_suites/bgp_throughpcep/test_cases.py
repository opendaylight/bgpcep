...
class TestBgpAppManyPeerPrefixCount:
    ...
    def test_bgp_app_many_peers_prefix_count(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("Updater_1"):
            ...
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=1)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_2"):
            """Run updater tool to change hops, using 1 blocking http thread."""
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=2)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_3"):
            """Run updater tool to change hops, using 1 blocking http thread."""
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=4)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_4"):
            """Run updater tool to change hops, using 1 blocking http thread."""
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=8)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_5"):
            """Run updater tool to change hops, using 1 blocking http thread."""
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=16)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_6"):
            """Run updater tool to change hops, using 1 blocking http thread."""
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=32)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_7"):
            """Run updater tool to change hops, using 1 blocking http thread."""
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=64)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_8"):
            """Run updater tool to change hops, using 1 blocking http thread."""
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=128)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_9"):
            """Run updater tool to change hops, using 1 blocking http thread."""
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=256)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_10"):
            """Run updater tool to change hops, using 1 blocking http thread."""
            hop = self.get_next_hop()
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=512)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
        ...
        with allure_step_with_separate_logging("Updater_with_delegate"):
            ...
            rc, stdout = pcep.start_update(hop=hop, pcc=PCCS, lsp=LSPS, workers=1, delegate=False, pccip="127.0.0.1", tunnel_no=2)
            assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
            log.info(f"{stdout=}")
            not_expected_log_message = f"Counter({{'pass': {0}}})"
            assert not_expected_log_message not in stdout, f"Did not expect message {not_expected_log_message=}, but was found in {stdout=}"
            utils.wait_until_function_returns_value(120, 1, TOTAL_LSPS, pcep.get_pcep_topology_hop_count, hop)

        with allure_step_with_separate_logging("Stop_Pcc_Mock"):
            """Send ctrl+c to pcc-mock, see prompt again within timeout."""
            output = self.pcc_mock_process.stdout
            log.info(f"Pcc mock process {output=}")
            infra.stop_process(self.pcc_mock_process, gracefully=True)
            pcep.kill_all_pcc_mock_simulators()
        ...
        with allure_step_with_separate_logging("PCEP_Sessions_Flapped_with_LSP_updates"):
            ...
            for _ in range(15):
                ...
                utils.wait_until_function_returns_value(60, 5, total_lsps, pcep.get_pcep_topology_hop_count, intial_hop)
                rc, stdout = pcep.start_update(hop=updated_hop, pcc=pccs, lsp=lsps, workers=1)
                assert rc == 0, f"Updater scipt returned non zero return code {rc=} {stdout=}"
                log.info(f"{stdout=}")
                expected_log_message = f"Counter({{'pass': {total_lsps}}})"
                assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
            ...
            expected_log_message = f"Counter({{'pass': {TOTAL_LSPS}}})"
            assert expected_log_message in stdout, f"Expected message {expected_log_message=} was not found in {stdout=}"
            utils.wait_until_function_returns_value(30, 1, TOTAL_LSPS, pcep.get_pcep_topology_hop_count, hop)
            output = self.pcc_mock_process.stdout
            log.info(f"Pcc mock process {output=}")
            infra.stop_process(self.pcc_mock_process, gracefully=True)
