...
ODL_PASSWD = os.environ["ODL_PASSWORD"]
TOOLS_IP = os.environ["TOOLS_IP"]
TOOLS_USER = os.environ["TOOLS_USER"]
TOOLS_PASSWD = os.environ["TOOLS_PASSWORD"]
...
class TestCases:
    ...
    def check_stability(self):
        ...
        self.pcc_mock_process = pcep.start_pcc_mock(
            pcc=PCCS, lsp=LSPS, output_file_name=LOG_NAME
        )
        rc, stdout = pcep.run_updater(
            hop=updated_hop, pcc=PCCS, lsp=LSPS, workers=2, refresh=UPDATER_REFRESH
        )
    ...
    def test_cases(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_verify_1"):
            ...
            topology."""
            utils.wait_until_function_returns_value(
                120, 1, TOTAL_LSPS, pcep.get_pcep_topology_hop_count, hop
            )
        ...
        if ODL_IP == TOOLS_IP:
            ...
            with allure_step_with_separate_logging(
                ...
                infra.ssh_put_file(
                    local_file_path="tools/updater.py",
                ...
                infra.ssh_put_file(
                    local_file_path="lib/AuthStandalone.py",
        ...
        with allure_step_with_separate_logging(
            ...
            for _ in range(15):
                ...
                self.pcc_mock_process = pcep.start_pcc_mock(
                    pcc=pccs, lsp=lsps, output_file_name="serial_execution.log"
                )
                rc, stdout = pcep.run_updater(
                    hop=updated_hop,
                    pcc=pccs,
