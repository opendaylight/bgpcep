...
class TestBgpAppSinglePeerPrefixCount:
    bgp_speaker_process = None
    last_change_count_single = 1

    def test_bgp_app_single_peer_prfix_count(self, step_logger):
        ...
        with step_logger("step_connect_bgp_peers"):
            """Start BGP peer tool"""
            TestBgpAppSinglePeerPrefixCount.bgp_speaker_process = bgp.start_bgp_speaker(
                ammount=0, listen=False, log_level="info"
            )
            assert infra.is_process_still_running(
                TestBgpAppSinglePeerPrefixCount.bgp_speaker_process
        ...
        with step_logger("step_stop_bgp_peer"):
            """Stop BGP peer tool."""
            bgp.stop_bgp_speaker(self.bgp_speaker_process)
