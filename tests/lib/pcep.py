...
def start_pcc_mock(
    ...
    utils.wait_until_function_returns_value(int(pcc * lsp / 10_000)+10, 1, pcc * lsp, get_pcep_topology_hop_count, "1.1.1.1/32")
...
def check_updater_response(stdout, lsps, parallel):
    log.info(f"Updater output: {stdout}")
    if parallel:
