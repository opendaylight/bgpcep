...
def get_ipv4_topology() -> json:
    ...
    """
    topology = templated_requests.get_request(url=f"http://{ODL_IP}:{RESTCONF_PORT}/rests/data/network-topology:network-topology/topology=example-ipv4-topology?content=nonconfig", expected_code=200)

    return topology.json()

def get_ipv4_topology_prefixes_count():
    """Returns number of prefixes stored in example ipv4 topology in ODL using RESTCONF.
...
def wait_for_ipv4_topology_prefixes_to_become_stable(excluded_value: int, timeout: int = 120, wait_period: int = 5, consecutive_times_stable_value: int = 4):
    """Waits until the number of prefixes in exmaple ipv4 topology stabilized on concrete constant vlaue not equal to the excluded value within pre-defined timeout. This value need to be stable for consecutive number of calls.

    Args:
        excluded_value (int): Value which is not observed as stable. (Usually this value is equal to the previous count before last operation was executed)
    ...
    while True:
        ...
        if (datetime.datetime.now() - start_time).seconds >= timeout:
            raise AssertionError(f"Expected Ipv4 topology to be stable after {timeout} seconds")
