...
def shell(command: str | list | tuple, joiner="; ", cwd: str | None = None, run_in_background: bool = False, timeout: int = None):
    """Run shell command or list of shell commands joined usind specifi joiner.

    Args:
        command (str | list | tuple): Shell command to be run
        joiner (str): Joiner for concatenating multiple commands.
        cwd (str): Current working directory from where the command needs to be executed.
        run_in_backgroud (bool): If the command should be started as background process without tty.
        timeotu (int): Timeout in seconds for the forground command.

    Returns:
        tuple[int, str] | subprocess.Popen : For forground process it returns final return code and stdout, for backgroud process it returns process handler.
    """
...
def retry_shell_command(retry_count: int, interval: int, *args, **kwargs):
    """Repeatedly reads text file until it finds specific substring in its content.
    ...
    Args:
        ...
        file_name (str): Name of the text file to be checked for presence of substring.
        threshold (int): Minimum number of occurences of the expected substring.

    Returns:
        int: number of substring occurences in text file
    """
    validator = lambda result:  result[0] == 0
    rc, output = utils.retry_function_with_return_value_validator(retry_count, interval, validator, shell, *args, **kwargs)
    return rc, output

def wait_for_string_in_file(retry_count: int, interval: int, string: str, file_name: str, threshold: int = 1) -> int:
    """Repeatedly reads text file until it finds specific substring in its content.
    ...
    rc, output = utils.retry_function_with_return_value_validator(retry_count, interval, validator, shell, f"grep -c '{string}' '{file_name}'")
...
def start_odl_with_features(features: tuple[str], timeout: int = 60):
    ...
    Args:
        features (tuple[str]): Features to be installed on ODL.
