...
def retry_shell_command(retry_count: int, interval: int, *args, **kwargs):
    ...
    rc, output = utils.retry_function_with_return_value_validator(
...
def start_odl_with_features(features: tuple[str], timeout: int = 60):
    ...
    shell(
        f"sed -ie 's/\(featuresBoot=\|featuresBoot =\)/featuresBoot = {",".join(features)},/g' etc/org.apache.karaf.features.cfg",
        cwd="opendaylight",
    )

    shell(
        "sed -ie 's/memory-mapped = true/memory-mapped = false/g' system/org/opendaylight/controller/sal-clustering-config/*/sal-clustering-config-*-factorypekkoconf.xml",
        cwd="opendaylight",
    )
