...
def put_request(url: str, headers: dict, data: dict | str, expected_code: int | None = None) -> requests.Response:
...
def get_templated_request(temlate_dir: str, mapping: dict, json: bool = True, verify: bool = False) -> requests.Response:
    ...
    if verify:
        ...
        expected_response = resolve_templated_text(temlate_dir + "/data." + file_name_suffix, mapping)
        ...
        visual_diff = "\n".join(difflib.unified_diff(normalized_response.splitlines(), normalized_expected_response.splitlines(), fromfile="received response", tofile="expected response", lineterm=''))
        assert  normalized_response == normalized_expected_response, f"Received response does not match expected response: \n{visual_diff}"

    return response


def put_templated_request(temlate_dir: str, mapping: dict, json = True) -> requests.Response:
