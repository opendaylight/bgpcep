...
def set_change_counter(topology_name: str) -> requests.Response:
    """Sets change counter in ODL to track changes in specific topology.

    It uses RESTCONF to set the change counter.

    Args:
        topology_name (str): Name of the topology which number of changes
            should be observed.
    ...
    resposne = templated_requests.put_templated_request(
        "data/templates/change_counter", mapping, json=False
    )
...
def get_change_count(data_change_counter: str = "data-change-counter") -> int:
    ...
    response = templated_requests.get_request(
        url=f"http://{ODL_IP}:{RESTCONF_PORT}/rests/data/data-change-counter:{data_change_counter}?content=nonconfig",
        expected_code=200,
    )
...
def wait_for_change_count_to_become_stable(
    ...
    while True:
        ...
        log.info(
            f"Waiting for change count to become stable: {current_count=}, {last_count=}, {minimum_value=}, {stable_times=}"
        )
