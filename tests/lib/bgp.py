...
def set_bgp_neighbour(ip: str, passive_mode: bool = True) -> requests.Response:
    ...
    resposne = templated_requests.put_templated_request(
        "data/templates/bgp_peer", mapping, json=True
    )
...
def set_bgp_neighbour_user(ip: str, passive_mode: bool = True) -> requests.Response:
    ...
    resposne = templated_requests.put_templated_request(
        "data/templates/bgp_peer_user", mapping, json=False
    )

    return resposne


def set_bgp_neighbours_user(
    first_neigbout_ip: str, count: int, passive_mode: bool = True
):
...
def start_bgp_speaker_with_verify_and_retry(
    ...
    Returns:
        subprocess.Popen: BGP speaker process handler.
    """
    bgp_speaker_process = utils.retry_function(
        retry_count=retries,
        interval=1,
...
def verify_bgp_speaker_connected(connected: bool = True) -> requests.Response:
    ...
    response = templated_requests.get_request(
        f"http://{ODL_IP}:{RESTCONF_PORT}/rests/data/bgp-rib:bgp-rib/rib=example-bgp-rib/peer=bgp:%2F%2F127.0.0.1?content=nonconfig",
