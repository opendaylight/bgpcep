...
def retry_function(
    ...
    return retry_function_with_return_value_validator(
        retry_count, interval, validator, function, *args, **kwargs
    )


def retry_function_and_expect_value(
    retry_count: int,
    interval: int,
    expected_value: Any,
    function: Callable,
...
def retry_function_with_return_value_validator(
    retry_count: int,
    interval: int,
    ...
    for retry_num in range(retry_count):
        try:
            ...
            if not return_value_validator(result):
                log.info(
                    f"{function.__name__}({','.join(args)} {kwargs or ''}) did not return expected value, but: {result}"
                )
            else:
                return result
        except Exception as e:
            log.info(
                f"{function.__name__}({','.join(args)} {kwargs or ''}) failed with: {e} ({retry_num}/{retry_count})"
            )
        time.sleep(interval)
    else:
        raise Exception(
            f"Failed to execute {function.__name__}({','.join(args)} {kwargs or ''}) after {retry_count} attempts."
        )
