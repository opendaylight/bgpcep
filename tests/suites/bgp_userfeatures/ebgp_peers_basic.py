...
from libraries import ip_topology
...
class TestEbgpPeersBasic:
    ...
    def configure_ibgp_peer(self, ip: str, rr_client: bool, cluster_id: bool = False):
        ...
        mapping = {"IP": ip, "HOLDTIME": 180, "PEER_PORT": 17900, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "RR_CLIENT": str(rr_client).lower(), "PASSIVE_MODE": "true"}
        templated_requests.put_templated_request(f"{BGP_DATA_FOLDER}/{template_folder}", mapping, json=False)

    def delete_ibgp_peer_configuration(self, ip: str, cluster_id: bool = False):
        template_folder = "cluster_id/ibgp_peer" if cluster_id else "ibgp_peers"
        mapping = {"IP": ip, "BGP_RIB_OPENCONFIG": "example-bgp-rib"}
        templated_requests.delete_templated_request(f"{BGP_DATA_FOLDER}/{template_folder}", mapping)

    def connect_bgp_peer1_and_verify(self, cluster_id: str = None):
        self.bgp_peer1_process = bgp.start_bgp_speaker(firstprefix=BGP_PEER1_FIRST_PREFIX_IP, prefixlen=PREFIX_LEN, ammount=3, my_ip=BGP_PEER1_IP, peer_ip=ODL_IP, cluster=cluster_id, log_level=BGP_PEER_LOG_LEVEL, log_file=BGP_PEER1_LOG_FILE)
        utils.wait_until_function_pass(10, 1, ip_topology.check_example_ipv4_topology_contains, f"'prefix': '{BGP_PEER1_FIRST_PREFIX_IP}/{PREFIX_LEN}'")

    def connect_bgp_peer2_and_verify(self, cluster_id: str = None):
        self.bgp_peer2_process = bgp.start_bgp_speaker(firstprefix=BGP_PEER2_FIRST_PREFIX_IP, prefixlen=PREFIX_LEN, ammount=3, my_ip=BGP_PEER2_IP, peer_ip=ODL_IP, cluster=cluster_id, log_level=BGP_PEER_LOG_LEVEL, log_file=BGP_PEER2_LOG_FILE)
        utils.wait_until_function_pass(10, 1, ip_topology.check_example_ipv4_topology_contains, f"'prefix': '{BGP_PEER2_FIRST_PREFIX_IP}/{PREFIX_LEN}'")
    ...
    def test_ebgp_peers_basic(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_tc1_bgp_peer1_check_log_for_introduced_prefixes"):
            """Check incomming updates for new routes."""
            utils.wait_until_function_pass(DEFAULT_LOG_CHECK_TIMEOUT, DEFAULT_LOG_CHECK_PERIOD, infra.verify_string_occurence_count_in_file, "nlri_prefix_received:", f"tmp/{BGP_PEER1_LOG_FILE}", BGP_PEER2_PREFIX_COUNT)
            infra.verify_string_occurence_count_in_file(f"nlri_prefix_received: {BGP_PEER2_FIRST_PREFIX_IP}/{PREFIX_LEN}", f"tmp/{BGP_PEER1_LOG_FILE}", 1)
            infra.verify_string_occurence_count_in_file("withdrawn_prefix_received:", f"tmp/{BGP_PEER1_LOG_FILE}", 0)

        with allure_step_with_separate_logging("step_tc1_bgp_peer2_check_log_for_introduced_prefixes"):
            """Check incomming updates for new routes."""
            utils.wait_until_function_pass(DEFAULT_LOG_CHECK_TIMEOUT, DEFAULT_LOG_CHECK_PERIOD, infra.verify_string_occurence_count_in_file, "nlri_prefix_received:", f"tmp/{BGP_PEER2_LOG_FILE}", BGP_PEER1_PREFIX_COUNT)
            infra.verify_string_occurence_count_in_file(f"nlri_prefix_received: {BGP_PEER1_FIRST_PREFIX_IP}/{PREFIX_LEN}", f"tmp/{BGP_PEER2_LOG_FILE}", 1)
            infra.verify_string_occurence_count_in_file("withdrawn_prefix_received:", f"tmp/{BGP_PEER2_LOG_FILE}", 0)
        ...
        with allure_step_with_separate_logging("step_tc1_bgp_peer2_check_log_for_withdrawn_prefixes"):
            """Check incomming updates for withdrawn routes."""
            utils.wait_until_function_pass(DEFAULT_LOG_CHECK_TIMEOUT, DEFAULT_LOG_CHECK_PERIOD, infra.verify_string_occurence_count_in_file, "withdrawn_prefix_received:", f"tmp/{BGP_PEER2_LOG_FILE}", BGP_PEER1_PREFIX_COUNT)
            infra.verify_string_occurence_count_in_file(f"withdrawn_prefix_received: {BGP_PEER1_FIRST_PREFIX_IP}/{PREFIX_LEN}", f"tmp/{BGP_PEER2_LOG_FILE}", 1)
        ...
        with allure_step_with_separate_logging("step_tc1_check_for_empty_ipv4_topology"):
            """Checks for empty topology after"""
            utils.wait_until_function_pass(10, 1, ip_topology.check_example_ipv4_topology_does_not_contain, 'prefix')

        with allure_step_with_separate_logging("step_tc1_delete_bgp_peers_configuration"):
        ...
        with allure_step_with_separate_logging("step_tc2_configure_one_ibgp_route_reflector_client_and_one_ibgp_non_client"):
        ...
        with allure_step_with_separate_logging("step_tc2_bgp_peer1_check_log_for_introduced_prefixes"):
            """Check incomming updates for new routes."""
            utils.wait_until_function_pass(DEFAULT_LOG_CHECK_TIMEOUT, DEFAULT_LOG_CHECK_PERIOD, infra.verify_string_occurence_count_in_file, "nlri_prefix_received:", f"tmp/{BGP_PEER1_LOG_FILE}", BGP_PEER2_PREFIX_COUNT)
            infra.verify_string_occurence_count_in_file(f"nlri_prefix_received: {BGP_PEER2_FIRST_PREFIX_IP}/{PREFIX_LEN}", f"tmp/{BGP_PEER1_LOG_FILE}", 1)
            infra.verify_string_occurence_count_in_file("withdrawn_prefix_received:", f"tmp/{BGP_PEER1_LOG_FILE}", 0)

        with allure_step_with_separate_logging("step_tc2_bgp_peer2_check_log_for_introduced_prefixes"):
            """Check incomming updates for new routes."""
            utils.wait_until_function_pass(DEFAULT_LOG_CHECK_TIMEOUT, DEFAULT_LOG_CHECK_PERIOD, infra.verify_string_occurence_count_in_file, "nlri_prefix_received:", f"tmp/{BGP_PEER2_LOG_FILE}", BGP_PEER1_PREFIX_COUNT)
            infra.verify_string_occurence_count_in_file(f"nlri_prefix_received: {BGP_PEER1_FIRST_PREFIX_IP}/{PREFIX_LEN}", f"tmp/{BGP_PEER2_LOG_FILE}", 1)
            infra.verify_string_occurence_count_in_file("withdrawn_prefix_received:", f"tmp/{BGP_PEER2_LOG_FILE}", 0)
        ...
        with allure_step_with_separate_logging("step_tc2_bgp_peer2_check_log_for_withdrawn_prefixes"):
            """Check incomming updates for withdrawn routes."""
            utils.wait_until_function_pass(DEFAULT_LOG_CHECK_TIMEOUT, DEFAULT_LOG_CHECK_PERIOD, infra.verify_string_occurence_count_in_file, "withdrawn_prefix_received:", f"tmp/{BGP_PEER2_LOG_FILE}", BGP_PEER1_PREFIX_COUNT)
            infra.verify_string_occurence_count_in_file(f"withdrawn_prefix_received: {BGP_PEER1_FIRST_PREFIX_IP}/{PREFIX_LEN}", f"tmp/{BGP_PEER2_LOG_FILE}", 1)
        ...
        with allure_step_with_separate_logging("step_tc2_check_for_empty_ipv4_topology"):
            """Checks for empty topology after"""
            utils.wait_until_function_pass(10, 1, ip_topology.check_example_ipv4_topology_does_not_contain, 'prefix')

        with allure_step_with_separate_logging("step_tc2_delete_bgp_peers_configuration"):
            """Delete all previously configured BGP peers."""
            self.delete_ibgp_peer_configuration(BGP_PEER1_IP, cluster_id=False)
            self.delete_ibgp_peer_configuration(BGP_PEER2_IP, cluster_id=False)

        with allure_step_with_separate_logging("step_tc3_configure_two_ibgp_non_client_peers"):
            """Configure iBGP peers: 1st one as RR client, 2nd one as RR non-client."""
        ...
        with allure_step_with_separate_logging("step_tc3_bgp_peer1_check_log_for_no_updates"):
            """Check for no updates received by iBGP peer No. 1."""
            utils.wait_until_function_pass(DEFAULT_LOG_CHECK_TIMEOUT, DEFAULT_LOG_CHECK_PERIOD, infra.verify_string_occurence_count_in_file, "total_received_update_message_counter: 1", f"tmp/{BGP_PEER1_LOG_FILE}", 2)
        ...
        with allure_step_with_separate_logging("step_tc3_bgp_peer2_check_log_for_no_updates"):
            """Check for no updates received by iBGP peer No. 2."""
            utils.wait_until_function_pass(DEFAULT_LOG_CHECK_TIMEOUT*2, DEFAULT_LOG_CHECK_PERIOD, infra.verify_string_occurence_count_in_file, "total_received_update_message_counter: 1", f"tmp/{BGP_PEER2_LOG_FILE}", 4)
        ...
        with allure_step_with_separate_logging("step_tc3_check_for_empty_ipv4_topology"):
            """Checks for empty topology after"""
            utils.wait_until_function_pass(10, 1, ip_topology.check_example_ipv4_topology_does_not_contain, 'prefix')

        with allure_step_with_separate_logging("step_tc3_delete_bgp_peers_configuration"):
            """Delete all previously configured BGP peers."""
            self.delete_ibgp_peer_configuration(BGP_PEER1_IP, cluster_id=False)
            self.delete_ibgp_peer_configuration(BGP_PEER2_IP, cluster_id=False)

        with allure_step_with_separate_logging("step_tc4_configure_two_ibgp_rr_clients_with_cluster_id"):
        ...
        with allure_step_with_separate_logging("step_tc4_bgp_peer1_check_rib_out_for_introduced_prefixes"):
            ...
            mapping = {"IP": BGP_PEER1_IP, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "PEER_NUMBER": 2, "CLUSTER_ID": BGP_PEER2_IP, "DEFAULT_ID": "192.0.2.2"}
            utils.wait_until_function_pass(10, 1, templated_requests.get_templated_request, f"{BGP_DATA_FOLDER}/cluster_id/peer_rib_out", mapping, verify=True)

        with allure_step_with_separate_logging("step_tc4_bgp_peer2_check_rib_out_for_introduced_prefixes"):
            """Check incomming updates for new routes and respective cluster-ids
            in second peer which has local route-reflector-cluster-id."""
            mapping = {"IP": BGP_PEER2_IP, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "PEER_NUMBER": 1, "CLUSTER_ID": BGP_PEER1_IP, "DEFAULT_ID": "127.0.0.4"}
            utils.wait_until_function_pass(10, 1, templated_requests.get_templated_request, f"{BGP_DATA_FOLDER}/cluster_id/peer_rib_out", mapping, verify=True)
        ...
        with allure_step_with_separate_logging("step_tc4_check_for_empty_ipv4_topology"):
            """Checks for empty topology after"""
            utils.wait_until_function_pass(10, 1, ip_topology.check_example_ipv4_topology_does_not_contain, 'prefix')

        with allure_step_with_separate_logging("step_tc4_delete_bgp_peers_configuration"):
            """Delete all previously configured BGP peers."""
