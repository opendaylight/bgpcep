...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/bgpcep/bgpuser/basic.robot
#
...
@pytest.mark.run(order=SuiteOrder.BGP_USER_MANY_BASIC)
class TestBasic:
    ...
    def configure_peer_group(self, peer_group_folder: str):
        ...
        templated_requests.put_templated_request(
            f"{BGP_VARIABLES_FOLDER}/{peer_group_folder}", mapping, json=False
        )
        templated_requests.get_templated_request(
            f"{BGP_VARIABLES_FOLDER}/verify_{peer_group_folder}.sulfur",
            mapping,
            verify=True,
        )
    ...
    def deconfigure_peer_group(self):
        ...
        templated_requests.delete_templated_request(
            f"{BGP_VARIABLES_FOLDER}/peer_group", mapping, json=False
        )

    def restart_talking_bgp_speaker(self):
        """Abort the Python speaker. And Start it again.
    ...
    def compare_topology(self, folder_name: str):
        ...
        templated_requests.get_jinja_templated_request(
            template_dir=f"{BGP_VARIABLES_FOLDER}/{folder_name}",
    ...
    def test_basic(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging(
            ...
            # Configure ibgp peers with local-address.
        ...
        with allure_step_with_separate_logging(
            ...
            for i in range(BGP_PEERS_COUNT):
                mapping = {"IP": f"127.0.1.{i}", "BGP_RIB_OPENCONFIG": RIB_NAME}
                templated_requests.delete_templated_request(
        ...
        with allure_step_with_separate_logging("step_tc_r_reset_bgp_peer_session"):
            # Reset Peer Session.
            for i in range(BGP_PEERS_COUNT):
                mapping = {"IP": f"127.0.1.{i}", "RIB_INSTANCE_NAME": RIB_NAME}
        ...
        with allure_step_with_separate_logging(
            ...
            self.wait_for_topology_to_change_to(
                "filled_topology_manypeers_with_withdrawals"
            )
        ...
        with allure_step_with_separate_logging(
            ...
            # See TCP (BGP) connections are established state.
            self.check_speaker_is_connected()
