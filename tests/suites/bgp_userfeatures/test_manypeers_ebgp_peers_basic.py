...
iBGP_PEER1_COMMAND = f"python3 tools/fastbgp/play.py --multiplicity {iBGP_PEERS1_COUNT} --firstprefix {iBGP_PEER1_FIRST_PREFIX_IP} --prefixlen {iBGP_PEER1_PREFIX_LEN} --amount {iBGP_PEER1_PREFIX_COUNT} --myip={iBGP_PEER1_IP} --myport={BGP_TOOL_PORT} --peerip={ODL_IP} --peerport={ODL_BGP_PORT} --{BGP_PEER_LOG_LEVEL} --logfile {iBGP_PEER1_LOG_FILE}"
eBGP_PEER1_COMMAND = f"python3 tools/fastbgp/play.py --multiplicity {eBGP_PEERS1_COUNT} --firstprefix {eBGP_PEER1_FIRST_PREFIX_IP} --prefixlen {eBGP_PEER1_PREFIX_LEN} --amount {eBGP_PEER1_PREFIX_COUNT} --myip={eBGP_PEER1_IP} --myport={BGP_TOOL_PORT} --peerip={ODL_IP} --peerport={ODL_BGP_PORT} --nexthop {eBGP_PEER1_NEXT_HOP} --asnumber {eBGP_PEER1_AS} --{BGP_PEER_LOG_LEVEL} --logfile {eBGP_PEER1_LOG_FILE}"
eBGP_PEER2_COMMAND = f"python3 tools/fastbgp/play.py --multiplicity {eBGP_PEERS2_COUNT} --firstprefix {eBGP_PEER2_FIRST_PREFIX_IP} --prefixlen {eBGP_PEER2_PREFIX_LEN} --amount {eBGP_PEER2_PREFIX_COUNT} --myip={eBGP_PEER2_IP} --myport={BGP_TOOL_PORT} --peerip={ODL_IP} --peerport={ODL_BGP_PORT} --nexthop {eBGP_PEER2_NEXT_HOP} --asnumber {eBGP_PEER2_AS} --{BGP_PEER_LOG_LEVEL} --logfile {eBGP_PEER2_LOG_FILE}"
...
class TestEbgpPeersBasic:
    ...
    def validate_rib(self, ip, skipped_prefix):
        ...
        env = Environment(loader=FileSystemLoader(f"{BGP_VARIABLES_FOLDER}/local_as/manypeers_adj_rib_out"))
        template = env.get_template("data.j2")
        expected_rib = template.render(mapping)
        response = templated_requests.get_templated_request(f"{BGP_VARIABLES_FOLDER}/local_as/adj_rib_out", mapping)
    ...
    def test_ebgp_peers_basic(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging(
            ...
            total_prefix_count = iBGP_PEERS1_COUNT * (iBGP_PEERS1_COUNT - 1 + eBGP_PEER1_PREFIX_COUNT + eBGP_PEER2_PREFIX_COUNT)
            utils.wait_until_function_pass(
                DEFAULT_TOPOLOGY_CHECK_COUNT,
                DEFAULT_TOPOLOGY_CHECK_PERIOD,
                infra.verify_string_occurence_count_in_file,
        ...
        with allure_step_with_separate_logging("step_delete_bgp_peers_configuration"):
            ...
            for i in range(eBGP_PEERS1_COUNT):
                mapping = {"IP": f"127.0.2.{i}", "BGP_RIB_OPENCONFIG": PROTOCOL_OPENCONFIG}
                templated_requests.delete_templated_request(
                    f"{BGP_VARIABLES_FOLDER}/ebgp_peers", mapping
                )
            for i in range(eBGP_PEERS2_COUNT):
                mapping = {"IP": f"127.0.3.{i}", "BGP_RIB_OPENCONFIG": PROTOCOL_OPENCONFIG}
                templated_requests.delete_templated_request(
                    f"{BGP_VARIABLES_FOLDER}/ebgp_peers", mapping
                )
