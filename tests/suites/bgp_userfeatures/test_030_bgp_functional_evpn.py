...
class TestBgpfunctionalEvpn:
    ...
    def get_update_content(self, expected_codes):
        templated_requests.get_from_uri(uri=EVPN_LOC_RIB, headers=XML_HEADERS, expected_code=expected_codes)
    ...
    def remove_configured_routes(self):
        ...
        templated_requests.get_from_uri(uri=EVPN_LOC_RIB, headers=JSON_HEADERS, expected_code=templated_requests.ALLOWED_DELETE_STATUS_CODES)
        response = templated_requests.get_from_uri(uri=f"{EVPN_CONF_URL}?content=config", headers=JSON_HEADERS, expected_code=templated_requests.ALLOWED_DELETE_STATUS_CODES)
        if response.status_code not in templated_requests.DELETED_STATUS_CODES:
            templated_requests.delete_from_uri_request(uri=EVPN_CONF_URL, expected_code=204)
    ...
    def odl_to_play_template(self, to_test: str):
        ...
        try:
            ...
            templated_requests.post_to_uri(uri=EVPN_CONF_URL, headers=XML_HEADERS, data=data_xml, expected_code=201)
            templated_requests.get_from_uri(uri=f"{EVPN_CONF_URL}?content=config", headers=XML_HEADERS)
    ...
    def verify_test_preconditions(self):
        templated_requests.get_from_uri(uri=f"{EVPN_CONF_URL}?content=config", headers=JSON_HEADERS, expected_code=templated_requests.DELETED_STATUS_CODES)
        resp = templated_requests.get_from_uri(uri=EVPN_FAMILY_LOC_RIB, headers=JSON_HEADERS)
