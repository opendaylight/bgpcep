...
class TestTcpMd5User:
    ...
    def start_pcc_mock_tool_with_password(self, password):
        ...
        self.pcep_mock_process = infra.shell(
            f"java -jar build_tools/pcep-pcc-mock.jar --pcc {PCCS} --lsp 1 --log-level debug --password {password} --reconnect 1 --local-address 127.0.1.0 --remote-address {ODL_IP} 2>&1 | tee pccmock.log",
