...
@pytest.mark.run(order=SuiteOrder.BGP_USER_MANY_ROUTE_REF)
class TestBgpfunctionalRouteRef:
    ...
    def verify_exaBgps_received_updates(self, exp_count: int):
        ...
        for i in range(BGP_PEERS_COUNT):
            count_recv = int(BGP_RPC_CLIENTS[i].exa_get_received_update_count())
    ...
    def test_bgp_functional_route_ref(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_exa_to_send_route_refresh"):
            ...
            try:
                for i in range(BGP_PEERS_COUNT):
                    BGP_RPC_CLIENTS[i].exa_clean_received_update_count()
                for i in range(BGP_PEERS_COUNT):
                    BGP_RPC_CLIENTS[i].exa_announce(
