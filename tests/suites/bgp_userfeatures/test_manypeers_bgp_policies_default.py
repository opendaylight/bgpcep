...
class TestBgpPoliciesDefault:
    ...
    def setup_config_files(self):
        ...
        env = Environment(loader=FileSystemLoader(f"{POLICIES_VAR}/exabgp_manypeers_configs"))
        for i in range(6):
            template = env.get_template(f"exabgp{i+1}.j2")
            config = template.render({"ODLIP": ODL_IP, "PEERS": peers[i], "ROUTEREFRESH": "disable" , "ADDPATH": "disable"})
