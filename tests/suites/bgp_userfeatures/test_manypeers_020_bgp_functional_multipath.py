...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/bgpcep/bgpfunct/020_bgp_functional_multipath.robot
#
...
@pytest.mark.run(order=SuiteOrder.BGP_USER_MANY_MULTIPATH)
class TestBgpfunctionalMultipath:
    ...
    def store_rib_configuration(self):
        ...
        response = templated_requests.get_templated_request(
            f"{MULT_VAR_FOLDER}/rib_config", mapping=DEFAULT_MAPPING
        )
    ...
    def configure_path_selection_and_app_peer_and_connect_peer(
        ...
        self.bgp_rpc_clients = [
            bgp.BgpRpcClient(f"127.0.1.{i}") for i in range(BGP_PEERS_COUNT)
        ]
    ...
    def configure_app_peer_with_routes(self):
        ...
        for path_id in PATH_ID_LIST:
            route_mapping = {
                "NEXTHOP": f"{NEXT_HOP_PREF}{path_id}",
                "LOCALPREF": f"{path_id}00",
                "PATHID": path_id,
                "APP_RIB": ODL_IP,
            }
            templated_requests.post_templated_request(
                f"{MULT_VAR_FOLDER}/route", route_mapping, json=False
            )
