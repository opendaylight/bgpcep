...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/bgpcep/bgpuser/ibgp_peers_basic.robot
#
...
@pytest.mark.run(order=SuiteOrder.BGP_USER_IBGP_PEERS)
class TestIbgpPeersBasic:
    ...
    @allure.description(
        textwrap.dedent(
            ...
            **Test Case 2**: Two iBGP peers: one RR client and one non-client \
            introduces prefixes
            Expected result: controller forwards updates towards both peers
            ...
            **Test Case 4**: Two iBGP(play.py) RR-client peers configured, \
            first of them configured with route-reflector-cluster-id, second \
            inherits it's cluster-id from global config. Each of them introduces \
            3 prefixes.
