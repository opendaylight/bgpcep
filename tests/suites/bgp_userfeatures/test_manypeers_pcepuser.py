...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/bgpcep/pcepuser/pcepuser.robot
#
...
class TestPcepUser:
    ...
    def compare_topology(self, exp, uri=None):
        """Get current pcep-topology as json, compare both expected and actual
        json. Error codes and normalized jsons should match exactly."""
        if uri:
        ...
        statics_volatile_keys = (
            "received-msg-count",
            "received-rpt-msg-count",
            "sent-init-msg-count",
            "sent-msg-count",
            "sent-upd-msg-count",
            "unknown-msg-received",
            "received-error-msg-count",
            "sent-error-msg-count",
            "delegated-lsps-count",
            "last-received-rpt-msg-timestamp",
        ...
        utils.verify_jsons_match(
            ...
            json2_data_label="Expected topology",
            volatiles_list=statics_volatile_keys,
        )
    ...
    def test_pcepuser(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_update_delegated"):
            ...
            for i in range(PCCS):
                ...
                templated_requests.post_templated_request(
                    f"{PCEP_VARIABLES_FOLDER}/update_lsp", mapping, json=False
                )
                pcc_ip += 1
        ...
        with allure_step_with_separate_logging("step_refuse_remove_delegated"):
            ...
            for i in range(PCCS):
                ...
                resp = templated_requests.post_templated_request(
                    f"{PCEP_VARIABLES_FOLDER}/remove_lsp",
                    mapping,
                    json=False,
        ...
        with allure_step_with_separate_logging("step_add_instantiated"):
            ...
            for i in range(PCCS):
                ...
                templated_requests.post_templated_request(
                    f"{PCEP_VARIABLES_FOLDER}/add_lsp", mapping, json=False
                )
                pcc_ip += 1
        ...
        with allure_step_with_separate_logging("step_remove_instantiated"):
            ...
            for i in range(PCCS):
                ...
                resp = templated_requests.post_templated_request(
                    f"{PCEP_VARIABLES_FOLDER}/remove_lsp", mapping, json=False
                )
                pcc_ip += 1
