...
ODL_3_IPS = [f"127.0.0.{i + int(BGP_PEERS_COUNT/2) + 3}" for i in range(int(BGP_PEERS_COUNT/2)-1)]
...
class TestBgpfunctionalRtConstrainValidation:
    ...
    def test_bgp_functional_rt_constrain_validation(
        ...
        with allure_step_with_separate_logging("step_start_bgp_peers"):
            ...
            self.start_bgp_peer(ODL_1_IPS[0], BGP_PEER_AS_NUMBERS[0], 8002, multiplicity=1, filename=f"play.py.090.2")
            log.info(f"IP: {ODL_2_IPS[0]} as_number: {BGP_PEER_AS_NUMBERS[1]}")
            self.start_bgp_peer(ODL_2_IPS[0], BGP_PEER_AS_NUMBERS[1], 8003, multiplicity=10, filename=f"play.py.090.3")
            log.info(f"IP: {ODL_3_IPS[0]} as_number: {BGP_PEER_AS_NUMBERS[2]}")
            self.start_bgp_peer(ODL_3_IPS[0], BGP_PEER_AS_NUMBERS[2], 8004, multiplicity=9, filename=f"play.py.090.4")
        ...
        with allure_step_with_separate_logging(
            ...
            for ip in ODL_2_IPS:
                mapping = {"PATH": f"peer=bgp:%2F%2F{ip}/adj-rib-out", "BGP_RIB": RIB_NAME}
                utils.wait_until_function_pass(
                    3,
                    2,
                    templated_requests.get_templated_request,
        ...
        with allure_step_with_separate_logging("step_play_to_odl_rt_constrain_type_1"):
            ...
            for i, ip in enumerate(ODL_3_IPS):
                mapping = {"PATH": f"peer=bgp:%2F%2F{ip}/effective-rib-in",
                    "BGP_RIB": {RIB_NAME},
                }
                utils.wait_until_function_pass(
                    3,
