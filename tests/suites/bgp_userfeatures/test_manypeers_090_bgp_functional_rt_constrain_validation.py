...
ODL_IPS = [f"127.0.0.{i+2}" for i in range(BGP_PEERS_COUNT)]
...
class TestBgpfunctionalRtConstrainValidation:
    ...
    def play_to_odl_routes_removal_template_bgprpcclient(
        self, bgp_rpc_client, to_test, dir, ipv="ipv4"
    ):
    ...
    def get_update_message_and_compare_with_hex_bgprpcclient(
        self, bgp_rpc_client, hex, option
    ):
    ...
    def check_for_l3vpn_odl_advertisement(self, announce_hex):
        ...
        for bgp_rpc_client, option in zip(BGP_RPC_CLIENTS, L3VPN_RT_CHECK):
            self.get_update_message_and_compare_with_hex_bgprpcclient(
                bgp_rpc_client, announce_hex, option
            )
    ...
    def verify_empty_reproted_data(self):
        ...
        mapping = {"PATH": f"peer=bgp:%2F%2F{ODL_3_IP}/adj-rib-out", "BGP_RIB": RIB_NAME}
        templated_requests.get_templated_request(
            f"{RT_CONSTRAIN_DIR}/empty_l3vpn", mapping=mapping, verify=True
        )
    ...
    def test_bgp_functional_rt_constrain_validation(
        ...
        with allure_step_with_separate_logging("step_start_bgp_peers"):
            ...
            log.info(f"IP: {ODL_IP} as_number: {BGP_PEER_AS_NUMBERS[0]}")
            self.start_bgp_peer(ODL_IP, BGP_PEER_AS_NUMBERS[0], 8002, multiplicity=1, filename=f"play.py.090.2")
            log.info(f"IP: {ODL_IP} as_number: {BGP_PEER_AS_NUMBERS[1]}")
            self.start_bgp_peer(ODL_IP, BGP_PEER_AS_NUMBERS[1], 8003, multiplicity=10, filename=f"play.py.090.3")
            log.info(f"IP: {ODL_IP} as_number: {BGP_PEER_AS_NUMBERS[2]}")
            self.start_bgp_peer(ODL_IP, BGP_PEER_AS_NUMBERS[2], 8004, multiplicity=9, filename=f"play.py.090.4")
        ...
        with allure_step_with_separate_logging("step_play_to_odl_ext_l3vpn_rt_arg"):
            ...
            self.play_to_odl_non_removal_bgprpcclient(
                BGP_RPC_CLIENT2, "ext_l3vpn_rt_arg", RT_CONSTRAIN_DIR
            )
            mapping = {
        ...
        with allure_step_with_separate_logging("step_play_to_odl_rt_constrain_type_0"):
            ...
            for i in range(int(BGP_PEERS_COUNT/2)):
                self.play_to_odl_non_removal_bgprpcclient(
                    BGP_RPC_CLIENT3, "rt_constrain_type_0", RT_CONSTRAIN_DIR, peer_id=i
                )
        ...
        with allure_step_with_separate_logging("step_play_to_odl_rt_constrain_type_1"):
            ...
            self.play_to_odl_non_removal_bgprpcclient(
                BGP_RPC_CLIENT4, "rt_constrain_type_1", RT_CONSTRAIN_DIR
            )
            mapping = {
