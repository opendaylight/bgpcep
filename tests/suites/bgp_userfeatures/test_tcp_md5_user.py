...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/bgpcep/tcpmd5user/tcpmd5user.robot
#
...
@pytest.mark.run(order=SuiteOrder.BGP_USER_TCP_MD5)
class TestTcpMd5User:
    ...
    def replace_password_on_pcep_node(self, password):
        """Send restconf PUT to replace the config module specifying PCEP
        password element."""
    ...
    def test_tcp_md5_user(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_topology_precondition"):
            # Compare current pcep-topology to empty pcep tology. Timeout
            # is long enough to ODL boot, to see that pcep is ready, with no
            # PCC is connected.
        ...
        with allure_step_with_separate_logging("step_start_secure_pcc_mock"):
            # Execute pcc-mock with password set, fail if pcc-mock promptly
            # exits. Keep pcc-mock running for next test cases.
        ...
        with allure_step_with_separate_logging("step_topology_unauthorized_2"):
            # The same logic as topology_unauthorized_1 as incorrect password
            # was provided to ODL.
            utils.wait_until_function_pass(10, 1, self.check_unathorized)
        ...
        with allure_step_with_separate_logging("step_topology_unauthorized_3"):
            # The same logic as Topology_Unauthorized_1, with no pcc-mock
            # running.
            utils.wait_until_function_pass(10, 1, self.check_unathorized)
        ...
        with allure_step_with_separate_logging("step_topology_updated"):
            # Compare pcep-topology/path-computation-client to
            # default_on_updated, which includes the updated tunnel.
            mapping = {
                "IP": TOOLS_IP,
                "CODE": TCP_MD5_VARIABLES["pcc_name_code"],
                "NAME": TCP_MD5_VARIABLES["pcc_name"],
