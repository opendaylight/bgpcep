...
class TestIbgpPeersBasic:
    ...
    def configure_ibgp_peers(self, ips: List[str], rr_client: bool, cluster_id: bool = False):
    ...
    def check_peer_adj_rib_out(self, peer_ip, skipped_prefix):
        ...
        peers1_data = [{"prefix": prefix + i*16, "cluster_id": cluster_id + i, "default_id": "127.0.0.4" } for i in range(int(BGP_PEERS_COUNT/2)) if prefix + i*16 != skipped_prefix]
        prefix = ipaddress.IPv4Address(BGP_PEER2_FIRST_PREFIX_IP)
        peers2_data = [{"prefix": prefix + i*16, "cluster_id": BGP_PEERS2_IP, "default_id": "192.0.2.2" } for i in range(int(BGP_PEERS_COUNT/2)) if prefix + i*16 != skipped_prefix]
        peers_data = peers1_data + peers2_data
        env = Environment(loader=FileSystemLoader(f"{BGP_DATA_FOLDER}/cluster_id/expected_peer_rib_out_manypeers"))
        # generate config file for bgp-flowspec-manypeers.cfg
        template = env.get_template("peer_rib_out.j2")
        expected_response = template.render({"peers_data": peers_data, "skipped_prefix": skipped_prefix})

        utils.verify_jsons_matach(response.text, expected_response, json1_data_label="received data", json2_data_label="expected data")
    ...
    def test_ibgp_peers_basic(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging(
            ...
            bgp_peers1_ips = [str(bgp_peers1_first_ip + i) for i in range(int(BGP_PEERS_COUNT/2))]
            ...
            bgp_peers2_ips = [str(bgp_peers2_first_ip + i) for i in range(int(BGP_PEERS_COUNT/2))]
