...
class TestIbgpPeersBasic:
    ...
    def check_peer_adj_rib_out(self, peer_ip, skipped_prefix):
        ...
        templated_requests.get_jinja_templated_request(
            template_dir=f"{BGP_DATA_FOLDER}/cluster_id/peer_rib_out_manypeers",
            mapping={
    ...
    @allure.description(
        textwrap.dedent(
            ...
            **Test Case 2**: Two iBGP peers groups: one RR client group and one \
            non-client group introduces prefixes
            Expected result: controller forwards updates towards both peers groups
