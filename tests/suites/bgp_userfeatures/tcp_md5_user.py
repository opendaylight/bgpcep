...
class TestTcpMd5User:
    ...
    def check_unathorized(self):
        ...
        empty topology."""
        templated_requests.get_templated_request(f"{DIR_WITH_TEMPLATES}/default_off", None, verify=True)
    ...
    def replace_password_on_pcep_node(self, password):
        ...
        templated_requests.put_templated_request(f"{DIR_WITH_TEMPLATES}/pcep_topology_node", mapping, json=False)

    def unset_password_on_pcep_node(self):
        """Send restconf PUT to unset the config module."""
        mapping = {"IP": TOOLS_IP}
        templated_requests.put_templated_request(f"{DIR_WITH_TEMPLATES}/pcep_topology_node_empty", mapping, json=False)

    def start_pcc_mock_tool_with_password(self, password):
        """Starts pcc-mock with password argument."""
        self.pcep_mock_process = infra.shell(f"java -jar build_tools/pcep-pcc-mock.jar --log-level debug --password {password} --reconnect 1 --local-address {TOOLS_IP} --remote-address {ODL_IP} 2>&1 | tee pccmock.log", run_in_background=True)
    ...
    def test_tcp_md5_user(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_topology_precondition"):
            ...
            utils.wait_until_function_pass(
                300, 1, templated_requests.get_templated_request, f"{DIR_WITH_TEMPLATES}/default_off", None, verify=True
            )
        ...
        with allure_step_with_separate_logging("step_topology_intercondition"):
            ...
            mapping = {"IP": TOOLS_IP, "CODE": TCP_MD5_VARIABLES["pcc_name_code"], "NAME": TCP_MD5_VARIABLES["pcc_name"], "IP_ODL": ODL_IP, "ERRORS": ERROR_ARGS}
            utils.wait_until_function_pass(30, 1, templated_requests.get_templated_request, f"{DIR_WITH_TEMPLATES}/default_on_state", mapping, verify=True)
        ...
        with allure_step_with_separate_logging("step_update_delegated"):
            ...
            response = templated_requests.post_templated_request(f"{DIR_WITH_TEMPLATES}/update_delegated", mapping, json=False)
        ...
        with allure_step_with_separate_logging("step_topology_updated"):
            ...
            mapping = {"IP": TOOLS_IP, "CODE": TCP_MD5_VARIABLES["pcc_name_code"], "NAME": TCP_MD5_VARIABLES["pcc_name"], "IP_ODL": ODL_IP, "ERRORS": ERROR_ARGS}
            utils.wait_until_function_pass(30, 1, templated_requests.get_templated_request, f"{DIR_WITH_TEMPLATES}/default_on_updated_state", mapping, verify=True)
        ...
        with allure_step_with_separate_logging("step_topology_postcondition"):
            ...
            utils.verify_function_does_not_fail_within_timeout(10, 1, self.check_unathorized)

        with allure_step_with_separate_logging("step_delete_pcep_client_module"):
            """Delete Pcep client module."""
            mapping = {"IP": TOOLS_IP}
            templated_requests.delete_templated_request(f" {DIR_WITH_TEMPLATES}/pcep_topology_node", mapping, json=False)