...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/bgpcep/bgpfunct/060_bgp_functional_mvpn.robot
#
...
@pytest.mark.run(order=SuiteOrder.BGP_USER_MANY_MVPN)
    ...
    def test_bgp_functional_Mvpn(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_odl_to_play_intra_as_ipmsi_ad"):
            bgp.odl_to_play_template(
                "intra_as_ipmsi_ad", MVPN_DIR, peers_count=BGP_PEERS_COUNT
            )

        with allure_step_with_separate_logging("step_play_to_odl_intra_as_ipmsi_ad"):
            bgp.play_to_odl_template(
                "intra_as_ipmsi_ad", MVPN_DIR, peer_id=0 % BGP_PEERS_COUNT
            )

        with allure_step_with_separate_logging("step_odl_to_play_inter_as_ipmsi_ad"):
            bgp.odl_to_play_template(
                "inter_as_ipmsi_ad", MVPN_DIR, peers_count=BGP_PEERS_COUNT
            )

        with allure_step_with_separate_logging("step_play_to_odl_inter_as_ipmsi_ad"):
            bgp.play_to_odl_template(
                "inter_as_ipmsi_ad", MVPN_DIR, peer_id=1 % BGP_PEERS_COUNT
            )
        ...
        with allure_step_with_separate_logging(
            ...
            bgp.odl_to_play_template(
                "intra_pe_distinguisher", MVPN_DIR, peers_count=BGP_PEERS_COUNT
            )
        ...
        with allure_step_with_separate_logging(
            ...
            bgp.play_to_odl_template(
                "intra_pe_distinguisher", MVPN_DIR, peer_id=7 % BGP_PEERS_COUNT
            )
