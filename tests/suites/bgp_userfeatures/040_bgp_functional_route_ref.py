...
class TestBgpfunctionalRouteRef:
    ...
    def verify_cli_output_count(self, notification_count, update_count, receive_count):
        ...
        stdout, stderror = infra.execute_karaf_command(f"bgp:operational-state -rib example-bgp-rib -neighbor ${TOOLS_IP}")
        log.info(f"Karaf stdout: {stdout}")
        mapping = {"IP": TOOLS_IP, "NOT_COUNT": notification_count, "SEND_COUNT": update_count, "RECV_COUNT": receive_count, "DIVIDER": "|"}
        exp_state = templated_requests.resolve_templated_text(f"{BGP_RR_VAR_FOLDER}/operational_cli/update.txt", mapping)
        log.info(f"Expected state: {exp_state}")
        line_count = exp_state.find("\n") + 1
        real_state = "\n".join(stdout.splitlines()[MSG_STATE_OFFSET:MSG_STATE_OFFSET+line_count])
        utils.verify_multiline_text_match(exp_state, real_state) 

    def verify_odl_operational_state_count(self, notification_count: int, update_count: int, receive_count: int):
        """Check notification and update count gained from operatial neighbor state
        It verifies these counts also against cli output."""
        mapping = {"IP": TOOLS_IP, "RIB_INSTANCE_NAME": RIB_INSTANCE, "NOT_COUNT": notification_count, "SEND_COUNT": update_count, "RECV_COUNT": receive_count}
        utils.wait_until_function_pass(3, 5, templated_requests.get_templated_request, f"{BGP_RR_VAR_FOLDER}/operational_state", mapping, verify=True)
        utils.wait_until_function_pass(3, 5, self.verify_cli_output_count, notification_count, update_count, receive_count)
    ...
    def configure_routes_and_start_exabgp(self, cfg_file: str):
        ...
        for prefix in ("1.1.1.1/32", "2.2.2.2/32"):
            ...
            templated_requests.post_templated_request(f"{BGP_RR_VAR_FOLDER}/route", mapping, json=False)
        self.exabgp_process = bgp.start_exabgp_and_verify_connected(cfg_file, TOOLS_IP, "exabgp.log")
    ...
    def deconfigure_routes_and_stop_exabgp(self, cfg_file: str):
        ...
        mapping = {"PREFIX": prefix, "APP_RIB": ODL_IP}
        templated_requests.delete_templated_request(f"{BGP_RR_VAR_FOLDER}/route", mapping)
    ...
    def test_bgp_functional_route_ref(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_exa_to_send_route_refresh"):
            ...
            try:
                ...
                update_count = 3
                utils.wait_until_function_pass(5, 2, self.verify_exaBgp_received_updates,update_count)
                # From neon onwards there are extra BGP End-Of-RIB message per address family
                update_count = 7
                utils.wait_until_function_pass(3, 5, self.verify_odl_operational_state_count, notification_count=0, update_count=update_count, receive_count=2)
            finally:
        ...
        with allure_step_with_separate_logging("step_odl_to_send_route_refresh"):
            ...
            try:
                ...
                templated_requests.post_templated_request(f"{BGP_VAR_FOLDER}/route_refresh", mapping, json=False)
                utils.wait_until_function_pass(5, 2, self.verify_exaBgp_received_route_refresh, 1)
                # From neon onwards there are extra BGP End-Of-RIB message per address family
                update_count = 9
                utils.wait_until_function_pass(3, 5, self.verify_odl_operational_state_count, notification_count=1, update_count=update_count, receive_count=4)
            finally:
