...
BGP_VAR_FOLDER = "data/bgpfunctional/ipv6"
...
class TestBgpIpv6Basic:
    ...
    def configure_ipv6_network(self):
        ...
        infra.shell(f"sudo ip -6 addr add {IPV6_IP}/{IPV6_PREFIX_LENGTH} dev {main_net_interface}")
    ...
    def setup_config_files(self):
        for config_file in (EXABGP_CFG, EXABGP3_CFG, EXABGP4_CFG):
            ...
            self.setup_config_file(f"tmp/{config_file}", exabgp_ip=IPV6_IP, odl_ip=CONTROLLER_IPV6, router_id=EXABGP_ID)
        infra.shell(f"cp  {BGP_VAR_FOLDER}/{EXABGP2_CFG} tmp/")
        self.setup_config_file(f"tmp/{EXABGP2_CFG}", exabgp_ip=IPV4_IP, odl_ip=ODL_IP, router_id=IPV4_IP)
    ...
    def verify_rib_status_empty(self):
        ...
        utils.wait_until_function_pass(5, 2, templated_requests.get_templated_request, f"{BGP_VAR_FOLDER}/ipv6_topology_empty", {}, verify=True)

    def verify_rib_status_filled(self):
        """Verifies that example-ipv6-topology is filled with ipv6 route"""
        utils.wait_until_function_pass(5, 2, templated_requests.get_templated_request, f"{BGP_VAR_FOLDER}/ipv6_topology_filled", {}, verify=True)
    ...
    def test_bgp_ipv6_basic(self, allure_step_with_separate_logging, step_tag_checker):
        ...
        with allure_step_with_separate_logging("step_reconfigure_odl_to_accept_connections"):
            ...
            mapping = {"IP": IPV6_IP, "PEER_PORT": 17900, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "HOLDTIME": HOLDTIME, "PASSIVE_MODE": "true"}
            templated_requests.put_templated_request(f"{BGP_VAR_FOLDER}/bgp_peer", mapping, json=False)

        with allure_step_with_separate_logging("step_start_exabgp"):
            """Start exabgp."""
            self.exabgp_process = bgp.start_exabgp_and_verify_connected(f"tmp/{EXABGP_CFG}", EXABGP_ID, log_file=EXABGP_LOG)
        ...
        with allure_step_with_separate_logging("step_delete_bgp_peer_configuration"):
            ...
            templated_requests.delete_templated_request(f"{BGP_VAR_FOLDER}/bgp_peer", mapping)
        ...
        with allure_step_with_separate_logging("step_reconfigure_odl_to_accept_connections_2"):
            """Configure BGP peer modules with initiate-connection set to false with ipv6 address without "::" shortened version."""
            mapping = {"IP": IPV6_IP_2, "PEER_PORT": 17900, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "HOLDTIME": HOLDTIME, "PASSIVE_MODE": "true"}
            templated_requests.put_templated_request(f"{BGP_VAR_FOLDER}/bgp_peer", mapping, json=False)
        ...
        with allure_step_with_separate_logging("step_reconfigure_odl_to_accept_connections_3"):
            """Configure BGP peer modules with initiate-connection set to false with full text ipv6 address."""
            mapping = {"IP": IPV6_IP_3, "PEER_PORT": 17900, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "HOLDTIME": HOLDTIME, "PASSIVE_MODE": "true"}
            templated_requests.put_templated_request(f"{BGP_VAR_FOLDER}/bgp_peer", mapping, json=False)
        ...
        with allure_step_with_separate_logging("step_configure_app_peer"):
            ...
            templated_requests.put_templated_request(f"{BGP_VAR_FOLDER}/application_peer", mapping, json=False)

        with allure_step_with_separate_logging("step_reconfigure_odl_to_accept_connections_4"):
            """Configure BGP peer modules with initiate-connection set to false with full text ipv6 address."""
            mapping = {"IP": IPV4_IP, "PEER_PORT": 17900, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "HOLDTIME": HOLDTIME, "PASSIVE_MODE": "true"}
            templated_requests.put_templated_request(f"{BGP_VAR_FOLDER}/bgp_neighbor_rib", mapping, json=False)

        with allure_step_with_separate_logging("step_start_exabgp_2"):
            """Start exabgp and Verify BGP connection."""
            self.exabgp_process = bgp.start_exabgp_and_verify_connected(f"tmp/{EXABGP2_CFG}", IPV4_IP, log_file=EXABGP2_LOG)
        ...
        with allure_step_with_separate_logging("step_inject_ipv6_route_1"):
            ...
            templated_requests.post_templated_request(f"{BGP_VAR_FOLDER}/ipv6_route_injection", mapping, json=False)
        ...
        with allure_step_with_separate_logging("step_check_ipv6_prefix_in_bgp_rib_1"):
            ...
            utils.wait_until_function_pass(5, 2, templated_requests.get_templated_request, f"{BGP_VAR_FOLDER}/bgp_rib", mapping, verify=False)

        with allure_step_with_separate_logging("step_delete_injected_ipv6_routes_1"):
            """Delete the injected IPV6 routes."""
            mapping = {"IP": "127.0.0.12"}
            templated_requests.delete_templated_request(f"{BGP_VAR_FOLDER}/ipv6_route_injection", mapping)
            karaf.fail_if_exception_found_during_test("step_delete_injected_ipv6_routes_1")
        ...
        with allure_step_with_separate_logging("step_delete_bgp_peer_configuration_4"):
            ...
            templated_requests.delete_templated_request(f"{BGP_VAR_FOLDER}/bgp_neighbor_rib", mapping)

        with allure_step_with_separate_logging("step_verify_ipv6_topology_empty_4"):
            """Verifies that example-ipv6-topology is empty after deconfiguring peer for the first time."""
            self.verify_rib_status_empty()

        with allure_step_with_separate_logging("step_reconfigure_odl_to_accept_connections_5"):
            """Configure BGP peer modules with initiate-connection set to false with short ipv6 address."""
            mapping = {"IP": IPV4_IP, "PEER_PORT": 17900, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "HOLDTIME": HOLDTIME, "PASSIVE_MODE": "true"}
            templated_requests.put_templated_request(f"{BGP_VAR_FOLDER}/bgp_peer", mapping, json=False)
        ...
        with allure_step_with_separate_logging("step_check_ipv6_prefix_in_bgp_rib_2"):
            """Check for the presence of Ipv6 Prefix in the BGP RIB."""
            mapping = {"BGP_RIB_OPENCONFIG": "example-bgp-rib"}
            templated_requests.get_templated_request(f"{BGP_VAR_FOLDER}/bgp_rib", mapping, json=False)
        ...
        with allure_step_with_separate_logging("step_delete_app_peer"):
            ...
            mapping = {"IP": "127.0.0.12", "BGP_RIB_OPENCONFIG": "example-bgp-rib"}
            templated_requests.delete_templated_request(f"{BGP_VAR_FOLDER}/application_peer", mapping)
        ...
        with allure_step_with_separate_logging("step_reconfigure_odl_to_accept_connections_6"):
            """Configure BGP peer modules with initiate-connection set to false with short ipv6 address."""
            mapping = {"IP": IPV6_IP, "PEER_PORT": 17900, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "HOLDTIME": HOLDTIME, "PASSIVE_MODE": "true"}
            templated_requests.put_templated_request(f"{BGP_VAR_FOLDER}/graceful_restart", mapping, json=False)

        with allure_step_with_separate_logging("step_start_exabgp_3"):
            """Start exabgp."""
            self.exabgp_process = bgp.start_exabgp_and_verify_connected(f"tmp/{EXABGP3_CFG}", EXABGP_ID, log_file=EXABGP3_LOG)
        ...
        with allure_step_with_separate_logging("step_delete_bgp_peer_configuration_6"):
            """Revert the BGP configuration to the original state: without any configured peers."""
            mapping = {"IP": IPV6_IP, "BGP_RIB_OPENCONFIG": "example-bgp-rib"}
            templated_requests.delete_templated_request(f"{BGP_VAR_FOLDER}/graceful_restart", mapping)
        ...
        if step_tag_checker("exclude"):
            with allure_step_with_separate_logging("step_reconfigure_odl_to_accept_connections_7"):
                """Configure BGP peer modules with initiate-connection set to false with short ipv6 address."""
                mapping = {"IP": IPV6_IP, "PEER_PORT": 17900, "BGP_RIB_OPENCONFIG": "example-bgp-rib", "HOLDTIME": HOLDTIME, "PASSIVE_MODE": "true"}
                templated_requests.put_templated_request(f"{BGP_VAR_FOLDER}/bgp_peer", mapping, json=False)
            ...
            with allure_step_with_separate_logging("step_start_exabgp_5"):
                """Start exabgp."""
                self.exabgp_process = bgp.start_exabgp_and_verify_connected(f"tmp/{EXABGP4_CFG}", EXABGP_ID, log_file=EXABGP4_LOG)
