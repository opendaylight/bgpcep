...
# hhttps://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/bgpcep/bgpfunct/bgp_policies_default.robot
#
...
POLICIES_VAR = "variables/bgpfunctional/bgppolicies"
...
@pytest.mark.run(order=SuiteOrder.BGP_USER_POLICIES)
class TestBgpPoliciesDefault:
    exabgp_processes = []

    def setup_config_files(self):
        """Copies exabgp config files and replaces variables within those
    ...
    def verify_rib_status(self):
        ...
        for index in range(1, 7):
            ...
            utils.wait_until_function_pass(
                ...
                f"{POLICIES_VAR}/effective_rib_in/peer_{index}",
                mapping,
                verify=True,
            )
    ...
    @allure.description(
        textwrap.dedent(
            ...
            *2x ibgp* Tests results on effective-rib-in dependant on their \
    ...
    def test_bgp_policies_default(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_configure_app_peer"):
            ...
            templated_requests.post_templated_request(
                f"{POLICIES_VAR}/app_peer_route", mapping, json=False
            )
        ...
        with allure_step_with_separate_logging(
            ...
            for index, peer_type in enumerate(PEER_TYPES):
                mapping = {
                    ...
                    "RIB_INSTACE_NAME": RIB_INSTANCE,
                    "HOLDTIME": HOLDTIME,
                    "PASSIVE_MODE": "true",
                }
                templated_requests.put_templated_request(
                    f"{POLICIES_VAR}/{peer_type}", mapping, json=False
                )
        ...
        with allure_step_with_separate_logging("step_delete_bgp_peer_configuration"):
            ...
            for index, peer_type in enumerate(PEER_TYPES):
                ...
                templated_requests.delete_templated_request(
                    f"{POLICIES_VAR}/{peer_type}", mapping
                )
        ...
        with allure_step_with_separate_logging("step_deconfigure_app_peer"):
            ...
            templated_requests.delete_templated_request(
                f"{POLICIES_VAR}/app_peer_route", mapping
            )
