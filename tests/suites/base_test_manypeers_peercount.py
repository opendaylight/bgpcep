...
# with its thread name, e.g. "BGP-Peer-1:") is redirected to tmp/<this file>.
...
class BaseTestManyPeerPeerCount:
    ...
    def test_many_peers_peer_count(
        ...
        with allure_step_with_separate_logging("step_check_logs_for_updates"):
            ...
            # prefixes. play.py splits the total prefix count across the peers,
            # so each peer advertises count_peer_count_many // bgp_peers_count
            # prefixes. As route-reflector clients, ODL reflects every peer's
            # routes to all the others, so each peer receives the total minus
            # the share it advertised itself.
            prefixes_per_peer = count_peer_count_many // bgp_peers_count
            expected_prefix_count = count_peer_count_many - prefixes_per_peer
            for index in range(1, bgp_peers_count + 1):
                expected_log_line = (
                    f"BGP-Peer-{index}: "
                    f"total_received_nlri_prefix_counter: {expected_prefix_count}"
                )
                # The counter is cumulative and increases monotonically, so
