...
BGP_FILLING_TIMEOUT = TEST_DURATION_MULTIPLIER * (COUNT_CHANGE_COUNT_SINGLE * 9.0 / 10000 + 20)
...
class TestSinglePeerChangeCount:
    ...
    def test_single_peer_change_count(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging(
            ...
            change_counter.wait_for_change_count_to_become_stable(
                ...
                timeout=BGP_FILLING_TIMEOUT, wait_period=CHECK_PERIOD_CHANGE_COUNT_SINGLE, consecutive_times_stable_value=REPETITIONS_CHANGE_COUNT_SINGLE
            )
        ...
        with allure_step_with_separate_logging("step_check_talking_ip_topology_count"):
            ...
            not correct."""
            prefix_counting.check_ipv4_topology_prefixes_count(COUNT_CHANGE_COUNT_SINGLE)
        ...
        with allure_step_with_separate_logging(
            ...
            infra.backup_file(f"totals-{RESULTS_FILE_NAME}" , target_file_name=f"changecount-talking-totals-${RESULTS_FILE_NAME}", src_dir=".")
            infra.backup_file(f"performance-{RESULTS_FILE_NAME}" , target_file_name=f"changecount-talking-performance-{RESULTS_FILE_NAME}", src_dir=".")
        ...
        with allure_step_with_separate_logging(
            ...
            """Store results for plotting."""
            infra.backup_file(f"totals-{RESULTS_FILE_NAME}" , target_file_name=f"changecount-listening-totals-${RESULTS_FILE_NAME}", src_dir=".")
            infra.backup_file(f"performance-{RESULTS_FILE_NAME}" , target_file_name=f"changecount-listening-performance-{RESULTS_FILE_NAME}", src_dir=".")
