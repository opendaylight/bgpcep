...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/bgpcep/bgpingest/manypeers_prefixcount.robot
#
...
@pytest.mark.run(order=SuiteOrder.BGP_INGEST_MANYPEERS_PREFIX_COUNT)
    ...
    test_description = textwrap.dedent(
        """
            **This suite uses play.py processes as multiple iBGP peers.**
            ...
            peers. Each peer is of ibgp type, and they contribute to the same \
            ...
            **TODO:** Currently, if a bug causes prefix count to remain at \
            ...
            If zero is allowed as stable, higher period or repetitions would be \
            required.

            The prefix counting is quite heavyweight and may induce large \
            variation in time. Try the other version of the suite \
            (test_manypeers_change_count.py) to get better precision.
            ...
            **TODO:** Figure out how to make it configurable. \
            As peer IP adresses are set incrementally, we need ipaddr \
            to be used in PyTest somehow.
            ...
            **TODO:** Is there a need for version of this suite where ODL \
            connects to pers?
            Note that configuring ODL is slow, which may affect measured \
            performance singificantly.
