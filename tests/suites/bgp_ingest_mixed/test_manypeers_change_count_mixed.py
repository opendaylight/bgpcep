...
# https://github.com/opendaylight/integration-test/blob/901c7e139945b436d95a44b3b592904c3d7a4f9f/csit/suites/bgpcep/bgpingest/manypeers_changecount.robot
#
...
class TestManyPeersChangeCountMixed(BaseTestManyPeerChangeCount):
    test_description = textwrap.dedent(
        """
            **BGP performance of ingesting from multiple iBGP peers**

            Data change counter is used. This suite uses play.py as single \
            iBGP peer. The suite only looks at example-ipv4-topology, so RIB \
            is not examined. This suite requires odl-bgpcep-data-change-counter \
            to be installed so make sure it is added to "install-features" \
            of any jobs that are going to invoke it. The suite consists of two \
            halves, differing on which side initiates BGP connection. \
