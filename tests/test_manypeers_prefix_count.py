...
class TestManyPeerPrefixCount:
    ...
    def test_change_karaf_logging_levels(self):
        infra.execute_karaf_command(f"log:set {KARAF_BGPCEP_LOG_LEVEL} org.opendaylight.bgpcep")
        infra.execute_karaf_command(f"log:set {KARAF_BGPCEP_LOG_LEVEL} org.opendaylight.protocol")

    @pytest.mark.skip_if_fails("test_reconfigure_odl_to_accept_connection")
    def test_start_talking_bgp_manager(self):
        TestManyPeerPrefixCount.bgp_speaker_process = bgp.start_bgp_speaker(ammount=PREFIXES_COUNT,
                          multiplicity=BGP_PEERS_COUNT,
                          listen=False)

    @pytest.mark.skip_if_fails(["test_reconfigure_odl_to_accept_connection",
                                "test_start_talking_bgp_manager"])
    ...
    def test_restore_karaf_logging_levels(self):
        infra.execute_karaf_command(f"log:set {KARAF_LOG_LEVEL} org.opendaylight.bgpcep")
        infra.execute_karaf_command(f"log:set {KARAF_LOG_LEVEL} org.opendaylight.protocol")

    def test_delete_bgp_peer_configuartion(self):
        bgp.delete_bgp_neighbours(first_neigbout_ip=FIRST_PEER_IP, count=BGP_PEERS_COUNT)