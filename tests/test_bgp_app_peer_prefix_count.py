...
class TestBgpAppPeerPrefixCount:
    ...
    def test_bgp_application_peer_prefill_routes(self):
        bgp.start_bgp_app_peer(count=PREFILL_COUNT, log_level="debug", timeout=3000)
    ...
    def test_bgp_application_peer_intorduce_single_routes(self):
        bgp.start_bgp_app_peer(count=ADDITIONAL_COUNT, command="add", prefix="12.0.0.0", log_level="debug", timeout=3000)
    ...
    def test_bgp_peer_updates_for_all_routes(self):
        count = utils.wait_for_string_in_file(20, 5, f"total_received_nlri_prefix_counter: {TOTAL_COUNT}", "bgp_peer.log")
        assert 1 <= count, "Did not find expected received prefixes in bgp_peer.log file"
    ...
    def test_bgp_application_peer_delete_all_routes(self):
        bgp.start_bgp_app_peer(command="delete-all", log_level="debug", timeout=3000)
    ...
    def test_bgp_peer_updates_for_prefix_withdrawals(self):
        count = utils.wait_for_string_in_file(20, 5, f"total_received_withdrawn_prefix_counter: {TOTAL_COUNT}", "bgp_peer.log")
        assert 1 <= count, "Did not find expected received prefixes in bgp_peer.log file"
