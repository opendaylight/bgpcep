...
class TestBgpAppPeerPrefixCount:
    ...
    def test_bgp_application_peer_prefill_routes(self):
        bgp.start_bgp_app_peer(count=PREFILL_COUNT, log_level="debug", timeout=300)
    ...
    def test_bgp_application_peer_intorduce_single_routes(self):
        bgp.start_bgp_app_peer(count=ADDITIONAL_COUNT, command="add", prefix="12.0.0.0", log_level="debug", timeout=300)
    ...
    def test_bgp_application_peer_delete_all_routes(self):
        bgp.start_bgp_app_peer(command="delete-all", log_level="debug", timeout=300)
