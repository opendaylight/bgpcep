...
class TestBgpAppPeerPrefixCount:
    ...
    def test_wait_for_ip_topology_is_prefilled(self):
        count = utils.retry_function_and_expect_value(
            retry_count=20,
            interval=5,
    ...
    def test_wait_for_ip_topology_is_filled(self):
        count = utils.retry_function_and_expect_value(
            20, 5, TOTAL_COUNT, ip_topology.get_ipv4_topology_prefixes_count
        )
        assert (
            count == TOTAL_COUNT
        ), "Not all expected prefixes were found in ipv4 topology"
