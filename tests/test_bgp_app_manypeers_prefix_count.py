...
class TestBgpAppPeerPrefixCount:
    ...
    def test_reconfigure_odl_to_accept_connection(self):
        bgp.set_bgp_neighbours(first_neigbout_ip="127.0.0.1", count=BGP_PEERS_COUNT, passive_mode=True)
    ...
    def test_bgp_peer_updates_for_prefilled_routes(self):
        for i in range(BGP_PEERS_COUNT):
            count = infra.wait_for_string_in_file(
                5, 3, f"total_received_nlri_prefix_counter: {PREFILL_COUNT}", f"bgp_peer_{i}.log"
            )
            assert (
                1 <= count
            ), f"Did not find expected received prefixes in bgp_peer_{i}.log file"
    ...
    def test_bgp_peer_updates_for_all_routes(self):
        for i in range(BGP_PEERS_COUNT):
            count = infra.wait_for_string_in_file(
                20, 5, f"total_received_nlri_prefix_counter: {TOTAL_COUNT}", "bgp_peer_{i}.log"
            )
            assert (
                1 <= count
            ), f"Did not find expected received prefixes in bgp_peer_{i}.log file"
