...
BGP_FILLING_TIMEOUT = TEST_DURATION_MULTIPLIER * (PREFILL_COUNT * 4.0 / 10000 + 80)
...
class TestBgpAppManyPeerPrefixCount:
    ...
    def test_bgp_app_many_peers_prefix_count(self, step_logger):

        with step_logger("step_check_for_empty_topology_before_starting"):
        ...
        with step_logger("step_reconfigure_odl_to_accept_bgp_application_peer"):
            """Configure BGP application peer module."""
            bgp.set_bgp_application_peer(ip="10.0.0.10")

        with step_logger("step_connect_bgp_peers"):
            """Start BGP peer tool"""
        ...
        with step_logger("step_bgp_application_peer_intorduce_single_routes"):
        ...
        with step_logger("step_disconnect_bgp_peer"):
            """Stop BGP peer tool"""
            bgp.stop_bgp_speaker(self.bgp_speaker_process)
            infra.shell("mv bgp_peer.log results/bgp_peer_disconnect.log")

        with step_logger("step_reconnect_bgp_peer"):
            """Start BGP peer tool"""
            TestBgpAppManyPeerPrefixCount.bgp_speaker_process = bgp.start_bgp_speaker(
        ...
        with step_logger("step_wait_for_stable_topology_after_deletion"):
        ...
        with step_logger("step_check_for_empty_topology_after_deletion"):
