...
class TestBgpAppManyPeerPrefixCount:
    ...
    def test_bgp_app_many_peers_prefix_count(self, allure_step_with_separate_logging):
        ...
        with allure_step_with_separate_logging("step_check_for_empty_topology_before_starting"):
        ...
        with allure_step_with_separate_logging("step_reconfigure_odl_to_accept_connections"):
            ...
            bgp.set_bgp_neighbours(
                first_neigbout_ip="127.0.0.1", count=BGP_PEERS_COUNT, passive_mode=True
            )
        ...
        with allure_step_with_separate_logging("step_connect_bgp_peers"):
            ...
            TestBgpAppManyPeerPrefixCount.bgp_speaker_process = bgp.start_bgp_speaker(
                ammount=0, multiplicity=BGP_PEERS_COUNT, listen=False, log_level="info"
            )
            assert infra.is_process_still_running(
                TestBgpAppManyPeerPrefixCount.bgp_speaker_process
            ), "Bgp speaker process is not running"

        with allure_step_with_separate_logging("step_bgp_application_peer_prefill_routes"):
            ...
            infra.shell(
                "mv bgp_app_peer.log results/bgp_app_peer_prefill.log", check_rc=True
            )

        with allure_step_with_separate_logging("step_wait_for_ip_topology_is_prefilled"):
        ...
        with allure_step_with_separate_logging("step_check_bgp_peer_updates_for_prefilled_routes"):
        ...
        with allure_step_with_separate_logging("step_bgp_application_peer_intorduce_single_routes"):
        ...
        with allure_step_with_separate_logging("step_check_bgp_peer_updates_for_all_routes"):
            """Count the routes introduced by updates."""
            count = infra.wait_for_string_in_file(
                CHECK_RETRY_COUNT,
                CHECK_INTERVAL,
        ...
        with allure_step_with_separate_logging("step_check_bgp_peer_updates_for_reintroduced_routes"):
            """Count the routes introduced by updates."""
            count = infra.wait_for_string_in_file(
                CHECK_RETRY_COUNT,
                CHECK_INTERVAL,
        ...
        with allure_step_with_separate_logging("step_bgp_application_peer_delete_all_routes"):
        ...
        with allure_step_with_separate_logging("step_wait_for_stable_topology_after_deletion"):
        ...
        with allure_step_with_separate_logging("step_check_for_empty_topology_after_deletion"):
            """Example-ipv4-topology should be empty now."""
            count = ip_topology.get_ipv4_topology_prefixes_count()
            assert (
                0 == count
            ), "There are still some prefixes in ipv4 topology after deletion, expected empty"

        with allure_step_with_separate_logging("step_check_bgp_peer_updates_for_prefix_withdrawals"):
