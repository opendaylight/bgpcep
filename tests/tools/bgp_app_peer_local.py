...
def post_prefixes(
    ...
    xml_data = " ".join( [chunk.decode() for chunk in _stream_data(xml_template, prefix_base, prefix_len, count, route_key)])
    logger.warn(xml_data)
    """send_request(
...
def put_prefixes(
    ...
    xml_data = "\n".join(list(_stream_data(xml_template, prefix_base, prefix_len, count, route_key)))
    logger.warn(xml_data)
    """send_request(
        "PUT",
        odl_ip,
