...
def configure_speaker_entitiy_identifier():
    ...
    resposne = templated_requests.put_templated_request(
        "variables/pcepuser/titanium/node_speaker_entity_identifier", mapping, json=False
    )
...
def add_lsp(pcc_ip: str, lsp_name: str):
    ...
    response = operate_xml_lsp_return_json("network-topology-pcep:add-lsp", VARIABLES["add_instantiated_xml"])
    return response
...
def update_lsp(pcc_ip: str, lsp_name: str):
   ...
   # 
    #return resposne
    response = operate_xml_lsp_return_json("network-topology-pcep:update-lsp", VARIABLES["update_instantiated_xml"])
    return response
...
def remove_lsp(pcc_ip: str, lsp_name: str):
    ...
    #
    #return resposne
    response = operate_xml_lsp_return_json("network-topology-pcep:remove-lsp", VARIABLES["remove_instantiated_xml"])
    return response

def operate_xml_lsp_return_json(uri_part, xml_data):
    uri_path = f"/rests/operations/{uri_part}"
    response = templated_requests.post_to_uri(uri_path, None, xml_data, expected_code=templated_requests.ALLOWED_STATUS_CODES)
