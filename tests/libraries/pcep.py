...
def get_stats(pcc_ip: str | None = None, verify_response: bool = False) -> requests.Response:
    ...
    response = templated_requests.post_templated_request_2(
...
def verify_stats_pcc_count(expected_pcc_count: int, expected_lsps_per_pcc_count: int = 1):
    ...
    assert expected_pcc_count == nodes_count, (
        f"Number of returned {expected_pcc_count} node/s in \"get-stats\" RPC "
        f"does not match expected {nodes_count} node/s."
    )
...
def set_stat_timer_value(
    ...
    response = templated_requests.put_templated_request_2(
