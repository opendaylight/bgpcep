...
def set_bgp_peer_group_members(
    ...
    Args:
        peer_group_name: Name of peer group to which BGP neighbours should be
            assigned.
        first_neighbour_ip (str): First ip address from sequention used for
...
def start_bgp_speaker(
    ...
    usepeerip: bool | None = None,
    allf: bool | None = None,
    evpn: bool | None = None,
    mvpn: bool | None = None,
    l3vpn_mcast: bool | None = None,
    rt_constrain: bool | None = None,
    wfr: int | None = None,
    ...
    Args:
        ...
        update (int): Number of ipv4 prefixes to be updated in ODL
            within one message.
        updates (str): 'single' or 'separate' updates for
            NLRI and WITHDRAWN lists
        firstprefix (str): Base prefix for prefix generation.
        prefixlen (int): The prefix lenght.
        ...
        as_number (int): Autonomus system number to be used.
        ...
        usepeerip (bool): Use peerip instead of myip for RPC server.
        allf (bool): Add all supported families without message decoding.
        evpn (bool): Open message includes multiprotocol extension
            capability l2vpn-evpn.
        mvpn (bool): Open message includes Graceful-restart capability,
            containing AFI/SAFIS: IPV4-Unicast, IPV6-Unicast, BGP-LS
        l3vpn_mcast (bool): Open message includes L3VPN-MULTICAST arguments.
        rt_constrain (bool): Open message includes ROUTE-TARGET-CONSTRAIN
            arguments.
        wfr (int): Wait for read timeout.
        cluster (str): Cluster list item identifier.
    ...
    if updates is not None:
        command_parts.append(f"--updates={updates}")
    if firstprefix is not None:
        command_parts.append(f"--firstprefix={firstprefix}")
    if prefixlen is not None:
        command_parts.append(f"--prefixlen={prefixlen}")
    if multiplicity is not None:
        command_parts.append(f"--multiplicity={multiplicity}")
    if listen:
        command_parts.append("--listen")
    if port:
        command_parts.append(f"--port={port}")
    if as_number:
        command_parts.append(f"--asnumber={as_number}")
    if bgpls is not None:
        command_parts.append(f"--bgpls {bgpls}")
    if skipAttr:
        command_parts.append("--skipattr")
    if usepeerip:
        command_parts.append("--usepeerip")
    if allf:
        command_parts.append("--allf")
    if evpn:
        command_parts.append("--evpn")
    if mvpn:
        command_parts.append("--mvpn")
    if l3vpn_mcast:
        command_parts.append("--l3vpn_mcast")
    if rt_constrain:
        command_parts.append("--rt_constrain")
    if cluster is not None:
        command_parts.append(f"--cluster {cluster}")
    if wfr is not None:
    ...
    process = controller_testlib.infra.shell(
        " ".join(command_parts), run_in_background=True
    )
...
def start_bgp_speaker_with_verify_and_retry(
    ...
    Returns:
        subprocess.Popen: BGP speaker process handler.
    """
    bgp_speaker_process = controller_testlib.utils.wait_until_function_pass(
        retry_count=retries,
        interval=1,
...
def stop_bgp_speaker(process: subprocess.Popen, gracefully=True):
    ...
    controller_testlib.infra.stop_process_by_pid(process.pid, gracefully=gracefully)
    # dump BGP speaker logs
    rc, stdout = controller_testlib.infra.shell("cat tmp/play.py.out")
    log.debug(f"Bgp speaker output: {stdout=}")


def kill_all_bgp_speakers():
    """Stop BGP speaker process by sending SIGKILL signal.
    ...
    controller_testlib.infra.shell(
        "ps axf | grep exabgp | grep -v grep | awk '{print \"kill -9 \" $1}' | sh"
    )
...
def start_exabgp(cfg_file: str, log_file: str | None = None) -> subprocess.Popen:
    ...
    if log_file:
        command += f" > tmp/{log_file}"

    process = controller_testlib.infra.shell(command, run_in_background=True)

    return process


def verify_exabgp_connection(
    exabgp_ips: str | List[str], expect_connected: bool = True
):
...
def stop_exabgp(process: subprocess.Popen, gracefully: bool = True):
    ...
    controller_testlib.infra.stop_process_by_pid(
        process.pid, gracefully=gracefully, timeout=150
    )
...
def start_gobgp(
    gobgp_path: str,
    cfg_file: str,
    log_file: str = "gobgp.log",
    grpc_address: str = ":50051",
) -> subprocess.Popen:
    """Start Gobgp peer simulator.

    Args:
        gobgp_path: Gobgp executable file path.
        cfg_file: Gobpg configuration file path.
...
def verify_gobgp_connection(gobgp_ip: str, expect_connected: bool = True):
    ...
    """
    expected_code = (
        templated_requests.ALLOWED_STATUS_CODE
        if expect_connected
...
def start_gobgp_and_verify_connected(
    gobgp_path: str,
    cfg_file: str,
    exabgp_ips: str,
    log_file: str = "gobgp.log",
    ...
    Args:
        gobgp_path: Gobgp executable file path.
        cfg_file: Gobpg configuration file path.
        gobgp_ip (str): Gobgp ip address.
        gobgp_ips (str | List[str]): Single or list of gobgp ip addresses
            if manypeer scenario.

    Returns:
        subprocess.Popen: Exabgp process handler.
    """
    gobgp_process = start_gobgp(gobgp_path, cfg_file, log_file, grpc_address)
    controller_testlib.utils.wait_until_function_pass(
        5, 5, verify_exabgp_connection, exabgp_ips=exabgp_ips, expect_connected=True
    )
...
def stop_gobgp(process: subprocess.Popen):
    ...
    Args:
        process (subprocess.Popen): Gobgp process handler.
    ...
    controller_testlib.infra.stop_process_by_pid(process.pid, gracefully=True)
    ...
    try:
        ...
        for peer_id in range(peers_count):
            update_messag = controller_testlib.utils.wait_until_function_pass(
                3, 2, get_update_message, peer_id
            )
...
def play_to_odl_template(to_test: str, dir: str, ipv: str = "ipv4", peer_id=0):
    ...
    Args:
        to_test (str): Scenario to test.
        dir (str): Directory containing hex values.
        ipv (str) Ip version ipv4 or ipv6.
...
def remove_configured_routes(to_test: str, dir: str) -> requests.Response:
    ...
    mapping = {"IP": ODL_IP, "BGP_RIB": "example-bgp-rib"}
    response = templated_requests.delete_templated_request(
        f"{dir}/{to_test}/app", mapping
    )

    return response


def check_example_ipv4_topology_content(string_to_check: str = ""):
    """Check the example-ipv4-topology content for string

    Args:
        string_to_check (str): String expected to be found in topology.
    ...
    assert (
        string_to_check in response.text
    ), f"Example ipv4 topology does not contain {string_to_check}"


def count_key_value_pairs(file_name: str, keyword: str, value: str = ""):
    """Check file for key value pairs and returns number of occurences

    It can be searched by either 'keyword' only or by both
    'keyword' and 'value'. It is not mandatory to have equal sign (=) between
    these value.

    Args:
        file_name (str): Text file name to be checked.
        keyword (str): Key part of the expected key-value pair.
        value (str): Value part of the expected key-value pair.

    Returns:
        int: number of occurences of the key value pairs.
    """
    rc, output_log = controller_testlib.infra.shell(
        f"grep '{keyword}' {file_name} | grep -c {value}"
    )
    return int(output_log)


def verify_two_hex_messages_are_equal(hex1: str, hex2: str):
    """Verifies two hex messages are equal even in case, their arguments are
    misplaced. Compares length of the hex messages and sums hex messages
    arguments as integers and compares results.

    Args:
        hex1 (str): First hex value for comparison.
        hex2 (str): Second hex value for comparison.
