...
class DeferredLogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord):
        """Store an emitted log record in the local buffer.
    ...
    def flush_to_target(self, target_logger: logging.Logger):
        ...
        Args:
            target_logger (logging.Logger): The logger to which the buffered
...
def deferred_logging() -> Generator['DeferredLogHandler', None, None]:
    ...
    generated within this context are stored in memory and can be flushed
    to the original handlers later or discarded. Mainly usefull for functions
    which produces a lot of logs and it is now known in advanced if those log
    entries should be kept or discarded (e.g. wait until functions).

    Args:
        None

    Yields:
        DeferredLogHandler: The temporary handler holding the buffered logs.
    """
...
def truncate_long_text(text: str, max_size: int) -> str:
    ...
    Args:
        text (str): The text to be truncate.
        max_size (int): The maximum allowed length. If -1, text is not truncated.

    Returns:
        str: Finall truncated text.
    """
    if max_size == -1:
        return text
