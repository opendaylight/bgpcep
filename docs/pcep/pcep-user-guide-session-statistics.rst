...
PCEP stats data are periodically synced with the current state of PCEP topology (network-topology-pcep:topology-pcep)
content. Interval of such update can by configured using the following edit (PUT) call:

**URL:** ``rests/data/network-topology:network-topology/topology=pcep-topology/topology-types/network-topology-pcep:topology-pcep/odl-pcep-stats-provider:timer``
   ...
   .. tab:: XML
      ...
      **Request Body:**
      ...
      .. code-block:: xml
          ...
          <!-- Inserting leaf values not supported using XML, use JSON instead -->
...
Previous example would result in an update interval of 15 seconds in pcep-topology. Default interval is 5 seconds,
...
User has to take into account that setting the timer to any value is guarantee that stats are updated exactly
after timer time pass by:

Duration of update can take some time, it depends on systme performance and datastore ability to process write
operations.

* if the duration of update take less than ``timer`` - task is rescheduled again after ``timer`` value decreased by
elapsed time, this ensures that all consequesnt updates are run in the same intervals
* if the duration of update take take longer than ``timer`` value - in this case new update is scheduled after original
``timer`` value
