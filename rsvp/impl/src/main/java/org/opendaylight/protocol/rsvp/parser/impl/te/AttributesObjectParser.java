...
public final class AttributesObjectParser extends AbstractRSVPObjectParser {
    ...
    final protected static List<FlagContainer> parseFlag(final ByteBuf byteBuf) {
    ...
    final protected static void serializeFlag(final List<FlagContainer> flagList, final ByteBuf bufferAux) {
        for (FlagContainer flagContainer : flagList) {
            bufferAux.writeBytes(flagContainer.getFlags());
        }
    }

    final protected static void serializeTLV(final int tlvType, final int lenght, final ByteBuf value, final ByteBuf auxBuffer) {
