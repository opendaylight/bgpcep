...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev241219.sr.attributes.LinkMsdBuilder;
...
public final class SrLinkAttributesParser {
    ...
    public static ByteBuf serializeAdjacencySegmentIdentifier(final SrAdjIds srAdjId) {
        ...
        serializeAdjFlags(srAdjId.getFlags(), srAdjId.getSidLabelIndex()).toByteBuf(output);
    ...
    public static ByteBuf serializeLanAdjacencySegmentIdentifier(final SrLanAdjIds srLanAdjId) {
        final var output = Unpooled.buffer();
        serializeAdjFlags(srLanAdjId.getFlags(), srLanAdjId.getSidLabelIndex()).toByteBuf(output);
        writeUint8(output, srLanAdjId.getWeight().getValue());
        output.writeZero(RESERVED);

        switch (srLanAdjId.getNeighborType()) {
            case IsisNeighborCase isis -> output.writeBytes(isis.getIsoSystemId().getValue());
            case OspfNeighborCase ospf -> output.writeBytes(Ipv4Util.bytesForAddress(ospf.getNeighborId()));
            case null, default -> {
                // no-op
            }
        }
    ...
    public static ByteBuf serializeEpeSegmentIdentifier(final EpeSidTlv epeSid) {
        ...
        serializeAdjFlags(epeSid.getFlags(), epeSid.getSidLabelIndex()).toByteBuf(output);
    ...
    private static BitArray serializeAdjFlags(final Flags flags, final SidLabelIndex sidLabelIndex) {
        final var bitFlags = new BitArray(FLAGS_BITS_SIZE);
        switch (flags) {
            case OspfAdjFlagsCase ospfAdj -> {
