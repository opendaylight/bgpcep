...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev200120.linkstate.object.type.Srv6SidCaseBuilder;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev200120.linkstate.object.type.srv6.sid._case.Srv6AttributesBuilder;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev200120.linkstate.object.type.srv6.sid._case.Srv6NodeDescriptorsBuilder;
...
public final class Srv6SidNlriParser extends AbstractNlriTypeCodec {
    ...
    private static Srv6Attributes parseSrv6Attributes(final ByteBuf buffer) {
        final var tlvs = SimpleNlriTypeRegistry.getInstance().parseSubTlvs(buffer);
        return new Srv6AttributesBuilder()
            .setSrv6EndpointBehavior(
                (Srv6EndpointBehavior) tlvs.get(SRv6EndpointTlvParser.SRV6_ENDPOINT_BEHAVIOR_QNAME))
            .setSrv6BgpPeerNode((Srv6BgpPeerNode) tlvs.get(SRv6PeerNodeTlvParser.SRV6_BGP_PEER_NODE_QNAME))
            .setSrv6SidStructure((Srv6SidStructure) tlvs.get(SRv6SidStructureTlvParser.SRV6_SID_STRUCTURE_QNAME))
            .build();
    }

    private static void serializeSrv6Attributes(final Srv6Attributes tlv, final ByteBuf buffer) {
        final var reg = SimpleNlriTypeRegistry.getInstance();
