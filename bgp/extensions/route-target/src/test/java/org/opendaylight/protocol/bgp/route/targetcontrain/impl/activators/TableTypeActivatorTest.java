...
public final class TableTypeActivatorTest {
    ...
    public void testActivator() {
        try (var registry = new DefaultBGPTableTypeRegistryProvider(new TableTypeActivator())) {
            assertEquals(Optional.of(ROUTETARGETCONSTRAIN.class), registry.getAfiSafiType(RT_TP));
            assertEquals(Optional.of(RT_TP), registry.getTableType(ROUTETARGETCONSTRAIN.class));
        }
    }
}