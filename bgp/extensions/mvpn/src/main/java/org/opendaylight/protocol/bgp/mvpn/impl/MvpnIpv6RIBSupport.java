...
final class MvpnIpv6RIBSupport extends AbstractMvpnRIBSupport<MvpnRoutesIpv6Case, MvpnRoutesIpv6> {
    ...
    private List<MvpnDestination> extractRoutes(final Collection<MapEntryNode> routes) {
        return routes.stream().map(this::extractDestination).collect(Collectors.toList());
    }

    private MvpnDestination extractDestination(final DataContainerNode mvpnDest) {
    ...
    protected DestinationType buildDestination(final Collection<MapEntryNode> routes) {
        return new DestinationMvpnIpv6AdvertizedCaseBuilder()
            .setDestinationMvpn(new DestinationMvpnBuilder().setMvpnDestination(extractRoutes(routes)).build())
            .build();
    }
    ...
    public NodeIdentifierWithPredicates createRouteKey(final UnkeyedListEntryNode mvpn) {
        ...
        Ipv6NlriHandler.serializeNlri(List.of(extractDestination(mvpn)), buffer);
