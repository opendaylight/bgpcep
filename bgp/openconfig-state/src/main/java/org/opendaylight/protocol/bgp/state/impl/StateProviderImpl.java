...
public final class StateProviderImpl implements TransactionChainListener, AutoCloseable {
    ...
    private Map<String, InstanceIdentifier<Bgp>> instanceIdentifiersCache = new HashMap<>();
    ...
    public StateProviderImpl(@Nonnull final DataBroker dataBroker, final int timeout,
        ...
        @Nonnull final BGPStateCollector stateCollector, @Nonnull final String networkInstanceName) {
        ...
        this.scheduleTask = GlobalEventExecutor.INSTANCE.scheduleAtFixedRate(task, 0, timeout, TimeUnit.SECONDS);
    }

    private synchronized void updateBGPStats(final WriteTransaction wTx) {
        final Set<String> oldStats = this.instanceIdentifiersCache.keySet();
        this.stateCollector.getRibStats()
            .forEach(bgpStateConsumer -> {
                final String ribId = bgpStateConsumer.getInstanceIdentifier().getKey().getId().getValue();
                storeOperationalState(bgpStateConsumer, ribId, wTx);
                oldStats.remove(ribId);
            });
    ...
    private synchronized void storeOperationalState(final BGPRIBState bgpStateConsumer,
        final String ribId, final WriteTransaction wTx) {
        final Global global = GlobalUtil.buildGlobal(bgpStateConsumer, this.bgpTableTypeRegistry);
        final Neighbors neighbors = NeighborStateUtil.buildNeighbors(bgpStateConsumer, this.bgpTableTypeRegistry);
        final PeerGroups peerGroups = PeerGroupUtil.buildPeerGroups(bgpStateConsumer);
