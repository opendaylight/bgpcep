...
final class BGPRIBPolicyImpl implements BGPRIBRoutingPolicy {
    ...
    @Nullable
    @Override
    public ContainerNode applyImportPolicies(final PathArgument key, final PeerId fromPeerId,
        final PeerRole fromPeerRole, final ContainerNode attributes) {
        ContainerNode finalAttributes = attributes;
        ...
        for (final String policyName : this.importPolicy) {
            ...
            for (final Statement statement : statements) {
                finalAttributes = this.policyRegistry.applyImportStatement(this.openconfigPolicy, this.localAs,
                    this.originatorId, this.clusterId, key, fromPeerId, fromPeerRole, attributes, statement,
                    conditionSatisfied);
            }
        }
        if (!conditionSatisfied) {
            if (DefaultPolicyType.REJECTROUTE.equals(this.importPolicy)) {
                return null;
            }
        }
    ...
    @Override
    public ContainerNode applyExportPolicies(final PathArgument key, final PeerId fromPeerId,
        ...
        for (final String policyName : this.exportPolicy) {
            ...
            for (final Statement statement : statements) {
                finalAttributes = this.policyRegistry.applyExportStatement(this.openconfigPolicy, this.localAs,
                    ...
                    attributes, statement, conditionSatisfied);
            }
        }
        if (!conditionSatisfied) {
            if (DefaultPolicyType.REJECTROUTE.equals(this.importPolicy)) {
