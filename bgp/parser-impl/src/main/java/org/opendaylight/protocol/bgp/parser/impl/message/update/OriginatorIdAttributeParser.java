...
public final class OriginatorIdAttributeParser implements AttributeParser, AttributeSerializer {
	...
	public void parseAttribute(final ByteBuf buffer, final PathAttributesBuilder builder) {
		...
		builder.setOriginatorId(new OriginatorIdBuilder().setBgpIdentifier(Ipv4Util.addressForBytes(ByteArray.readBytes(buffer, ORIGINATOR_LENGTH))).build());
	}

    @Override
    public void serializeAttribute(DataObject attribute, ByteBuf byteAggregator) {
        OriginatorId originatorId = (OriginatorId) attribute;
        byteAggregator.writeBytes(Ipv4Util.bytesForAddress(originatorId.getBgpIdentifier()));
    }
}