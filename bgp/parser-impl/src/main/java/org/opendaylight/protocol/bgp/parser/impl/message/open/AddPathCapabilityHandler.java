...
public class AddPathCapabilityHandler implements CapabilityParser, CapabilitySerializer {
    ...
    enum SendReceive {
        RECEIVE((short)1), SEND((short)2), BOTH((short)3);
        private short size;
        private static final Map<Short, SendReceive> VALUE_MAP;
        static {
            final ImmutableMap.Builder<java.lang.Short, SendReceive> b = ImmutableMap.builder();
            for (final SendReceive enumItem : SendReceive.values()){
                b.put(enumItem.size, enumItem);
            }
            VALUE_MAP = b.build();
        }
        SendReceive(final short size) {
            this.size = size;
        }
        static SendReceive forValue(final short value) {
    ...
    public void serializeCapability(final CParameters capability, final ByteBuf byteAggregator) {
        ...
        final ByteBuf capBuffer = Unpooled.buffer(LENGTH);
        final Class<? extends AddressFamily> afi = addPathCap.getAfi();
        final Integer afival = this.afiReg.numberForClass(afi);
        Preconditions.checkArgument(afival != null, "Unhandled address family " + afi);
        capBuffer.writeShort(afival);

        final Class<? extends SubsequentAddressFamily> safi = addPathCap.getSafi();
        final Integer safival = this.safiReg.numberForClass(safi);
        Preconditions.checkArgument(safival != null, "Unhandled subsequent address family " + safi);
        capBuffer.writeByte(safival);

        final short sendReceive = addPathCap.getSendReceive();
        Preconditions.checkArgument(SendReceive.forValue(sendReceive) != null, "Unhandled subsequent address family " + safi);
        capBuffer.writeByte(sendReceive);
    ...
    public CParameters parseCapability(final ByteBuf buffer) throws BGPDocumentedException, BGPParsingException {
        ...
        final short sendReceive = buffer.readUnsignedByte();
        if (SendReceive.forValue(sendReceive) == null) {
            throw new BGPParsingException("Send/Receive value '" + sendReceive + "' is not valid.");
        }
        return new CParametersBuilder().addAugmentation(CParameters1.class,new CParameters1Builder().setAddPathCapability(
            new AddPathCapabilityBuilder().setAfi(afi).setSafi(safi).setSendReceive(sendReceive).build()).build()).build();
    }
