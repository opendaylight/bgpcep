...
public final class OriginAttributeParser extends AbstractAttributeParser implements AttributeSerializer {
    ...
    public void parseAttribute(final ByteBuf buffer, final AttributesBuilder builder,
        ...
        if (readable != 1) {
            final BGPTreatAsWithdrawException ex = new BGPTreatAsWithdrawException(BGPError.ATTR_LENGTH_ERROR,
                "ORIGIN attribute is expected to have size 1, but has %s", readable);
            if (errorHandling != RevisedErrorHandling.NONE) {
                throw ex;
            }
            throw ex.toDocumentedException();
        }

        final byte rawOrigin = buffer.readByte();
        final BgpOrigin borigin = BgpOrigin.forValue(UnsignedBytes.toInt(rawOrigin));
        if (borigin == null) {
            final BGPTreatAsWithdrawException ex = new BGPTreatAsWithdrawException(BGPError.ORIGIN_ATTR_NOT_VALID,
                "Unknown ORIGIN type %s", rawOrigin);
            if (errorHandling != RevisedErrorHandling.NONE) {
                throw ex;
            }
            throw ex.toDocumentedException();
