...
public final class BGPRouteRefreshMessageParser implements MessageParser, MessageSerializer {
    ...
    private final MultiProtocolCapabilityHandler helperHandler;

    public BGPRouteRefreshMessageParser(final AddressFamilyRegistry afiReg, final SubsequentAddressFamilyRegistry safiReg) {
        this.helperHandler = new MultiProtocolCapabilityHandler(afiReg, safiReg);
    }
    ...
    public void serializeMessage(final Notification message, final ByteBuf bytes) {
        ...
        this.helperHandler.serializeMPAfiSafi(msg.getAfi(), msg.getSafi(), msgBuf);
    ...
    public RouteRefresh parseMessageBody(final ByteBuf body, final int messageLength) throws BGPDocumentedException {
        ...
        try {
            final CParameters mpcap = this.helperHandler.parseCapability(body);
            return new RouteRefreshBuilder().setAfi(mpcap.getAugmentation(CParameters1.class).getMultiprotocolCapability().getAfi())
                .setSafi(mpcap.getAugmentation(CParameters1.class).getMultiprotocolCapability().getSafi()).build();
