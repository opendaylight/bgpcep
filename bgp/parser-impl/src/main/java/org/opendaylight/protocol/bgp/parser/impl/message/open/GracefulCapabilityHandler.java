...
public final class GracefulCapabilityHandler implements CapabilityParser, CapabilitySerializer {
    ...
    public void serializeCapability(final CParameters capability, ByteBuf byteAggregator) {
        ...
        bytes.writeByte(UnsignedBytes.checkedCast(flagBits + timeval / 256));
        bytes.writeByte(UnsignedBytes.checkedCast(timeval % 256));
        ...
        for (final Tables t : tables) {
            ...
            bytes.writeByte(UnsignedBytes.checkedCast(afival / 256));
            bytes.writeByte(UnsignedBytes.checkedCast(afival % 256));
            bytes.writeByte(UnsignedBytes.checkedCast(safival));
            if (t.getAfiFlags().isForwardingState()) {
                bytes.writeByte(UnsignedBytes.checkedCast(AFI_FLAG_FORWARDING_STATE));
            }
        }
        byteAggregator.writeBytes(CapabilityUtil.formatCapability(CODE, bytes));
    }
