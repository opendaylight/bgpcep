        ...
        if (unreachNlri != null) {
            ...
            if (!reachKey.equals(unreachKey)) {
                throw new IllegalStateException(String.format(
                        "MP_REACH afi-safi %s has different type as MP_UNREACH afi-safi %s", reachKey, unreachKey));
            }
        }
        final MpUnreachNlri resultingUnreach = this.nlriReg.convertMpReachToMpUnReach(mpReachNlri, unreachNlri);
        builder.addAugmentation(Attributes2.class, new Attributes2Builder()
            .setMpUnreachNlri(resultingUnreach).build());
