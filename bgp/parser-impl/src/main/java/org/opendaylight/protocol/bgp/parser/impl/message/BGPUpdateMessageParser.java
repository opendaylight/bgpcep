...
public final class BGPUpdateMessageParser implements MessageParser, MessageSerializer {
    ...
    public Update parseMessageBody(final ByteBuf buffer, final int messageLength,
        ...
        if (withdrawnRoutesLength > 0) {
            ...
            while (withdrawnRoutesBuffer.isReadable()) {
                ...
                withdrawnRoutesBuilder.setPrefix(readPrefix(withdrawnRoutesBuffer, revised));
        ...
        // FIXME: BGPCEP-359: take revised error handling into account
        ...
        while (buffer.isReadable()) {
            ...
            nlriBuilder.setPrefix(readPrefix(buffer, revised));
        ...
        if (withdrawCause.isPresent()) {
            LOG.debug("Converting BGP Update message to withdraw", msg);
            // FIXME: BGPCEP-359: check if we can treat the message as withdraw, otherwise promote to session reset
    ...
    private static Ipv4Prefix readPrefix(final ByteBuf buf, final RevisedErrorHandling revised)
            throws BGPDocumentedException {
        final int prefixLength = UnsignedBytes.toInt(buf.readByte());
        if (revised != RevisedErrorHandling.NONE) {
            ...
            if (prefixLength > buf.readableBytes()) {
