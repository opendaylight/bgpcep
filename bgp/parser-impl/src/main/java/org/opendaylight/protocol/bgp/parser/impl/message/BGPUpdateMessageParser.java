 ...
 */
public final class BGPUpdateMessageParser implements MessageParser, MessageSerializer, MultiPathMessageParser {
    ...
    public Update parseMessageBody(final ByteBuf buffer, final int messageLength) throws BGPDocumentedException {
        return parseMultiPathMessageBody(buffer, messageLength, null);
    }
    ...
    @Override
    public Update parseMultiPathMessageBody(final ByteBuf buffer, final int messageLength, final MultiPathSupport multiPathSupport)
        ...
        if (totalPathAttrLength > 0) {
            try {
                ...
                if (multiPathSupport != null) {
                    attributes = this.reg.parseAttributes(buffer.readSlice(totalPathAttrLength));
                } else {
                    attributes = this.reg.parseMultiPathAttributes(buffer.readSlice(totalPathAttrLength), multiPathSupport);
                }
