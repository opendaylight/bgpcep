...
public final class Ipv4MultiPathNlriParser implements NlriParser {

    private static DestinationIpv4 prefixes(final ByteBuf nlri) {
        final List<Ipv4Prefixes> prefixes = new ArrayList<>();
        while (nlri.isReadable()) {
            final long pathId = nlri.readUnsignedInt();
            final Ipv4Prefix ipv4Prefix = Ipv4Util.prefixForByteBuf(nlri);
            prefixes.add(new Ipv4PrefixesBuilder().setPathId(new PathId(pathId)).setPrefix(ipv4Prefix).build());
        }
    ...
    public void parseNlri(final ByteBuf nlri, final MpReachNlriBuilder builder) throws BGPParsingException {
        builder.setAdvertizedRoutes(new AdvertizedRoutesBuilder().setDestinationType(
                new DestinationIpv4CaseBuilder().setDestinationIpv4(prefixes(nlri)).build()).build());
    }
