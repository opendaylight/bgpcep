...
public class MultiProtocolCapabilityHandlerTest {
    ...
    public void testCapabilityHandler() throws BGPDocumentedException, BGPParsingException {
        final CParameters capabilityToSerialize = new MultiprotocolCaseBuilder().setMultiprotocolCapability(new MultiprotocolCapabilityBuilder().setAfi(this.afi).setSafi(this.safi).build()).build();
    ...
    public void noSerializationTest() {
        final CParameters noMPInstance = new GracefulRestartCaseBuilder().setGracefulRestartCapability(new GracefulRestartCapabilityBuilder().build()).build();

        final ByteBuf bytes = Unpooled.buffer();
        final MultiProtocolCapabilityHandler handler = new MultiProtocolCapabilityHandler(this.afir, this.safir);

        handler.serializeCapability(noMPInstance, bytes);
