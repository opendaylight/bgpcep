 ...
 */
abstract class AbstractL3vpnMcastIpRIBSupport<C extends Routes & DataObject, S extends DataObject>
    ...
    protected final List<L3vpnMcastDestination> extractRoutes(final Collection<MapEntryNode> routes) {
        return routes.stream().map(this::extractDestinations).collect(Collectors.toList());
    }

    protected final L3vpnMcastDestination extractDestinations(
