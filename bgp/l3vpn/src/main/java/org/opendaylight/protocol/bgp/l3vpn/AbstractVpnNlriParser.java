...
public abstract class AbstractVpnNlriParser implements NlriParser, NlriSerializer {
    ...
    protected static <T extends L3vpnDestination> List<VpnDestination> getVpnDestination(DestinationType dstType, Class<T> clazz) {
        if (clazz.isInstance(dstType)) {
            return clazz.cast(dstType).getVpnDestination();
        } else return null;
    }
...
//            Preconditions.checkNotNull(prefix.getIpv6Prefix(), "Ipv6 prefix is missing.");
    ...
    private static List<VpnDestination> parseNlri(final ByteBuf nlri, final Class<? extends AddressFamily> afi) {
        ...
        while (nlri.isReadable()) {
            ...
            Preconditions.checkState(prefixLen > 0, "VPN IPv6 is required.");//TODO
