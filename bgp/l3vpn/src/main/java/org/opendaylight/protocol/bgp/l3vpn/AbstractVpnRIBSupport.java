 ...
 * @author Kevin Wang
 */
public abstract class AbstractVpnRIBSupport<VPN_DST extends L3vpn> extends AbstractRIBSupport {
    ...
    protected final NodeIdentifier PREFIX_TYPE_NID;
    protected final NodeIdentifier LABEL_STACK_NID;
    protected final NodeIdentifier LV_NID;
    ...
    protected AbstractVpnRIBSupport(
        ...
        QName dstVpnContainerClassQName,
        QName dstVpnClassQName
    ) {
        super(cazeClass, containerClass, listClass);
        CONTAINER_CLASS_QNAME = BindingReflections.findQName(containerClass).intern();
        LIST_CLASS_QNAME = BindingReflections.findQName(listClass).intern();
        ADDRESS_FAMILY_CLAZZ = addressFamilyClass;

        NLRI_ROUTES_LIST = NodeIdentifier.create(dstVpnClassQName);
        PREFIX_TYPE_NID = NodeIdentifier.create(QName.create(dstVpnClassQName, "prefix").intern());
        LABEL_STACK_NID = NodeIdentifier.create(QName.create(dstVpnClassQName, "label-stack").intern());
        LV_NID = NodeIdentifier.create(QName.create(dstVpnClassQName, "label-value").intern());
        RD_NID = NodeIdentifier.create(QName.create(dstVpnClassQName, "route-distinguisher").intern());
        DESTINATION = NodeIdentifier.create(dstVpnContainerClassQName);
        ...
        EMPTY_ROUTES = Builders.choiceBuilder()
            ...
            .addChild(
                Builders.containerBuilder()
                    ...
                    .addChild(
                        ImmutableNodes.mapNodeBuilder(LIST_CLASS_QNAME).build()
                    ).build()
            ).build();
    }

    protected abstract VPN_DST extractVpnDestination(final DataContainerNode<? extends YangInstanceIdentifier.PathArgument> route);

    protected RouteDistinguisher extractRouteDistinguisher(final DataContainerNode<? extends YangInstanceIdentifier.PathArgument> route) {
    ...
    protected abstract DestinationType getAdvertizedDestinationType(List<VPN_DST> dests);

    protected abstract DestinationType getWithdrawnDestinationType(List<VPN_DST> dests);
    ...
    protected MpReachNlri buildReach(Collection<MapEntryNode> routes, CNextHop hop) {
        ...
        final List<VPN_DST> dests = new ArrayList<>(routes.size());
    ...
    private NodeIdentifierWithPredicates createRouteKey(final UnkeyedListEntryNode l3vpn) {
        ...
        final VPN_DST dest = extractVpnDestination(l3vpn);
