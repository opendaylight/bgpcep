...
public final class NextHopUtil {
    ...
    public static CNextHop parseNextHop(final ByteBuf buffer) {
        ...
        case Ipv4Util.IP4_LENGTH:
            addr = new Ipv4NextHopCaseBuilder().setIpv4NextHop(new Ipv4NextHopBuilder().setGlobal(Ipv4Util.addressForByteBuf(buffer)).build()).build();
            break;
        case Ipv6Util.IPV6_LENGTH:
            addr = new Ipv6NextHopCaseBuilder().setIpv6NextHop(new Ipv6NextHopBuilder().setGlobal(Ipv6Util.addressForByteBuf(buffer)).build()).build();
            break;
