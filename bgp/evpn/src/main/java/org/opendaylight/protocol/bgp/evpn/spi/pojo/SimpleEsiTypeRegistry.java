...
public final class SimpleEsiTypeRegistry implements EsiRegistry {
    ...
    private final Map<Class<? extends Esi>, EsiSerializer> modelSerializer = new HashMap<>();
    ...
    public AutoCloseable registerEsiSerializer(final Class<? extends Esi> esiType, final EsiSerializer serializer) {
        this.modelSerializer.put(esiType, serializer);
    ...
    @Override
    public Esi serializeEsiModel(final Class<? extends Esi> esiType, final ContainerNode esiChoice) {
        return this.modelSerializer.get(esiType).serializeEsi(esiChoice);
    }
}
