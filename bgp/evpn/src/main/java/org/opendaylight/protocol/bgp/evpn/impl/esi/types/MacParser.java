...
final class MacParser extends AbstractEsiType {
    public static final NodeIdentifier MAC_AUTO_CASE_NID = new NodeIdentifier(MacAutoGeneratedCase.QNAME);
    static final NodeIdentifier MAC_AUTO_NID = new NodeIdentifier(MacAutoGenerated.QNAME);
    static final NodeIdentifier MAC_NID = NodeIdentifier.create(QName.create(Evpn.QNAME, "system-mac-address").intern());
    ...
    public void serializeEsi(final Esi esi, final ByteBuf buffer) {
        ...
        body.writeByte(EsiType.MacAutoGenerated.getIntValue());
    ...
    public Esi serializeEsi(final ContainerNode esi) {
        ...
        builder.setSystemMacAddress(extractMac(esi));
    ...
    private Uint24 extractUint24LD(final ContainerNode esiVal) {
        if (esiVal.getChild(LD_NID).isPresent()) {
            return (Uint24) esiVal.getChild(LD_NID).get().getValue();
        }
        return null;
    }

    private MacAddress extractMac(final ContainerNode macGEn) {
        if (macGEn.getChild(MAC_NID).isPresent()) {
            return (MacAddress) macGEn.getChild(MAC_NID).get().getValue();
        }
        return null;
    }
