...
public class SimpleEvpnNlriRegistry implements EvpnRegistry {
    ...
    public EvpnChoice serializeEvpnModel(final ChoiceNode evpnChoice) {
        ...
        for (final DataContainerChild<? extends PathArgument, ?> entry : evpnChoice.getValue()) {
            final EvpnSerializer serializer = this.modelHandlers.get((NodeIdentifier) entry.getIdentifier());
            if (serializer != null) {
                return serializer.serializeEvpnModel((ContainerNode) entry);
            }
        }
        return null;
    }
    ...
    public EvpnChoice serializeEvpnRouteKey(final ChoiceNode evpnChoice) {
        ...
        for (final DataContainerChild<? extends PathArgument, ?> entry : evpnChoice.getValue()) {
            final EvpnSerializer serializer = this.modelHandlers.get((NodeIdentifier) entry.getIdentifier());
            if (serializer != null) {
                return serializer.createRouteKey((ContainerNode) entry);
            }
        }
        return null;
    }
