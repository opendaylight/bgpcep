...
public class BGPCEP1080Test extends AbstractRIBTestSetup {
    ...
    public void releaseRibOutChainDoesNotBlockOnPendingSubmit() throws Exception {
        final var submitted = mock(FluentFuture.class);
        ...
        verify(submitted).addListener(listener.capture(), eq(MoreExecutors.directExecutor()));
    ...
    private void arm(final FluentFuture<CommitInfo> submitted) {
        doNothing().when(submitted).addCallback(any(), any());
        doNothing().when(submitted).addListener(any(), any());
