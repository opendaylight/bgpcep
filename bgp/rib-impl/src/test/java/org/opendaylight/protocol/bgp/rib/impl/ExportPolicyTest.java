...
public class ExportPolicyTest {
    ...
    private static final ToReflectorClientExportPolicy refPolicy = new ToReflectorClientExportPolicy(ipv4, cluster);
    private static final long localAs = 8;
    private static final ToExternalExportPolicy extPolicy = new ToExternalExportPolicy(localAs);
    private static final ToInternalExportPolicy intPolicy = new ToInternalExportPolicy(ipv4, cluster);
    ...
    private static UnkeyedListEntryNode createSequenceWithLocalAs() {
        return Builders.unkeyedListEntryBuilder().withNodeIdentifier(BestPathSelectorTest.SEGMENTS_NID)
            .addChild(Builders.orderedLeafSetBuilder().withNodeIdentifier(BestPathSelectorTest.SEQ_LEAFLIST_NID)
                .addChild(Builders.leafSetEntryBuilder().withNodeIdentifier(new NodeWithValue(BestPathSelectorTest.AS_NUMBER_Q, localAs)).withValue(localAs).build())
                .build()).build();
    }
}
