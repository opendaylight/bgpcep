...
public class AddPathBasePathsTest extends AbstractAddPathTest {
    ...
    */
    ...
    public void testUseCase1() throws Exception {
        ...
        final Channel session5 = createPeerSession(PEER5, PeerRole.RrClient, nonAddPathParams, ribImpl, hf, listener5);
        Thread.sleep(1000);
        ...
        assertEquals(UPD_NA_100, listener5.getListMsg().get(0));
        ...
        assertEquals(UPD_NA_200, listener5.getListMsg().get(1));

        /*
        FIXME
        Race condition is observed, on Loc Rib datachanges from effect rib comes before
        than the supported table changes, therefore peer is not added to exportgroup and route are not advertised
        ...
        assertEquals(UPD_NA_100, listener5.getListMsg().get(2));

        //Remove second best, no advertisement should be done
        sendWithdrawalRouteAndCheckIsOnLacRib(session2, PREFIX1, 50, 1);
        assertEquals(3, listener4.getListMsg().size());
        assertEquals(3, listener5.getListMsg().size());

        //Remove best, 1 advertisement
        sendWithdrawalRouteAndCheckIsOnLacRib(session1, PREFIX1, 100, 1);
        assertEquals(4, listener4.getListMsg().size());
        assertEquals(4, listener5.getListMsg().size());

        //Remove best, 1 withdrawal
        sendWithdrawalRouteAndCheckIsOnLacRib(session3, PREFIX1, 20, 0);
