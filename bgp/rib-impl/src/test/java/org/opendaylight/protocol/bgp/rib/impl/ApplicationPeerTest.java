...
public class ApplicationPeerTest {
    ...
    public void testOnDataChanged() {
        ...
        final MpReachNlri mp = new MpReachNlriBuilder().setAfi(Ipv4AddressFamily.class).setSafi(UnicastSubsequentAddressFamily.class).build();
        created.put(this.id, new Ipv4RouteBuilder().setAttributes(new AttributesBuilder().setMpReachNlri(mp).build()).build());
