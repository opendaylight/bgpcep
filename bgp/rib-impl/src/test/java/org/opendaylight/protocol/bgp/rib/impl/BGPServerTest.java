...
public class BGPServerTest {
    ...
    // TODO remove all implement normal server test
    @Ignore
    @Test
    public void testCreateServer() throws Exception {
        final BGPDispatcherImpl dispatcher = new BGPDispatcherImpl(ServiceLoaderBGPExtensionProviderContext.getSingletonInstance().getMessageRegistry(), new HashedWheelTimer(), new NioEventLoopGroup(), new NioEventLoopGroup());

        final InetSocketAddress adr = new InetSocketAddress(Inet4Address.getByName("0.0.0.0"), 8383);
        final BGPSessionPreferences prefs = new BGPSessionPreferences(new AsNumber(1L), 50, new Ipv4Address("0.0.0.0"), Lists.newArrayList(new BgpParametersBuilder().setCParameters(new As4BytesCaseBuilder().setAs4BytesCapability(new As4BytesCapabilityBuilder().setAsNumber(new AsNumber(1L)).build()).build()).build()));
        ...
        final ReconnectStrategyFactory tcpStrategyFactory = new ReconnectStrategyFactory() {
            @Override
        ...
        final BGPPeerRegistry registry = new BGPPeerRegistry() {
            ...
            @Override
            public boolean isPeerConfigured(IpAddress ip) {
                return true;
            }

            @Override
            public BGPSessionListener getPeer(final IpAddress ip, final Ipv4Address bgpIdentifier, final Ipv4Address bgpId) throws BGPDocumentedException {
                return new BGPPeer("testing client", rib);
            }
        ...
        dispatcher.createServer(new StrictBGPPeerRegistry(), adr, new BGPSessionValidator() {
            @Override
