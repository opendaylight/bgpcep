...
public class AbstractRIBTestSetup {
    ...
    private static final TablesKey KEY = new TablesKey(AFI, SAFI);
    ...
    public Collection<DataTreeCandidate> appPeerInput(final Ipv4Prefix... prefix) {
        final Collection<DataTreeCandidate> col = new HashSet<>();
        final YangInstanceIdentifier base = this.rib.getYangRibId().node(LocRib.QNAME).node(Tables.QNAME).node(RibSupportUtils.toYangTablesKey(new TablesKey(AFI, SAFI)));
        for (final Ipv4Prefix p : prefix) {
            ...
            col.add(new DefaultDataTreeCandidate(this.ribSupport.routePath(base.node(Routes.QNAME), routekey), DataTreeCandidateNodes.fromNormalizedNode(b.build())));
        }
        return col;
    }
