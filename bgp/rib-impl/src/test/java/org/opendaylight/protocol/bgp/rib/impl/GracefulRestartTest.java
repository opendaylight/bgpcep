...
public class GracefulRestartTest extends AbstractAddPathTest {
    ...
    public void setUp() throws Exception {
        ...
        final BgpParameters parameters = createParameter(true, true);
    ...
    public void resetConnectionOnOpenTest() {
        ...
        CheckUtil.checkState(this.peer, State.IDLE, peer -> {
            if (((BGPPeer) peer).getBGPSessionState() == null) {
                return State.IDLE;
            } else {
                return ((BGPPeer) peer).getBGPSessionState().getSessionState();
            }
        });
    ...
    public void retainRoutesOnPeerRestartTest() throws Exception {
        final ArrayList<Ipv4Prefixes> ipv4prefixes = new ArrayList<>();
        final ArrayList<Ipv6Prefixes> ipv6prefixes = new ArrayList<>();
        ...
        this.session.close();
        CheckUtil.checkState(this.peer, State.IDLE, peer -> {
            final BGPPeer bgpPeer = (BGPPeer) peer;
            if (bgpPeer.getBGPSessionState() != null) {
                return bgpPeer.getBGPSessionState().getSessionState();
            } else {
                return State.IDLE;
            }
        });
    ...
    public void removeRoutesOnMissingGracefulRestartTest() throws Exception {
        ...
        CheckUtil.checkState(listener, State.UP, listener -> ((SimpleSessionListener) listener).getState());
        checkLocRib(0, true);
        checkLocRib(0, false);
    }
    ...
    public void removeRoutesOnMissingGracefulRestartAfiSafiTest() throws Exception {
        ...
        CheckUtil.checkState(listener, State.UP, listener -> ((SimpleSessionListener) listener).getState());
        CheckUtil.checkState(this.peer, State.UP, peer -> {
            if (((BGPPeer) peer).getBGPSessionState() == null) {
                return State.IDLE;
            } else {
                return ((BGPPeer) peer).getBGPSessionState().getSessionState();
    ...
    public void removeStaleRoutesAfterRestartTest() throws Exception {
        retainRoutesOnPeerRestartTest();
        this.session = createPeerSession(PEER1, createParameter(true, true), this.listener);
        CheckUtil.checkState(listener, State.UP, listener -> ((SimpleSessionListener) listener).getState());
    ...
    private void checkLocRib(final int expectedRoutesOnDS, boolean ipv4)
            throws Exception {
        if (ipv4) {
    ...
    private BGPPeer configurePeer(final BGPTableTypeRegistryConsumer tableRegistry,
                                  final Ipv4Address peerAddress, final RIBImpl ribImpl,
