...
public class LocRibWriterNewPeerInitTest extends AbstractRIBTestSetup {
    ...
    public <C extends Routes & DataObject & ChoiceIn<Tables>, S extends ChildOf<C>>
        ...
        try (final LocRibWriter<C, S> locRibWriter = LocRibWriter.create(ribSupport, IPV4UNICAST.VALUE, chain, ribIId,
                new AsNumber(Uint32.valueOf(AS)), dataBroker, policies, peerTracker,
    ...
    private DataTreeCandidate effRibInEvent(final PeerId peerId, final MapEntryNode tableNode) {
        final YangInstanceIdentifier tablePath = ribIId.node(PEER_NID).node(IdentifierUtils.domPeerId(peerId))
            .node(EFFRIBIN_NID).node(TABLES_NID).node(tableNode.name());
    ...
    private MapEntryNode buildTableWithRoute() {
        ...
        final var route = new Ipv4RouteBuilder()
            ...
            .build();
        final Routes routes = new Ipv4RoutesCaseBuilder()
        ...
        final DataObjectIdentifier<Tables> tableId = DataObjectIdentifier.builder(BgpRib.class)
