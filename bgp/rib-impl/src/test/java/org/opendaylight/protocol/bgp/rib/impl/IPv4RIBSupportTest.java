...
public final class IPv4RIBSupportTest extends AbstractRIBSupportTest {
    ...
    public void testEmptyRoute() throws Exception {
        final Routes empty = new Ipv4RoutesCaseBuilder().setIpv4Routes(new Ipv4RoutesBuilder().setIpv4Route(Collections.emptyList()).build()).build();
    ...
    public void testBuildMpUnreachNlriUpdate() {
        final Update update = RIB_SUPPORT.buildUpdate(Collections.emptyList(), createRoutes(
            new Ipv4RoutesBuilder().setIpv4Route(Collections.singletonList(ROUTE)).build()), ATTRIBUTES);
    ...
    public void testBuildMpReachNlriUpdate() {
        final Update update = RIB_SUPPORT.buildUpdate(createRoutes(
            new Ipv4RoutesBuilder().setIpv4Route(Collections.singletonList(ROUTE)).build()), Collections.emptyList(), ATTRIBUTES);
    ...
    public void testIsComplexRoute() {
        Assert.assertEquals(false, RIB_SUPPORT.isComplexRoute());
    }
    ...
    public void testRouteIdAddPath() {
        final NodeIdentifierWithPredicates expected = createRouteNIWP(new Ipv4RoutesBuilder().setIpv4Route(Collections.singletonList(ROUTE)).build());
    ...
    public void testRoutePath() {
        final NodeIdentifierWithPredicates prefixNii = createRouteNIWP(new Ipv4RoutesBuilder().setIpv4Route(Collections.singletonList(ROUTE)).build());
    ...
    public void testExtractPathId() {
        final ContainerNode cont = ImmutableContainerNodeSchemaAwareBuilder.create().withNodeIdentifier(RIB_SUPPORT.routePathIdNid()).
            addChild(new ImmutableLeafNodeBuilder<>().withNodeIdentifier(RIB_SUPPORT.routePathIdNid()).withValue(PATH_ID.getValue()).build()).build();
        Assert.assertEquals(PATH_ID.getValue(), RIB_SUPPORT.extractPathId(cont));
    }
    ...
    public void testChangedRoutes() {
        ...
        final Routes routes = new Ipv4RoutesCaseBuilder().setIpv4Routes(new Ipv4RoutesBuilder().setIpv4Route(Collections.singletonList(ROUTE)).build()).build();
