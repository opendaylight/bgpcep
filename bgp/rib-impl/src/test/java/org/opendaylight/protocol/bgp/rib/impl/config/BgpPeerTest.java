...
public class BgpPeerTest extends AbstractConfig {
    ...
    private Neighbor createNeighborExpected(final IpAddress neighborAddress) {
    ...
    static Config createConfig() {
        return new org.opendaylight.yang.gen.v1.http.openconfig.net.yang.bgp.rev151009.bgp.neighbor.group.ConfigBuilder().setPeerAs(AS)
            .setPeerType(PeerType.INTERNAL).setAuthPassword("pass").setRouteFlapDamping(false).setSendCommunity(CommunityType.NONE).build();
    }

    private static AfiSafis createAfiSafi() {
        return new AfiSafisBuilder().setAfiSafi(Collections.singletonList(new AfiSafiBuilder().setAfiSafiName(IPV4UNICAST.class)
