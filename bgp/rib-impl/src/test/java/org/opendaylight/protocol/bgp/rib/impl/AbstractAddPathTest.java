...
class AbstractAddPathTest extends AbstractDataBrokerTest {
    ...
    BGPSessionImpl createPeerSession(final Ipv4Address peer, final PeerRole peerRole, final BgpParameters nonAddPathParams, final RIBImplImpl ribImpl,
        final BGPPeerSessionListener sessionListener) throws InterruptedException, ExecutionException {
    ...
    private static void configurePeer(final Ipv4Address localAddress, final RIBImplImpl ribImpl, final BgpParameters bgpParameters, final PeerRole peerRole) {
        final IpAddress ipAddress = new IpAddress(new Ipv4Address(InetAddresses.forString(localAddress.getValue())
            .getHostAddress()));


        final BGPPeer bgpPeer = new BGPPeer(ipAddress, ribImpl, peerRole, null, AFI_SAFIS_ADVERTIZED,
    ...
    private static BGPSessionImpl connectPeer(final Ipv4Address localAddress, final BgpParameters bgpParameters,
        final BGPDispatcherImpl dispatcherImpl, final BGPPeerSessionListener sessionListener) throws InterruptedException {
