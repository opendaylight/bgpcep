 ...
 *
 * <p>{@link BGPPeer#onSessionDown(org.opendaylight.protocol.bgp.rib.spi.BGPSession, Exception)} holds the peer's
 * monitor while releasing the RIB-out chain. If it waits there for the last submitted RIB-out transaction to complete
 * and the RIB-out transaction chain fails at that very moment, the thread delivering the failure cannot enter
 * {@code BGPPeer.onRibOutChainFailed()} -- which is the very thing which would release the chain without waiting --
 * because the monitor is held by the thread waiting for that transaction.
 *
 * <p>That wait can never be satisfied: mdsal reports chain failure from the very listener list of the commit future
 * which is being waited for, and the chain-failure listener is registered ahead of the one completing the commit.
 */
public class BGPPeerShutdownTest extends AbstractRIBTestSetup {
    ...
    public void testRibOutChainFailureDuringSessionDown() throws Exception {
        ...
        final var ribOutCallback = ribOutChainCallback();

        // Have the peer submit a RIB-out transaction which does not complete, so that it becomes the transaction
    ...
    /**
     * Returns the callback {@link BGPPeer#onSessionUp} has registered on the RIB-out chain. All chains are backed by
     * the same mock here and {@link AbstractPeer} registers the peer itself on the RIB-in ones, hence the filtering.
     */
    private FutureCallback<Empty> ribOutChainCallback() {
        final ArgumentCaptor<FutureCallback<Empty>> captor = ArgumentCaptor.captor();
        verify(peer.ribOutChain, atLeastOnce()).addCallback(captor.capture());
        return captor.getAllValues().stream()
            .filter(callback -> callback != peer)
            .findFirst()
            .orElseThrow(() -> new AssertionError("No RIB-out chain callback registered"));
    }
