 ...
 * on-wire NLRI identity changed while the route-key stayed stable, the superseded NLRI must be withdrawn before the
 * replacement is advertised; otherwise only the advertisement is sent.
 */
@ExtendWith(MockitoExtension.class)
class AdjRibOutListenerTest {
    ...
    private static final QName VALUE_QNAME = QName.create(ROUTE_QNAME, "value").intern();
     ...
     */
    @Test
    void testChangedIdentityWithdrawsBeforeAdvertising() {
     ...
     */
    @Test
    void testUnchangedIdentityAdvertisesWithoutWithdrawal() {
    ...
    private static MapEntryNode route(final Uint32 value) {
        return ImmutableNodes.newMapEntryBuilder()
            .withNodeIdentifier(NodeIdentifierWithPredicates.of(ROUTE_QNAME, ROUTE_KEY_QNAME, "test-route"))
            .withChild(ImmutableNodes.leafNode(VALUE_QNAME, value))
            .build();
    }
}
