...
public class AdjRibOutListenerTest {
    ...
    /**
     * When an in-place route modification changes the on-wire NLRI identity, the listener must write a withdrawal of
     * the superseded NLRI before advertising the replacement.
     */
    @Test
    public void changedIdentityWithdrawsBeforeAdvertising() {
        final AdjRibOutListener listener = stubInPlaceReplace();
    ...
    /**
     * When an in-place route modification keeps the on-wire NLRI identity, the listener must advertise the replacement
     * without emitting any withdrawal.
     */
    @Test
    public void unchangedIdentityAdvertisesWithoutWithdrawal() {
        final AdjRibOutListener listener = stubInPlaceReplace();
    ...
    // Wires an in-place modification of a single route (before -> after) and returns the created listener. Callers add
    // the requiresWithdrawalOnReplace outcome (and, when true, the withdraw Update) before firing onDataTreeChanged.
    private AdjRibOutListener stubInPlaceReplace() {
        ...
        final AdjRibOutListener listener = AdjRibOutListener.create(new PeerId("bgp://127.0.0.1"),
