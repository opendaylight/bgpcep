...
abstract sealed class AbstractPeer extends BGPPeerStateImpl
    ...
    public final synchronized <C extends Routes & DataObject & ChoiceIn<Tables>, S extends ChildOf<? super C>>
            FluentFuture<? extends CommitInfo> initializeRibOut(
    ...
    @Override
    public final synchronized <C extends Routes & DataObject & ChoiceIn<Tables>, S extends ChildOf<? super C>>
            FluentFuture<? extends CommitInfo> refreshRibOut(
                final RouteEntryDependenciesContainer entryDep,
                final List<StaleBestPathRoute> staleRoutes,
                final List<AdvertizedRoute<C, S>> newRoutes) {
        if (ribOutChain == null) {
            LOG.debug("Session closed, skip changes to peer AdjRibsOut {}", getPeerId());
            return CommitInfo.emptyFluentFuture();
        }
    ...
    @Override
    public final synchronized <C extends Routes & DataObject & ChoiceIn<Tables>, S extends ChildOf<? super C>>
            FluentFuture<? extends CommitInfo> reEvaluateAdvertizement(
                final RouteEntryDependenciesContainer entryDep,
                final List<ActualBestPathRoutes<C, S>> routesToStore) {
        if (ribOutChain == null) {
            LOG.debug("Session closed, skip changes to peer AdjRibsOut {}", getPeerId());
