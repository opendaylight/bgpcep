...
final class BestPathSelector {
    ...
    void processPath(final UnsignedInteger routerId, final PathAttributes attrs) {
        ...
        if (attrs != null) {
             ...
             * are better.
             */
            if (bestOriginatorId == null || selectPath(originatorId, attrs)) {
    ...
    BestPath result() {
        return bestRouterId == null ? null : new BestPath(bestRouterId, bestAttrs);
    }
    ...
    private boolean selectPath(final @Nonnull UnsignedInteger originatorId, final @Nonnull PathAttributes attrs) {
         ...
         */
        if (bestOriginatorId.compareTo(originatorId) > 0) {
            return true;
        }
