...
class FromInternalImportPolicy extends AbstractImportPolicy {
    ...
    ContainerNode effectiveAttributes(final ContainerNode attributes) {
        final AttributeUtils utils = AttributeUtils.getInstance(attributes.getNodeType().getModule());
         ...
         * we filter the route.
         */
        final Object originatorId = utils.getOriginatorId(attributes);
         ...
         */
        final LeafSetNode<?> clusterList = utils.getClusterList(attributes);
        if (clusterList != null) {
