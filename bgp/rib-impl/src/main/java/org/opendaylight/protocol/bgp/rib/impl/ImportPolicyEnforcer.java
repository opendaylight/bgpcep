...
final class ImportPolicyEnforcer {
    ...
    private final class RouteListener implements DOMDataTreeChangeListener {
        ...
        public void onDataTreeChanged(final Collection<DataTreeCandidate> changes) {
            ...
            for (DataTreeCandidate tc : changes) {
                ...
                // FIXME: Lookup RIBSupport
                final RIBSupport ribSupport = null;

                // Per-table attribute path
                final NodeIdentifier attrPath = ribSupport.routeAttributes();

                // Lookup per-table attributes from RIBSupport
                final ContainerNode adverisedAttrs = (ContainerNode) NormalizedNodes.findNode(tc.getRootNode().getDataAfter(), attrPath).orNull();
                ...
                updateRoutes(peerId, tableKey, tc.getRootNode(), attrPath, effectiveAttrs);
            }
        }
    }
    ...
    private void updateRoutes(final PeerId peerId, final NodeIdentifierWithPredicates tableKey, final DataTreeCandidateNode routes, final NodeIdentifier attrPath, final ContainerNode effectiveAttrs) {
        final DOMDataWriteTransaction tx = chain.newWriteOnlyTransaction();

        // FIXME: need new routeId
        final YangInstanceIdentifier routeId = null;

        if (effectiveAttrs != null) {
            tx.put(LogicalDatastoreType.OPERATIONAL, routeId, routes.getDataAfter().get());
            tx.put(LogicalDatastoreType.OPERATIONAL, routeId.node(attrPath), effectiveAttrs);
