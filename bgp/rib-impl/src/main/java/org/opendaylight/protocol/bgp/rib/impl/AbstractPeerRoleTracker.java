...
abstract class AbstractPeerRoleTracker implements AutoCloseable {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(PeerRoleListener.class);
    ...
    protected AbstractPeerRoleTracker(final @Nonnull DOMDataTreeChangeService service, @Nonnull final YangInstanceIdentifier ribId) {
        ...
        this.registration = service.registerDataTreeChangeListener(
            new DOMDataTreeIdentifier(LogicalDatastoreType.OPERATIONAL, ribId.node(Peer.QNAME).node(Peer.QNAME).node(PEER_ROLE)),
