...
class AdjRIBsTransactionImpl implements AdjRIBsTransaction {
    ...
    private final Collection<AdjRIBsOut> ribs;

    AdjRIBsTransactionImpl(final Collection<AdjRIBsOut> ribs, final BGPObjectComparator comparator, final WriteTransaction writeTransaction) {
    ...
    @Override
    public <K, V extends Route> void advertise(final RouteEncoder ribOut, final K key, final InstanceIdentifier<V> id, final V obj) {
        trans.put(LogicalDatastoreType.OPERATIONAL, id, obj, true);
        for (AdjRIBsOut r : ribs) {
            r.put(ribOut, key, obj);
        }
    }
