...
final class AdjRibOutListener implements ClusteredDOMDataTreeChangeListener, PrefixesSentCounters {
    ...
    private void processRouteChange(final DataTreeCandidateNode route) {
        ...
        switch (route.getModificationType()) {
            ...
            case WRITE:
                update = advertise((MapEntryNode) route.getDataBefore().orElse(null),
                    (MapEntryNode) route.getDataAfter().get());
    ...
    private Update advertise(final MapEntryNode before, final MapEntryNode after) {
        ...
        final Attributes attributes = routeAttributes(after);
        if (!this.llgrSupport && attributes.nonnullCommunities().contains(CommunityUtil.LLGR_STALE)) {
            return before == null
                    || routeAttributes(before).nonnullCommunities().contains(CommunityUtil.LLGR_STALE) ? null
                            : withdraw(before);
        }
        this.prefixesSentCounter.increment();
        if (!this.mpSupport) {
            return buildUpdate(Collections.singleton(after), Collections.emptyList(), attributes);
        }
        return this.support.buildUpdate(Collections.singleton(after), Collections.emptyList(), attributes);
    }
