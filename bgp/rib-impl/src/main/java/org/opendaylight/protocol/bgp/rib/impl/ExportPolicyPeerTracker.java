...
final class ExportPolicyPeerTracker extends AbstractPeerRoleTracker {
    ...
    void addToExportPolicies(final PeerId peerId, final YangInstanceIdentifier peerIId, final PeerRole peerRole,
            final Set<TablesKey> tables) {
        peerRoleChanged(peerIId, peerRole);
        onTablesChanged(peerId, tables);
    }

    void removeFromExportPolicies(final YangInstanceIdentifier peerIId) {
        peerRoleChanged(peerIId, null);
        onTablesChanged(IdentifierUtils.peerId(IdentifierUtils.peerKey(peerIId)), null);
    }

    private void onTablesChanged(final PeerId peerId, final Set<TablesKey> tables) {
        if (tables != null) {
