...
public final class BgpDeploymentManager implements AutoCloseable {
    ...
    private final Map<ProtocolKey, RIBImpl> ribs = new HashMap<>();
    private final Map<ProtocolKey, Map<NeighborKey, AutoCloseable>> peers = new HashMap<>();
    ...
    public BgpDeploymentManager(final RIBExtensionConsumerContext extensions, final BGPDispatcher dispatcher,
        ...
        LOG.info("BGP deployment manager started.");
    }

    @Override
    public void close() throws Exception {
        for (final Map<NeighborKey, AutoCloseable> map : this.peers.values()) {
            for (final AutoCloseable peer : map.values()) {
                peer.close();
            }
        }
        this.peers.clear();
        ...
        this.ribs.clear();
        LOG.info("BGP deployment manager stopped.");
    }
    ...
    public void onGlobalCreated(final ProtocolKey identifier, final Global global) {
        final String name = identifier.getName();
        LOG.debug("Creating new RIB instance {}.", name);
        final RIBImpl newRib = createRib(global, name);
        ...
        LOG.debug("New RIB instance created: {}.", global);
    }

    public void onGlobalModified(final ProtocolKey identifier, final Bgp bgp) {
    ...
    public void onGlobalRemoved(final ProtocolKey identifier) {
        final String name = identifier.getName();
        LOG.debug("Removing RIB instance {}.", name);
        final RIBImpl remove = this.ribs.remove(identifier);
        Preconditions.checkNotNull(remove);
        remove.close();
        LOG.debug("RIB instance removed: {}", name);
    }

    public void onNeighborCreated(final ProtocolKey identifier, final Neighbor neighbor) {
        LOG.debug("Creating new Peer instance {}.", neighbor.getKey());
    ...
    private void onPeerCreated(
        ...
        LOG.debug("New BGP Peer created: {}", neighbor);
    }
    ...
    private void onApplicationPeerCreated(final Neighbor neighbor, final Map<NeighborKey, AutoCloseable> neighbors,
        ...
        LOG.debug("New Application Peer created: {}", neighbor);
    }

    public void onNeighborModified(final ProtocolKey identifier, final Neighbor neighbor) {
        onNeighborRemoved(identifier, neighbor);
        onNeighborCreated(identifier, neighbor);
    }

    public void onNeighborRemoved(final ProtocolKey identifier, final Neighbor neighbor) {
        LOG.debug("Removing Peer instance {}.", neighbor.getKey());
        final Map<NeighborKey, AutoCloseable> neighbors = this.peers.get(identifier);
        Preconditions.checkNotNull(neighbors);
        ...
        try {
            peer.close();
            LOG.debug("Peer instance removed {}.", neighbor.getKey());
