...
final class RibTableEntry<T> {
    ...
    private synchronized Collection<ConsumingRouter> internalSelectRoute(final Collection<ConsumingRouter> potentialSinks, final T nlri, final AsNumber localAs) {
        ...
        if (newBestPath != null) {
             ...
             */
            attrs = newBestPath.getAttributes();
            if (bestPath != null && bestPath.getAttributes() == attrs) {
                return Collections.emptyList();
            }
        } else {
            attrs = null;
        ...
        final Collection<ConsumingRouter> ret = new ArrayList<>(potentialSinks.size());
        for (ConsumingRouter s : potentialSinks) {
            ...
            if (attrs != null && s.canConsumeRoute(attrs)) {
                newProd = attrs;
            } else {
                newProd = null;
            }
    ...
    /**
     * Perform route selection. The explicit assumption is that this method is always
     * called from a single thread.
     *
     * @param table Route table
     * @param nlri Route NLRI
     * @param localAs Local AS number
     * @param potentialSinks Collection of potential subscribers to the route
     */
    void selectRoute(final RibTable<T> table, final T nlri, final AsNumber localAs) {
        /*
         * This method is split into two: internalSelectRoute() runs synchronized and
         * returns a collection of sinks, which need to be notified. Notification runs
         * unsynchronized.
         */
        for (ConsumingRouter s : internalSelectRoute(table.getPotentialSinks(), nlri, localAs)) {
            s.routeUpdated(table, nlri, this);
        }
    }

    /**
     * Return the path which is considered to be best.
     *
     * @return Currently-selected best path
     */
    synchronized BestPath getBestPath() {
        return bestPath;
    }
}
