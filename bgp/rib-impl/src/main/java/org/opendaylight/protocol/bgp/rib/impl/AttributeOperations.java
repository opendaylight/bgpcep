...
final class AttributeOperations {
    ...
    ContainerNode reflectedAttributes(final ContainerNode attributes) {
        ...
        if (maybeClusterList.isPresent()) {
            ...
            addOtherClusterEntries(maybeClusterList, clusterBuilder);
            // Now wrap it in a container and add it to attributes
            final DataContainerNodeAttrBuilder<NodeIdentifier, ContainerNode> clusterListContainer = Builders.containerBuilder();
            clusterListContainer.withNodeIdentifier(this.clusterListContainer);
            clusterListContainer.withChild(clusterBuilder.build());
            attributesContainer.withChild(clusterListContainer.build());
        }
    ...
    ContainerNode reflectedAttributes(final ContainerNode attributes, final Ipv4Address originatorId, final ClusterIdentifier clusterId) {
        ...
        if (!maybeOriginatorId.isPresent()) {
            ...
            attributesContainer.withChild(oib.build());
        }

        return attributesContainer.build();
    }

    private void addOtherClusterEntries(final Optional<NormalizedNode<?, ?>> maybeClusterList, final ListNodeBuilder<Object, LeafSetEntryNode<Object>> clb) {
