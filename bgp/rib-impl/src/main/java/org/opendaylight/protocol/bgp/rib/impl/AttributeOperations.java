...
final class AttributeOperations {
    ...
    private final Iterable<PathArgument> asPathPath;
    ...
    private AttributeOperations(final QNameModule namespace) {
        ...
        this.asPathPath = ImmutableList.<PathArgument>of(asPathContainer, asPathSegments, asPathChoice, asPathId);
    ...
    ContainerNode exportedAttributes(final ContainerNode attributes, final Long localAs) {
        ...
        final ContainerNode transitives = transitiveAttributes(attributes);

        // FIXME: allow transitiveAttributes() only add to the builder
        final DataContainerNodeAttrBuilder<NodeIdentifier, ContainerNode> b = Builders.containerBuilder(transitives);
        b.withNodeIdentifier(transitives.getIdentifier());

        // Create a new AS_PATH builder
        final ListNodeBuilder<Object, LeafSetEntryNode<Object>> apb = Builders.leafSetBuilder();
        apb.withNodeIdentifier(asPathId);

        // prepend local AS into AS_PATH, as per https://tools.ietf.org/html/rfc4271#section-5.1.2
        apb.withChild(Builders.leafSetEntryBuilder().withValue(localAs).build());

        // FIXME: munge apb into the attributes structure
