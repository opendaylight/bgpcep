...
final class AttributeOperations {
    ...
    private LeafSetNode<?> reusableSequence(final UnkeyedListEntryNode segment) {
    ...
    ContainerNode exportedAttributes(final ContainerNode attributes, final Long localAs) {
        ...
        if (maybeOldAsSegments.isPresent() && !((UnkeyedListNode) maybeOldAsSegments.get()).getValue().isEmpty()) {
            ...
            /*
             * We need to check the first entry. As-sequence has to have less than 255 elements, we need to modify that one.
            ...
            final LeafSetNode<?> reusableAsSeq = reusableSequence(firstSegment);
