...
final class EffectiveRibInWriter implements PrefixesReceivedCounters, PrefixesInstalledCounters,
    ...
    private void changeDataTree(
        ...
        switch (modificationType) {
            case DISAPPEARED:
            case DELETE:
                CountersUtil.decrement(this.prefixesInstalled.get(tk), tk);//FIXME for DISAPPEARED
                //remove whole table
            ...
            case APPEARED:
            case WRITE:
                writeTable(tx, tk, table, effectiveTablePath);
                break;
            case UNMODIFIED:
    ...
    private void processTableChildren(
        ...
        for (final DataTreeCandidateNode child : children) {
            ...
            switch (child.getModificationType()) {
                case DELETE:
                case DISAPPEARED:
                   //FIXME here we need to clean up route target cached
                    processDeleteRouteTables(child, childIdentifier, tx, ribSupport, routesPath);
    ...
    private void processDeleteRouteTables(
        ...
        if (TABLE_ROUTES.equals(childIdentifier)) {
            ...
            for (final DataTreeCandidateNode route : changedRoutes) {
                    deleteRoute(tx, ribSupport, routesPath.getParent(), route.getDataBefore().orElse(null), tablesKey);
            }
        }
    }
    ...
    private void processRoute(
        ...
        switch (route.getModificationType()) {
            ...
            case WRITE:
                ...
                final Attributes routeAttrs = ribSupport.attributeFromContainerNode(routePath, advertisedAttrs);
    ...
    private void writeTable(
        ...
        ctx.createEmptyTableStructure(tx, tablePath);
        processTableChildren(tx, ctx.getRibSupport(), tablePath, table.getChildNodes());
    }
