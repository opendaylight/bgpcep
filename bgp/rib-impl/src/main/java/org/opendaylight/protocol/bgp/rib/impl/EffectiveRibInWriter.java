                ...
                case WRITE:
                    if (TABLE_ROUTES.equals(child.getIdentifier())) {
                        ...
                        if (childDataBefore.isPresent()) {
                            final NormalizedNode<?, ?> data = childDataBefore.get();
                            if (data instanceof ChoiceNode) {
                                final Collection<DataContainerChild<? extends PathArgument, ?>> routes = ((ChoiceNode)
                                    childDataBefore.get()).getValue();
                                if (!routes.isEmpty()) {
                                    tx.put(LogicalDatastoreType.OPERATIONAL, childPath, data);
                                }
                            }
                        }
