...
public final class RIBImpl extends DefaultRibReference implements ClusterSingletonService, AutoCloseable, RIB, TransactionChainListener, SchemaContextListener {
    ...
    public RIBImpl(final ClusterSingletonServiceProvider provider, final RibId ribId, final AsNumber localAs, final BgpId localBgpId,
        ...
        final BGPStateProvider stateProvider, final BGPStateFactory bgpStateFactory, final InstanceIdentifier<Bgp> bgpIId) {
        ...
        final Set<Class<? extends AfiSafiType>> afiSafiSupported = OpenConfigMappingUtil
            .toAfiSafiTypeSet(this.localTablesKeys, this.tableTypeRegistry);
        this.globalState = bgpStateFactory.createBGPGlobalState(afiSafiSupported, this.ribId.getValue(),
            this.bgpIdentifier, this.localAs, bgpIId);
        this.renderStats = new RIBImplRuntimeMXBeanImpl(localBgpId, ribId, localAs, cId, this.globalState,
            this.localTablesKeys, this.tableTypeRegistry);
    ...
    private void startLocRib(final TablesKey key) {
        ...
        final UnsignedInt32Counter prefixesCounter = this.globalState.getPrefixesCounter(afiSafi.getAfiSafiName());
        this.locRibs.add(LocRibWriter.create(this.ribContextRegistry, key, createPeerChain(this),
            getYangRibId(), this.localAs, getService(), this.exportPolicyPeerTrackerMap.get(key), pathSelectionStrategy,
            this.globalState.getPathCounter(afiSafi.getAfiSafiName()), prefixesCounter));
    }
