...
final class OpenConfigMappingUtil {
    ...
    static Optional<Integer> getGracefulRestartTimer(final Neighbor neighbor, final PeerGroup peerGroup, final int holdTimer) {
        if(!isGracefulEnabled(neighbor, peerGroup)) {
    ...
    private static boolean isGracefulEnabled(final Neighbor neighbor, final PeerGroup peerGroup) {
        if (peerGroup != null && peerGroup.getGracefulRestart() != null) {
            final GracefulRestart gracefulRestart = peerGroup.getGracefulRestart();
            return gracefulRestart.getConfig() != null && gracefulRestart.getConfig().isEnabled();
        } else {
            if (neighbor != null) {
                final GracefulRestart gracefulRestart = neighbor.getGracefulRestart();
    ...
    private static Optional<Integer> getGracefulRestartTimer(final GracefulRestart gracefulRestart) {
        if (gracefulRestart != null) {
            ...
            if (config != null && config.getRestartTime() != null) {
                return Optional.of(config.getRestartTime());
            }
        }
        return Optional.empty();
    }
