...
final class OpenConfigMappingUtil {
    ...
    private static final List<AfiSafi> DEFAULT_AFISAFI = Collections.singletonList(IPV4_AFISAFI);
    ...
    public static PortNumber getPort(final Neighbor neighbor) {
        if (neighbor.getTransport() != null) {
            ...
            if (config != null && config.getAugmentation(Config1.class) != null) {
                return config.getAugmentation(Config1.class).getRemotePort();
            }
        }
        return PORT;
    }

    //make sure IPv4 Unicast RIB is always there -> RFC 4271
    public static List<AfiSafi> getAfiSafiWithDefault(final BgpCommonAfiSafiList afiSAfis) {
        if (afiSAfis == null || afiSAfis.getAfiSafi() == null) {
            return DEFAULT_AFISAFI;
        }
