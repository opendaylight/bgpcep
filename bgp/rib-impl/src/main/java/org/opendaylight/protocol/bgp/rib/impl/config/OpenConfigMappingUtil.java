...
final class OpenConfigMappingUtil {
    ...
    static RevisedErrorHandlingSupport getRevisedErrorHandling(final PeerRole role,final PeerGroup peerGroup,
        ...
        final RevisedErrorHandlingSupport ret = getRevisedErrorHandling(role, neighbor);
        return ret != null ? ret : getRevisedErrorHandling(role, peerGroup);
    }

    @Nullable
    private static RevisedErrorHandlingSupport getRevisedErrorHandling(final PeerRole role,
            final BgpNeighborGroup group) {
        if (group == null) {
            return null;
        }
        ...
        if (config == null || !config.isTreatAsWithdraw()) {
            return null;
        }
        switch (role) {
            case Ebgp:
