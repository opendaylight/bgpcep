...
public final class OpenConfigMappingUtil {
    ...
    public static List<AfiSafi> toAfiSafisList(final Set<TablesKey> advertizedTables, final BGPTableTypeRegistryConsumer bgpTableTypeRegistryConsumer) {
        return advertizedTables.stream().map(tablesKey -> OpenConfigMappingUtil.toAfiSafi(tablesKey, bgpTableTypeRegistryConsumer))
            .filter(Optional::isPresent).map(Optional::get).collect(Collectors.toList());
    }

    /**
     * Builds a set of AfiSafiType from a set of TablesKey
     *
     * @param tables Set of Tables
     * @param tableTypeRegistry bgp table registry
     * @return AfiSafiType Set
     */
    public static Set<Class<? extends AfiSafiType>> toAfiSafiTypeSet(final Set<TablesKey> tables,
    ...
    /**
     * Builds an AfiSafi from a tableType
     *
     * @param tableType table type
     * @param tableTypeRegistry bgp table registry
     * @return AfiSafi if supported, otherwise empty
     */
    public static Optional<AfiSafi> toAfiSafi(final TablesKey tableType, final BGPTableTypeRegistryConsumer tableTypeRegistry) {
        final Optional<Class<? extends AfiSafiType>> afiSafi = tableTypeRegistry
            .getAfiSafiType(new BgpTableTypeImpl(tableType.getAfi(), tableType.getSafi()));
        if (afiSafi.isPresent()) {
            return Optional.of(new AfiSafiBuilder().setAfiSafiName(afiSafi.get()).build());
        }
        return Optional.empty();
    ...
    /**
     * Builds an AfiSafi with Graceful Restart enable configuration from a tableType if, table type supported.
     *
     * @param tableType table type
     * @param advertizedGracefulRestartTableTypes list containing whole graceful restart table type advertised
     * @param tableTypeRegistry bgp table registry
     * @return AfiSafi if supported, otherwise empty
     */
    public static Optional<AfiSafi> toAfiSafi(final TablesKey tableType, final List<BgpTableType> advertizedGracefulRestartTableTypes,
        final BGPTableTypeRegistryConsumer tableTypeRegistry) {
        final Optional<Class<? extends AfiSafiType>> afiSafi = tableTypeRegistry
            .getAfiSafiType(new BgpTableTypeImpl(tableType.getAfi(), tableType.getSafi()));
        if (afiSafi.isPresent()) {
            final AfiSafiBuilder builder = new AfiSafiBuilder();
            if (advertizedGracefulRestartTableTypes.contains(tableType)) {
                builder.setGracefulRestart(new GracefulRestartBuilder().setConfig(new org.opendaylight.yang.gen.v1.http.openconfig.net.yang.bgp
                    .multiprotocol.rev151009.bgp.common.afi.safi.list.afi.safi.graceful.restart.ConfigBuilder().setEnabled(true).build()).build());
            }
            return Optional.of(builder.setAfiSafiName(afiSafi.get()).build());
        }
        return Optional.empty();
    }
}
