...
final class BestPathState {
    ...
    final class AttributesCollection {
        ...
        public AttributesCollection(final Collection<PathArgument> asPath, final Collection<PathArgument> locPref,
            final Collection<PathArgument> med, final Collection<PathArgument> orig) {
            this.asPath = asPath;
        ...
        try {
            col = attrs.get(attributes.getNodeType().getModule(), new Callable<AttributesCollection>() {
                ...
                public AttributesCollection call() throws Exception {
                    NodeIdentifier container = new NodeIdentifier(QName.cachedReference(QName.create(attributes.getNodeType(), AsPath.QNAME.getLocalName())));
                    NodeIdentifier leaf = new NodeIdentifier(QName.cachedReference(QName.create(attributes.getNodeType(), "segments")));
                    final Collection<PathArgument> as = ImmutableList.<PathArgument>of(container, leaf);

                    container = new NodeIdentifier(QName.cachedReference(QName.create(attributes.getNodeType(), LocalPref.QNAME.getLocalName())));
                    leaf = new NodeIdentifier(QName.cachedReference(QName.create(attributes.getNodeType(), "pref")));
                    final Collection<PathArgument> locPref = ImmutableList.<PathArgument>of(container, leaf);

                    container = new NodeIdentifier(QName.cachedReference(QName.create(attributes.getNodeType(), MultiExitDisc.QNAME.getLocalName())));
                    leaf = new NodeIdentifier(QName.cachedReference(QName.create(attributes.getNodeType(), "med")));
                    final Collection<PathArgument> med = ImmutableList.<PathArgument>of(container, leaf);

                    container = new NodeIdentifier(QName.cachedReference(QName.create(attributes.getNodeType(), Origin.QNAME.getLocalName())));
                    leaf = new NodeIdentifier(QName.cachedReference(QName.create(attributes.getNodeType(), "value")));
                    final Collection<PathArgument> orig = ImmutableList.<PathArgument>of(container, leaf);
                    return new AttributesCollection(as, locPref, med, orig);
                }
            });
        } catch (final ExecutionException e) {
            LOG.error("Error creating namespace-specific attributes collection.");
        }
        Preconditions.checkNotNull(col);
        this.asPath = col.asPath;
        this.locPref = col.locPref;
        this.med = col.med;
        this.orig = col.orig;
    }
