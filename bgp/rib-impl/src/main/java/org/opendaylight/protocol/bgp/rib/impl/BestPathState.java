...
final class BestPathState {
    ...
    public List<Segments> extractSegments(final UnkeyedListNode segments) {
        ...
        for (final UnkeyedListEntryNode seg : segments.getValue()) {
            ...
            sb.setAsSequence(extractAsSequence(seg)).setAsSet(extractAsSet(seg));
            extracted.add(sb.build());
        }
        return extracted;
    }

    private List<AsNumber> extractAsSet(final UnkeyedListEntryNode segments) {
        final List<AsNumber> ases = new ArrayList<>();
        // leaf-list as-set
        final Optional<NormalizedNode<?, ?>> maybeSet = NormalizedNodes.findNode(segments, this.ids.getAsSet());
        if (maybeSet.isPresent()) {
            final LeafSetNode<?> list = (LeafSetNode<AsNumber>)maybeSet.get();
    ...
    private List<AsNumber> extractAsSequence(final UnkeyedListEntryNode segments) {
        final List<AsNumber> ases = new ArrayList<>();
        // leaf-list as-sequence
        final Optional<NormalizedNode<?, ?>> maybeSet = NormalizedNodes.findNode(segments, this.ids.getAsSeq());
        if (maybeSet.isPresent()) {
            final LeafSetNode<?> list = (LeafSetNode<AsNumber>)maybeSet.get();
            for (final LeafSetEntryNode<?> as : list.getValue())  {
                ases.add(new AsNumber((Long)as.getValue()));
