...
public abstract class AbstractBGPSessionNegotiator extends AbstractSessionNegotiator<Notification, BGPSessionImpl> {
    ...
    protected void startNegotiation() {
        ...
        int as = getPreferences().getMyAs().getValue().intValue();
        // Set as AS_TRANS if the value is bigger than 2B
        if (as > Values.UNSIGNED_SHORT_MAX_VALUE) {
            as = AS_TRANS;
        }
        this.sendMessage(new OpenBuilder().setMyAsNumber(as).setHoldTimer(getPreferences().getHoldTime()).setBgpIdentifier(
                getPreferences().getBgpId()).setBgpParameters(getPreferences().getParams()).build());
    ...
    private void handleOpen(final Open openObj) {
        try {
            sessionValidator.validate(openObj, getRemoteIp());
