...
public final class BgpConfigurationService implements ClusteredDataTreeChangeListener<Bgp>, AutoCloseable {

    //TODO make configurable
    private static final String GLOBAL_BGP = "global-bgp";

    private static final Logger LOG = LoggerFactory.getLogger(BgpConfigurationService.class);
    ...
    private final ListenerRegistration<BgpConfigurationService> registration;

    public BgpConfigurationService(final BgpDeploymentManager bgpDeploymentManager) {
        ...
        LOG.info("BGP Deployer {} started.", GLOBAL_BGP);
    }
    ...
    public void onDataTreeChanged(final Collection<DataTreeModification<Bgp>> changes) {
        for (final DataTreeModification<Bgp> dataTreeModification : changes) {
            ...
            final ProtocolKey protocolKey = rootIdentifier.firstKeyOf(Protocol.class);
            ...
            for (final DataObjectModification<? extends DataObject> dataObjectModification : rootNode.getModifiedChildren()) {
                ...
                case DELETE:
                    onDeletedBgp(dataObjectModification, protocolKey);
                    break;
                case SUBTREE_MODIFIED:
                case WRITE:
                    onModifiedBgp(dataObjectModification, protocolKey, rootNode.getDataBefore());
                    break;
                default:
                    break;
                }
    ...
    public void close() throws Exception {
        ...
        this.bgpDeploymentManager.close();
        LOG.info("BGP Deployer {} stopped.", GLOBAL_BGP);
    }

    private void onDeletedBgp(final DataObjectModification<? extends DataObject> dataObjectModification, final ProtocolKey protocolKey) {
        ...
        if (dataObjectModification.getDataType().equals(Global.class)) {
            this.bgpDeploymentManager.onGlobalRemoved(protocolKey);
            ...
            for (final Neighbor neighbor : removed.getNeighbor()) {
                this.bgpDeploymentManager.onNeighborRemoved(protocolKey, neighbor);
            }
        }
    }

    private void onModifiedBgp(final DataObjectModification<? extends DataObject> dataObjectModification, final ProtocolKey protocolKey,
            final Bgp bgp) {
        ...
        if (dataObjectModification.getDataType().equals(Global.class)) {
            ...
            if (dataObjectModification.getModificationType().equals(ModificationType.WRITE)) {
                this.bgpDeploymentManager.onGlobalCreated(protocolKey, global);
            } else {
                this.bgpDeploymentManager.onGlobalModified(protocolKey, bgp);
            }
        } else if (dataObjectModification.getDataType().equals(Neighbors.class)) {
            for (final DataObjectModification<? extends DataObject> modifiedNeighbors : dataObjectModification.getModifiedChildren()) {
                ...
                case DELETE:
                    ...
                    this.bgpDeploymentManager.onNeighborRemoved(protocolKey, removedNeighbor);
                    break;
                case SUBTREE_MODIFIED:
                    final Neighbor updatedNeighbor = (Neighbor) modifiedNeighbors.getDataAfter();
                    this.bgpDeploymentManager.onNeighborModified(protocolKey, updatedNeighbor);
                case WRITE:
                    final Neighbor createNeighbor = (Neighbor) modifiedNeighbors.getDataAfter();
                    this.bgpDeploymentManager.onNeighborCreated(protocolKey, createNeighbor);
                    break;
                default:
                    break;
                }
