...
final class AdjRibInWriter {
    ...
    final void storeStaleRoutes(final Set<TablesKey> gracefulTables) {
        gracefulTables.forEach(tablesKey -> {
            ...
            final YangInstanceIdentifier iid = ctx.getRibSupport().routesPath(ctx.getTableId());
            ...
            try {
                ...
                if (routesOptional.isPresent()) {
                    ...
                    routes.addAll(routesNode.getValue().stream()
    ...
    final void removeStaleRoutes(final TablesKey tableKey) {
        ...
        if (routeKeys != null) {
            routeKeys.forEach(routeKey -> {
                final YangInstanceIdentifier routePath = ctx.getRibSupport().routePath(ctx.getTableId(), routeKey);
    ...
    final FluentFuture<? extends CommitInfo> clearTables(final Set<TablesKey> tablesToClear) {
        ...
        tablesToClear.forEach(tableKey -> {
            final TableContext ctx = this.tables.get(tableKey);
            wtx.delete(LogicalDatastoreType.OPERATIONAL, ctx.getRibSupport().routesPath(ctx.getTableId()).getParent());
        });
        return wtx.commit();
    }
}
