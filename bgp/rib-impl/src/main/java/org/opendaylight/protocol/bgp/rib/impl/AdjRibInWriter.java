...
final class AdjRibInWriter {
    ...
    synchronized void updateRoutes(final MpReachNlri nlri, final org.opendaylight.yang.gen.v1.urn.opendaylight.params
        ...
        final Set<NodeIdentifierWithPredicates> staleRoutes = this.staleRoutesRegistry.get(key);
        if (staleRoutes != null) {
    ...
    synchronized final void storeStaleRoutes(final Set<TablesKey> gracefulTables) {
        gracefulTables.forEach(tablesKey -> {
            ...
            try {
                ...
                if (routesOptional.isPresent()) {
                    ...
                    final Set<NodeIdentifierWithPredicates> routes = routesNode.getValue().stream()
                            .map(MapEntryNode::getIdentifier)
                            .collect(Collectors.toSet());
                    if (!routes.isEmpty()) {
                        this.staleRoutesRegistry.put(tablesKey, routes);
                    }
                }
    ...
    synchronized final void removeStaleRoutes(final TablesKey tableKey) {
        ...
        routeKeys.forEach(routeKey -> {
            final YangInstanceIdentifier routePath = ctx.routePath(routeKey);
    ...
    synchronized final FluentFuture<? extends CommitInfo> clearTables(final Set<TablesKey> tablesToClear) {
