...
final class AdjRibInWriter {
    ...
    void storeLlStaleRoutes(final Set<TablesKey> llGracefulTables) {
        ...
        for (TablesKey tablesKey : llGracefulTables) {
            ...
            // FIXME: write llgr-stale container
                ...
                public void onSuccess(final Optional<NormalizedNode<?, ?>> result) {
                    try {
                        if (result.isPresent()) {
                            ...
                            final Collection<MapEntryNode> routes = routesNode.getValue();
                            if (!routes.isEmpty()) {
                                final Set<NodeIdentifierWithPredicates> staleIds = routesNode.getValue().stream()
                                        .map(MapEntryNode::getIdentifier).collect(Collectors.toSet());
                                synchronized (AdjRibInWriter.this.llStaleRoutesRegistry) {
                                    AdjRibInWriter.this.llStaleRoutesRegistry.put(tablesKey, staleIds);
                                }
                            }
                        }
                    } finally {
