...
final class AdjRibInWriter {
     ...
     */
    private AdjRibInWriter(final YangInstanceIdentifier ribId, final DOMTransactionChain chain, final String role, final YangInstanceIdentifier tablesRoot, final Map<TablesKey, TableContext> tables) {
    ...
    AdjRibInWriter transform(final Ipv4Address newPeerId, final RIBExtensionConsumerContext registry, final Set<TablesKey> tableTypes) {
        ...
        if (!newPeerId.equals(this.peerId)) {
            if (peerId != null) {
                ...
                tx.delete(LogicalDatastoreType.OPERATIONAL, ribId.node(Peer.QNAME).node(new NodeIdentifierWithPredicates(Peer.QNAME, PEER_ID_QNAME, peerId.getValue())));
            }
            ...
            final YangInstanceIdentifier newPeerPath = ribId.node(Peer.QNAME).node(peerKey);
        ...
        return new AdjRibInWriter(ribId, chain, role, newTablesRoot, tb.build());
    }
