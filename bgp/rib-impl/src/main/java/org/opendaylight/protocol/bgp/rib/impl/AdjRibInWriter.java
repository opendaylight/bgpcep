...
final class AdjRibInWriter {
    ...
    @GuardedBy("this")
    private final Map<TablesKey, Set<NodeIdentifierWithPredicates>> llStaleRoutesRegistry = new HashMap<>();
    @GuardedBy("this")
    ...
    void updateRoutes(final MpReachNlri nlri, final org.opendaylight.yang.gen.v1.urn.opendaylight.params
        ...
        final Set<NodeIdentifierWithPredicates> llStaleRoutes = this.llStaleRoutesRegistry.get(key);
        if (staleRoutes != null) {
        ...
        try (final DOMDataReadOnlyTransaction tx = this.chain.getDomChain().newReadOnlyTransaction()) {
            ...
            gracefulTables.forEach(tablesKey -> {
                ...
                Futures.addCallback(readFuture, new FutureCallback<Optional<NormalizedNode<?, ?>>>() {
                    @Override
                    public void onSuccess(Optional<NormalizedNode<?, ?>> routesOptional) {
                    ...
                    public void onFailure(Throwable throwable) {
                        LOG.error("Failed to store stale routes for table {}", tablesKey, throwable);
                    }
                }, threadPool);
            });
            readFutures.entrySet().stream().forEach(entry -> {
                try {
                    ...
                    LOG.error("Failed to store stale routes for table {}", entry.getKey(), e);
                }
            });
            threadPool.shutdown();
        }
    ...
    private void removeStaleRoutes(final TablesKey tableKey,
                            final Map<TablesKey, Set<NodeIdentifierWithPredicates>> routeRegistry) {
        final TableContext ctx = this.tables.get(tableKey);
        if (ctx == null) {
            LOG.debug("No table for {}, not removing any stale routes", tableKey);
            return;
        }
        final Set<NodeIdentifierWithPredicates> routeKeys = routeRegistry.get(tableKey);
        ...
        future.addCallback(new FutureCallback<CommitInfo>() {
            ...
            @Override
            public void onFailure(final Throwable throwable) {
                LOG.error("Removing routes {}, failed", routeKeys, throwable);
            }
        }, MoreExecutors.directExecutor());
    }
