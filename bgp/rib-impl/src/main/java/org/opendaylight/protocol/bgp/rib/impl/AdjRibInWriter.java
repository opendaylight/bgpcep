...
final class AdjRibInWriter {
    ...
    private static final QName AFI_QNAME = QName.cachedReference(QName.create(Peer.QNAME, "afi"));
    private static final QName SAFI_QNAME = QName.cachedReference(QName.create(Peer.QNAME, "safi"));
    ...
    AdjRibInWriter transform(final PeerId newPeerId, final RIBSupportContextRegistry registry, final Set<TablesKey> tableTypes, final boolean isAppPeer) {
        ...
        for (final TablesKey k : tableTypes) {
            ...
            if (ctx == null) {
                ...
                if (!isAppPeer) {
                    final ImmutableMap<QName, Object> keyValues = ImmutableMap.<QName, Object> of(AFI_QNAME, BindingReflections.findQName(k.getAfi()), SAFI_QNAME, BindingReflections.findQName(k.getSafi()));
                    final NodeIdentifierWithPredicates supTablesKey = new NodeIdentifierWithPredicates(SupportedTables.QNAME, keyValues);
                    ...
                    rs.clearTable(tx, newPeerPath.node(EMPTY_ADJRIBOUT.getIdentifier()).node(TABLES).node(supTablesKey));
                }
