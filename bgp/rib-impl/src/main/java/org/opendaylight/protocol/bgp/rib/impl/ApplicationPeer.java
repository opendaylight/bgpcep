...
public class ApplicationPeer implements AutoCloseable, org.opendaylight.protocol.bgp.rib.spi.Peer, DOMDataTreeChangeListener, TransactionChainListener {
        ...
        for (final DataTreeCandidate tc : changes) {
            ...
            for (final DataTreeCandidateNode child : tc.getRootNode().getChildNodes()) {
                ...
                case WRITE:
                    ...
                    } else if (child.getDataAfter().isPresent()) {
    ...
    private Attributes routeAttributes(final MapEntryNode route, final RIBSupport ribSupport) {
    ...
    protected void checkMandatoryAttributesPresence(final Attributes attrs) throws BGPDocumentedException {
        if (attrs == null) {
            throw new BGPDocumentedException("Well known mandatory attribute missing", BGPError.WELL_KNOWN_ATTR_MISSING);
        }
        ...
        if (missingAttr != null) {
            throw new BGPDocumentedException("Mandatory attribute " + missingAttr + " is not present", BGPError.WELL_KNOWN_ATTR_MISSING);
        }
    }
