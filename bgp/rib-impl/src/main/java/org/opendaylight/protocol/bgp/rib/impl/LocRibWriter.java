...
final class LocRibWriter implements DOMDataTreeChangeListener {
    ...
    public void onDataTreeChanged(final Collection<DataTreeCandidate> changes) {
        ...
        for (Entry<PathArgument, RouteEntry> e : toUpdate.entrySet()) {
            /*
             * FIXME: we need to keep track of routers and populate adj-ribs-out, too. If we do not, we need to
             *        expose whether a particular route was learned from a Route Reflector client in the local
             *        rib, and have the writer act accordingly.
             */
            final RouteEntry entry = e.getValue();
            if (entry != null) {
                if (entry.selectBest(ourAs)) {
                    final NormalizedNode<?, ?> value = entry.bestValue(e.getKey());
                    if (value != null) {
                        tx.put(LogicalDatastoreType.OPERATIONAL, target.node(e.getKey()), value);
                    } else {
