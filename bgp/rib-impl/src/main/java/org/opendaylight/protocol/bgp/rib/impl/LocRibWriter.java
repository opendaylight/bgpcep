...
final class LocRibWriter implements AutoCloseable, DOMDataTreeChangeListener {
        ...
        if (!table.getDataBefore().isPresent() && isTableSupported(peerIdOfNewPeer)) {
            ...
            for (Map.Entry<PathArgument, AbstractRouteEntry> entry : this.routeEntries.entrySet()) {
                ...
                for (BestPath path : bestPath) {
                    final NormalizedNode<?, ?> value = routeEntry.createValue(routeId, path);
    ...
    private void updateRoutesEntries(final DataTreeCandidateNode child, final PeerId peerId, final Map<RouteUpdateKey, AbstractRouteEntry> routes) {
        ...
        for (final DataTreeCandidateNode route : modifiedRoutes) {
            ...
            AbstractRouteEntry abstractRouteEntry = this.routeEntries.get(routeId);
            final String prefix = getPrefix(route);

            final Optional<NormalizedNode<?, ?>> maybeData = route.getDataAfter();
            if (maybeData.isPresent()) {
                if (abstractRouteEntry == null) {
                    abstractRouteEntry = createEntry(routeId);
                }
                abstractRouteEntry.addRoute(routerId, prefix, this.attributesIdentifier, maybeData.get());
            } else if (abstractRouteEntry != null && abstractRouteEntry.removeRoute(routerId, prefix)) {
                this.routeEntries.remove(routeId);
                LOG.trace("Removed route from {}", routerId);
            }
            final RouteUpdateKey routeUpdateKey = new RouteUpdateKey(peerId, routeId);
            LOG.debug("Updated route {} entry {}", routeId, abstractRouteEntry);
            routes.put(routeUpdateKey, abstractRouteEntry);
        }
    }
    ...
    private void walkThrough(final DOMDataWriteTransaction tx, final Map<RouteUpdateKey, AbstractRouteEntry> toUpdate) {
        for (final Entry<RouteUpdateKey, AbstractRouteEntry> e : toUpdate.entrySet()) {
            ...
            final AbstractRouteEntry abstractRouteEntry = e.getValue();
            final RouteUpdateKey routeUpdateKey = e.getKey();
            final PathArgument routeId = routeUpdateKey.getRouteId();

            if (!abstractRouteEntry.selectBest(this.ourAs, getNBestPaths())) {
                // Best path has not changed, no need to do anything else. Proceed to next route.
                LOG.trace("Continuing");
                continue;
            }

            final List<BestPath> bestPathRemoved = abstractRouteEntry.getBestPathRemovedList();
            bestPathRemoved.forEach(path -> removePathFromDataStore(path, routeUpdateKey.getPeerId(), routeId, tx));

            final List<BestPath> bestPath = abstractRouteEntry.getBestPathList();
            bestPath.forEach(path -> addPathToDataStore(path, abstractRouteEntry, routeId, tx));
        }
    }
    ...
    private void addPathToDataStore(final BestPath path, final AbstractRouteEntry abstractRouteEntry, final PathArgument routeId,
        ...
        final NormalizedNode<?, ?> value = abstractRouteEntry.createValue(routeIdAddPath, path);
        LOG.trace("Selected best value {}", value);
        fillLocRib(tx, value, routeIdAddPath);
        fillAdjRibsOut(tx, path.getState().getAttributes(), value, routeId, routePeerId, routeIdAddPath);
    }
            ...
            if (peerGroup != null) {
                ...
                for (final Entry<PeerId, YangInstanceIdentifier> pid : peerGroup.getPeers()) {
                    ...
                    if (!routePeerId.equals(peerDestiny) && isTableSupported(peerDestiny) && !this.cacheDisconnectedPeers.isPeerDisconnected(peerDestiny)) {
                        YangInstanceIdentifier routeTarget = getRouteTarget(pid.getValue(), routeId);
                        if (isAddPathSupportedAndInsideTheNbestPaths()) {
                            routeTarget = getRouteTarget(pid.getValue(), routeIdAddPath);
                        }
