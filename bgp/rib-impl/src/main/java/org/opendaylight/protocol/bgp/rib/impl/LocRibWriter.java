...
final class LocRibWriter<C extends Routes & DataObject & ChoiceIn<Tables>, S extends ChildOf<? super C>>
    ...
    private volatile FastThreadPoolExecutor executor;
    ...
    private synchronized void init() {
        // Single thread, so batches run in the order they arrived. The thread is created on demand and reclaimed
        // when idle.
        executor = new FastThreadPoolExecutor(1, 10_000, "bgp-locrib-writer",
            LocRibWriter.class);
        // Caller-waits policy. When the 10000-batch queue is full the submitting thread blocks until a slot frees,
        // rather than running the batch on the submitting thread or dropping it, so arrival order holds.
        executor.setRejectedExecutionHandler(CountingRejectedExecutionHandler.newCallerWaitsPolicy());
    ...
    public synchronized void close() {
        ...
        if (executor != null) {
            // shutdownNow so a restart's new executor does not run concurrently with queued batches of the old
            // one. The owner check in processChanges skips any batch that was already dequeued before shutdown.
    ...
    public void onDataTreeChanged(final List<DataTreeCandidate> changes) {
        ...
        try {
            exec.execute(() -> processChanges(exec, changes));
        } catch (final RejectedExecutionException e) {
            LOG.trace("Writer shut down mid-submit, ignoring received data change {} to LocRib {}", changes, this, e);
        }
    }
    ...
    private synchronized void processChanges(final ExecutorService owner, final List<DataTreeCandidate> changes) {
        if (owner != executor) {
            // A restart replaced the executor, so this batch was queued against the old chain. Skip it to avoid
            // reordering against batches on the new chain.
            LOG.trace("Writer restarted, ignoring stale data change {} to LocRib {}", changes, this);
            return;
        }
        if (chain == null) {
    ...
    private synchronized void onPeerAdded(final @NonNull Peer peer) {
        ...
        pendingRibOut.put(peer.getPeerId(), peer.initializeRibOut(entryDep, routesToStore));
    }
    ...
    public void refreshTable(final TablesKey tk, final PeerId peerId) {
        ...
        try {
            exec.execute(() -> refreshTableNow(exec, peerId));
        } catch (final RejectedExecutionException e) {
            LOG.trace("Writer shut down mid-submit, ignoring table refresh for peer {}", peerId, e);
        }
    }

    private synchronized void refreshTableNow(final ExecutorService owner, final PeerId peerId) {
        if (owner != executor) {
            // A restart replaced the executor, so this refresh was queued against the old chain. Skip it to avoid
            // reordering against batches on the new chain.
            LOG.trace("Writer restarted, ignoring stale table refresh for peer {}", peerId);
            return;
        }
        final var toPeer = peerTracker.getPeer(peerId);
        // An application peer (PeerRole.Internal) has no BGP session, so nothing reads its AdjRibsOut. Skip it,
        // as onPeerAdded does.
        if (toPeer != null && toPeer.getRole() != PeerRole.Internal
                && toPeer.supportsTable(entryDep.getLocalTablesKey())) {
            ...
            pendingRibOut.put(peerId, toPeer.reEvaluateAdvertizement(entryDep, routesToStore));
        }
    }
