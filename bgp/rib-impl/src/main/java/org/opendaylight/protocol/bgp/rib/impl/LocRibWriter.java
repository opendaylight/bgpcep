        ...
        for (final PeerRole role : PeerRole.values()) {
            ...
            if (peerGroup != null) {
                ...
                for (final Entry<PeerId, YangInstanceIdentifier> pid : peerGroup.getPeers()) {
                    final YangInstanceIdentifier routeTarget = this.ribSupport.routePath(pid.getValue().node(AdjRibOut.QNAME).node(Tables.QNAME).node(this.tableKey).node(Routes.IDENTIFIER), key.getRouteId());
