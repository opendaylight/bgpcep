...
final class BGPSessionNegotiator extends AbstractSessionNegotiator<BGPMessage, BGPSessionImpl> {
	...
	BGPSessionNegotiator(Timer timer, Promise<BGPSessionImpl> promise, Channel channel,
			BGPSessionPreferences initialPrefs, BGPSessionListener listener) {
	...
	protected void startNegotiation() {
		...
		channel.write(new BGPOpenMessage(localPref.getMyAs(), (short) localPref.getHoldTime(), localPref.getBgpId(), localPref.getParams()));
	...
	protected synchronized void handleMessage(final BGPMessage msg) {
		...
		channel.write(new BGPNotificationMessage(BGPError.FSM_ERROR));
