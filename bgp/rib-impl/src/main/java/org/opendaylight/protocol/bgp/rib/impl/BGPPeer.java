...
public class BGPPeer extends BGPPeerState implements BGPSessionListener, Peer, AutoCloseable, BGPPeerRuntimeMXBean, TransactionChainListener {
    ...
    public BGPPeer(final IpAddress neighborAddress, final RIB rib, final PeerRole role, final SimpleRoutingPolicy peerStatus,
        ...
        this.peerStats = new BGPPeerStatsImpl(this.name, this.tables, this.neighborState, this.tableTypeRegistry);
    ...
    public synchronized void onSessionUp(final BGPSession session) {
        ...
        if(!isLearnNone(this.simpleRoutingPolicy)) {
            this.effRibInWriter = EffectiveRibInWriter.create(this.rib.getService(), this.rib.createPeerChain(this), this.peerIId,
                this.rib.getImportPolicyPeerTracker(), this.rib.getRibSupportContext(), this.peerRole, this.peerStats.getAdjRibInRouteCounters(),
