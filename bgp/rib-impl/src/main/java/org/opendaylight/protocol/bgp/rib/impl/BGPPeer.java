...
public class BGPPeer extends AbstractPeer implements BGPSessionListener {
    ...
    private synchronized void onUpdateMessage(final Update message) throws BGPDocumentedException {
        ...
        if (mpUnreach != null) {
            ...
            this.ribWriter.removeLlStaleRoutes(IPV4_UCAST_TABLE_KEY);
            handleGracefulEndOfRib();
        }
    }
    ...
    public synchronized void onSessionUp(final BGPSession session) {
        ...
        if (llTables != null) {
            llTables.forEach(table -> llTablesReceived.put(new TablesKey(table.getAfi(), table.getSafi()),
        ...
        if (this.llgrSupport) {
            llTablesReceived.entrySet().forEach(entry -> {
    ...
    private synchronized void addBgp4Support(final boolean llgrSupport) {
        if (!this.tables.contains(IPV4_UCAST_TABLE_KEY)) {
            ...
            createAdjRibOutListener(IPV4_UCAST_TABLE_KEY, false, llgrSupport);
        }
    }
