...
public class BGPPeer extends BGPPeerState implements BGPSessionListener, Peer, AutoCloseable, BGPPeerRuntimeMXBean, TransactionChainListener {
    ...
    public BGPPeer(final IpAddress neighborAddress, final RIB rib, final PeerRole role, final SimpleRoutingPolicy peerStatus,
        final RpcProviderRegistry rpcRegistry, final Set<Class<? extends AfiSafiType>> afiSafis,
        final BGPTableTypeRegistryConsumer tableTypeRegistry) {
        super(neighborAddress, afiSafis, tableTypeRegistry);
        this.peerRole = role;
    ...
    public BGPPeer(final IpAddress neighborAddress, final RIB rib, final PeerRole role, final RpcProviderRegistry rpcRegistry,
        final Set<Class<? extends AfiSafiType>> afiSafis, final BGPTableTypeRegistryConsumer tableTypeRegistry) {
        this(neighborAddress, rib, role, null, rpcRegistry, afiSafis, tableTypeRegistry);
    }
    ...
    private void onUpdateMessage(final Update message) throws BGPDocumentedException {
        ...
        if (mpReach != null) {
            final Class<? extends AfiSafiType> afiSafi = OpenConfigMappingUtil
                .toAfiSafi(mpReach, this.tableTypeRegistry).get().getAfiSafiName();
            this.ribWriter.updateRoutes(mpReach, nextHopToAttribute(attrs, mpReach), afiSafi);
        }
    ...
    private void createAdjRibOutListener(final PeerId peerId, final TablesKey key, final boolean mpSupport) {
        ...
        if (context != null && this.session instanceof BGPSessionImpl) {
            ...
            this.adjRibOutListenerSet.put(key, AdjRibOutListener.create(peerId, key, afiSafi.getAfiSafiName(),
    ...
    public void onTransactionChainFailed(final TransactionChain<?, ?> chain, final AsyncTransaction<?, ?> transaction, final Throwable cause) {
        ...
        this.ribWriter = AdjRibInWriter.create(this.rib.getYangRibId(), this.peerRole, this.simpleRoutingPolicy, this.chain, this.neighborState);
        releaseConnection();
    }
