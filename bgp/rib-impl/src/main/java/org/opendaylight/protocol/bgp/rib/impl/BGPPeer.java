...
public class BGPPeer implements BGPSessionListener, Peer, AutoCloseable, BGPPeerRuntimeMXBean, TransactionChainListener {
    ...
    public BGPPeer(final String name, final RIB rib, final PeerRole role) {
        this.rib = (RIBImpl)Preconditions.checkNotNull(rib);
        this.name = name;
        this.chain = rib.createPeerChain(this);
        this.peerRole = role;
        this.ribWriter = AdjRibInWriter.create(rib.getYangRibId(), role, this.chain);
        this.exportPolicyPeerTracker = this.rib.getExportPolicyPeerTracker();
    }
    ...
    public synchronized void onSessionUp(final BGPSession session) {
        ...
        this.effRibInWriter = EffectiveRibInWriter.create(this.rib.getService(), this.rib.createPeerChain(this), this.peerIId, this.rib.getImportPolicyPeerTracker(),
