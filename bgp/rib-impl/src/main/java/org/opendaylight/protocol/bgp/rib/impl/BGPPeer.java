...
public class BGPPeer implements BGPSessionListener, Peer, AutoCloseable, BGPPeerRuntimeMXBean, TransactionChainListener {
    ...
    private final Set<AutoCloseable> tableRegistration = new HashSet<>();
    ...
    public synchronized void onSessionUp(final BGPSession session) {
        ...
        this.tables.forEach(tablesKey -> {
            ...
            if (exportTracker != null) {
                tableRegistration.add(exportTracker.registerPeer(peerId, addPathTableMaps.get(tablesKey), this.peerIId, this.peerRole,
    ...
    private void closeRegistration() {
        for (final AutoCloseable tableCloseable : tableRegistration) {
            try {
