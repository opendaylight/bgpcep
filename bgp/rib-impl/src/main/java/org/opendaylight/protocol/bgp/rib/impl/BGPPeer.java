...
public class BGPPeer implements BGPSessionListener, Peer, AutoCloseable, BGPPeerRuntimeMXBean, TransactionChainListener {
    ...
    /**
     * Check for presence of mandatory path attributes in Update message
     *
    ...
    private void checkMandatoryAttributesPresence(final Update message) throws BGPDocumentedException {
        ...
        if (isAnyNlriPresent && !isNextHopAttributePresent) {
            final String error = "Mandatory attribute NEXT_HOP is not present";
            LOG.info(error);
            throw new BGPDocumentedException(error, BGPError.WELL_KNOWN_ATTR_MISSING,
        ...
        if (isAnyNlriPresent || isMpNlriPresent) {
            ...
            if (!isOriginAttributePresent) {
                final String error = "Mandatory attribute ORIGIN is not present";
                LOG.info(error);
                throw new BGPDocumentedException(error, BGPError.WELL_KNOWN_ATTR_MISSING,
            ...
            if (!isAsPathAttributePresent) {
                final String error = "Mandatory attribute AS_PATH is not present";
                LOG.info(error);
                throw new BGPDocumentedException(error, BGPError.WELL_KNOWN_ATTR_MISSING,
            ...
            if (this.peerRole == PeerRole.Ibgp && !isLocalPrefAttributePresent) {
                final String error = "Mandatory attribute LOCAL_PREF is not present";
                LOG.info(error);
                throw new BGPDocumentedException(error, BGPError.WELL_KNOWN_ATTR_MISSING,
