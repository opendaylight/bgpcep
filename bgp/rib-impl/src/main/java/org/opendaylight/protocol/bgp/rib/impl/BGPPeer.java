...
public class BGPPeer extends AbstractPeer implements BGPSessionListener {
    ...
    public synchronized void onSessionUp(final BGPSession session) {
        ...
        if (llTables != null && !llTables.isEmpty()) {
        ...
        this.llgrSupport = llTables != null;
    ...
    private synchronized void handleRestartTimer(final Set<TablesKey> gracefulTables) {
        ...
        new ScheduledThreadPoolExecutor(1)
                .schedule(() -> handleRestartTimer(gracefulTables), restartExpire - ct, TimeUnit.NANOSECONDS);
    }
    ...
    private synchronized void handleLlRestartTimer(final TablesKey table, final int timer) {
        ...
        final long ct = System.nanoTime();
        final long restartExpire = this.peerRestartStart + TimeUnit.SECONDS.toNanos(timer);
        ...
        new ScheduledThreadPoolExecutor(1)
                .schedule(() -> handleLlRestartTimer(table, timer), restartExpire - ct, TimeUnit.NANOSECONDS);
    }
    ...
    private synchronized void handleSelectionReferralTimer() {
        ...
        new ScheduledThreadPoolExecutor(1)
                .schedule(this::handleSelectionReferralTimer, timerExpire - ct, TimeUnit.NANOSECONDS);
    }
