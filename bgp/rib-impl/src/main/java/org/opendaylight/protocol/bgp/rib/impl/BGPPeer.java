...
public class BGPPeer extends AbstractPeer implements BGPSessionListener {
    ...
    public synchronized FluentFuture<? extends CommitInfo> releaseConnection() {
        ...
        if (!isPeerRestarting()) {
            ...
            this.ribWriter.removeTables(getTablesToRemove(this.tables));
    ...
    private List<TablesKey> getTablesToRemove(Set<TablesKey> allTableKeys) {
        return allTableKeys.stream()
                .filter(tableKey-> !isGracefulRestartAdvertized(tableKey))
                .collect(Collectors.toList());
    }
