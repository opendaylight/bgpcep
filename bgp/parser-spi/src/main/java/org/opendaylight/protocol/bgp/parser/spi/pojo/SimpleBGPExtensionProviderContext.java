...
public class SimpleBGPExtensionProviderContext extends SimpleBGPExtensionConsumerContext implements BGPExtensionProviderContext {
	...
	private final ReferenceCache<?> referenceCache = new ReferenceCache<Object>() {
		@Override
		public Object getSharedReference(final Object object) {
			final Cache<Object, Object> cache = cacheRef.get();

			final Object ret = cache.getIfPresent(object);
			if (ret == null) {
	...
	@Override
	public <T> ReferenceCache<T> getReferenceCache(final Class<T> objectClass) {
		@SuppressWarnings("unchecked")
		final ReferenceCache<T> ret = (ReferenceCache<T>) referenceCache;
		return ret;
	}
