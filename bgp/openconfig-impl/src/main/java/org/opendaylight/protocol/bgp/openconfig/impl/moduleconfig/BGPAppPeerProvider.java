...
final class BGPAppPeerProvider {
    ...
    public void onNeighborModified(final Neighbor modifiedAppNeighbor) {
        ...
        if (moduleKey != null) {
            ...
            if (ribImplKey != null) {
                try {
                    final TargetRib ribFuture = RibInstanceFunction.getRibInstance(this.configModuleOp, this.TO_RIB_FUNCTION, ribImplKey.getName(), rTx);
                    final Module peerConfigModule = toPeerConfigModule(modifiedAppNeighbor, ribFuture);
                    configModuleOp.putModuleConfiguration(peerConfigModule, dataBroker.newWriteOnlyTransaction());
