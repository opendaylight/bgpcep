...
final class BGPPeerProvider {
    ...
    private List<AdvertizedTable> getAdvertizedTables(final Neighbor modifiedNeighbor, final ReadOnlyTransaction rTx) {
        try {
            return new TableTypesFunction<>(rTx, configModuleOp, ADVERTIZED_TABLE_FUNCTION).apply(modifiedNeighbor.getAfiSafis().getAfiSafi()).get();
        } catch (Exception e) {
            LOG.error("Failed to read table: {}", modifiedNeighbor.getAfiSafis().getAfiSafi(), e);
            throw new IllegalStateException(e);
        }
    }
    ...
    private void createNewPeerConfiguration(final ModuleKey moduleKey, final Neighbor modifiedNeighbor, final List<AdvertizedTable>
        ...
        if (ribImplKey != null) {
            try {
                ...
                final Module peerConfigModule = toPeerConfigModule(modifiedNeighbor, advertizedTables, ribFuture.get());
                configModuleOp.putModuleConfiguration(peerConfigModule, dataBroker.newWriteOnlyTransaction());
