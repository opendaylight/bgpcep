...
final class BGPPeerProvider {
    ...
    private void createNewPeerConfiguration(final ModuleKey moduleKey, final Neighbor modifiedNeighbor, final List<AdvertizedTable>
        ...
        if (ribImplKey != null) {
            try {
                final Rib ribFuture = RibInstanceFunction.getRibInstance(this.configModuleOp, this.TO_RIB_FUNCTION, ribImplKey.getName(), rTx);
                final Module peerConfigModule = toPeerConfigModule(modifiedNeighbor, advertizedTables, ribFuture);
                configModuleOp.putModuleConfiguration(peerConfigModule, dataBroker.newWriteOnlyTransaction());
