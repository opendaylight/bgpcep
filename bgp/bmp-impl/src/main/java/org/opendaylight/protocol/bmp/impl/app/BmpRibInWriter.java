...
final class BmpRibInWriter {
    ...
    public void onMessage(final UpdateMessage message) {
        ...
        if (!checkEndOfRib(message)) {
            final org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.message.rev130919.path.attributes.Attributes attrs = message.getAttributes();
    ...
    private MpReachNlri prefixesToMpReach(final UpdateMessage message) {
        ...
        if (message.getAttributes() != null) {
            final Ipv4NextHop nextHop = ((Ipv4NextHopCase) message.getAttributes().getCNextHop()).getIpv4NextHop();
            b.setCNextHop(new Ipv4NextHopCaseBuilder().setIpv4NextHop(new Ipv4NextHopBuilder().setGlobal(nextHop.getGlobal())
                .build()).build());
        }
        return b.build();
    }
