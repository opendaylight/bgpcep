...
public class RouteMirroringMessageHandler extends AbstractBmpPerPeerMessageParser<TlvsBuilder> {
    ...
    public Notification parseMessageBody(final ByteBuf bytes) throws BmpDeserializationException {
        final RouteMirroringMessageBuilder routeMonitor = new RouteMirroringMessageBuilder().setPeerHeader(parsePerPeerHeader(bytes));
        final TlvsBuilder tlvsBuilder = new TlvsBuilder();
        parseTlvs(tlvsBuilder, bytes);
        return routeMonitor.setTlvs(tlvsBuilder.build()).build();
    }
