 ...
 */
public class BmpSynchronization {
    private static final Logger LOG = LoggerFactory.getLogger(BmpSynchronization.class);
    @VisibleForTesting
    public final Map<TablesKey, SyncVariables> syncStorage = Maps.newHashMap();
    private final BmpSessionListener listener;

    public BmpSynchronization(final BmpSessionListener listener, final Set<TablesKey> types) {
    ...
    public void updReceived(final Update msg) {
        ...
        if (s == null) {
            LOG.warn("BMPTableType was not present in open message : {}", type);
            return;
        }
        s.setUpd(true);
        if (isEOR) {
            ...
            LOG.info("BMP Synchronization finished for table {} ", type);
        }
    }
    ...
    public void kaReceived() {
        for (final Map.Entry<TablesKey, SyncVariables> entry : this.syncStorage.entrySet()) {
