...
public class BmpRouterImpl implements BmpRouter, TransactionChainListener {
    ...
    private void createRouterEntry() {
        ...
        wTx.put(LogicalDatastoreType.OPERATIONAL, this.routerYangIId,
                ...
                .withNodeIdentifier(new NodeIdentifierWithPredicates(Router.QNAME, ROUTER_ID_QNAME, this.routerId.getValue()))
                .withChild(ImmutableNodes.leafNode(ROUTER_ID_QNAME, this.routerId.getValue()))
    ...
    private void onPeerUp(final PeerUpNotification peerUp) {
        final PeerId peerId = getPeerId(peerUp);
        if (!getPeer(peerId).isPresent()) {
            final BmpRouterPeer peer = BmpRouterPeerImpl.createRouterPeer(this.domTxChain, this.peersYangIId, peerUp, this.extensions, this.tree);
            this.peers.put(peerId, peer);
            LOG.debug("Router {}: Peer {} goes up.", this.routerId.getValue(), peerId.getValue());
        } else {
            LOG.debug("Peer: {} for Router: {} already exists.", peerId.getValue(), this.routerId.getValue());
        }
    }
    ...
    private void delegateToPeer(final Notification perPeerMessage) {
        ...
        if (maybePeer.isPresent()) {
            ...
            LOG.debug("Peer: {} for Router: {} was not found.", peerId.getValue(), this.routerId.getValue());
        }
    }
