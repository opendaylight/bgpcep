...
public class PeerUpHandler extends AbstractBmpPerPeerMessageParser {
    ...
    public void serializeMessage(final Notification message, final ByteBuf buffer) {
        ...
        buffer.writeByte(peerUp.getLocalPort().getValue());
        buffer.writeByte(peerUp.getRemotePort().getValue());
    ...
    public Notification parseMessage(final ByteBuf bytes) {
        ...
        peerUpNot.setPeerHeader(header);
        if(header.getAddress().getIpv4Address() != null){
            ...
            peerUpNot.setLocalAddress(new IpAddress(new Ipv6Address(TlvUtil.parseIPV(ByteArray.readBytes(bytes,
                IPV6_SIZE)))));
        }
        final int localPortNumber = bytes.readUnsignedByte();
        peerUpNot.setLocalPort(new PortNumber(localPortNumber));
        final int remotePortNumber = bytes.readUnsignedByte();
