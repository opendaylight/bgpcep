...
public class BmpDispatcherImpl extends AbstractDispatcher<BmpSessionImpl, BmpSessionListener> implements AutoCloseable, BmpDispatcher {
    ...
    public BmpDispatcherImpl(final BmpMessageRegistry messageRegistry,
                             final EventLoopGroup bossGroup, final EventLoopGroup workerGroup,
                             final MD5ChannelFactory<?> cf, final MD5ServerChannelFactory<?> scf) {
        this(messageRegistry, null, bossGroup, workerGroup, cf, scf);
    }

    protected BmpDispatcherImpl(final BmpMessageRegistry messageRegistry, final SessionListenerFactory<BmpSessionListener> negotiatorFactory,
                             final EventLoopGroup bossGroup, final EventLoopGroup workerGroup,
                             final MD5ChannelFactory<?> cf, final MD5ServerChannelFactory<?> scf) {
    ...
    public Future<Void> createReconnectingClient(final InetSocketAddress address, final ReconnectStrategyFactory
        connectStrategyFactory, final ReconnectStrategyFactory reestablishStrategyFactory, final SessionListenerFactory<BmpSessionListener> slf) {
        return this.createReconnectingClient(address, connectStrategyFactory, reestablishStrategyFactory, null, slf);
    }

    @Override
    public Future<Void> createReconnectingClient(final InetSocketAddress address, final ReconnectStrategyFactory
        connectStrategyFactory, final ReconnectStrategyFactory reestablishStrategyFactory, final KeyMapping keys, final SessionListenerFactory<BmpSessionListener> slf) {
        this.keys = keys;
