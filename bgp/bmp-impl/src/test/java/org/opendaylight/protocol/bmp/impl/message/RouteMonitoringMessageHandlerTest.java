...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bmp.message.rev150512.peer.header.PeerHeader.AdjRibInType;
...
public class RouteMonitoringMessageHandlerTest {
     ...
     * 02 <- Update message
     */
    private static final byte[] routeMonitMsgData = {
        (byte) 0x03,
    ...
    public void testSerializeRouteMonitMessage() throws BmpDeserializationException  {
        ...
        this.messageRegistry.serializeMessage(createRouteMonitMsg(false), buffer);
        assertArrayEquals(routeMonitMsgData, ByteArray.readAllBytes(buffer));
    }

    @Test
    public void testParseRouteMonitMessage() throws BmpDeserializationException {
        final RouteMonitoringMessage parsedInitMsg = (RouteMonitoringMessage) messageRegistry.parseMessage(Unpooled.copiedBuffer(routeMonitMsgData));
        assertEquals(createRouteMonitMsg(true), parsedInitMsg);
    }

    private static final RouteMonitoringMessage createRouteMonitMsg(final boolean withNormalizedIpv4Prefixes) {
    ...
    private static final Update createUpdate(final boolean withNormalizedIpv4Prefixes) {
    ...
    private static final Nlri createNlriWitNormalizedIpv4Prefixes() {
        final NlriBuilder nlriBuilder = new NlriBuilder()
