...
public class StatisticsReportHandlerTest {
    ...
    private static StatisticsReportHandler statsReportHandler;

    private final byte[] statsRepMsgData = {
        /*
         * 03 <- bmp version
    ...
    public static void init() {
        ...
        statsReportHandler = new StatisticsReportHandler(context.getMessageRegistry());
    }

    @Test
    public void testSerializeStatsReportMessage() throws BMPDeserializationException {
        final ByteBuf buffer = Unpooled.buffer();
        statsReportHandler.serializeMessage(createStatsReportMsg(), buffer);
        final byte[] serializedMsg = new byte[buffer.writerIndex()];
        buffer.readBytes(serializedMsg);
        assertArrayEquals(statsRepMsgData, serializedMsg);
    }

    @Test
    public void testParseStatsReportMessage() throws BMPDeserializationException {
        final ByteBuf dataWithoutHeader = Unpooled.copiedBuffer(statsRepMsgData).skipBytes(InitiationHandlerTest.HEADER_LENGTH);
        final StatsReportsMessage parsedInitMsg = (StatsReportsMessage) statsReportHandler.parseMessageBody(dataWithoutHeader);
    ...
    private static final StatsReportsMessage createStatsReportMsg() {
        final StatsReportsMessageBuilder statsReportMsgBuilder = new StatsReportsMessageBuilder()
            .setAdjRibsInRoutes(new Gauge64(BigInteger.valueOf(10L)))
            .setDuplicatePrefixAdvertisements(new Counter32(16L))
            .setDuplicateWithdraws(new Counter32(11L))
            .setInvalidatedAsConfedLoop(new Counter32(55L))
            .setInvalidatedAsPathLoop(new Counter32(66L))
            .setInvalidatedClusterListLoop(new Counter32(53L))
            .setInvalidatedOriginatorId(new Counter32(70L))
            .setLocRibRoutes(new Gauge64(BigInteger.valueOf(100L)))
            .setPeerHeader(RouteMonitoringMessageHandlerTest.createPeerHeader())
            .setRejectedPrefixes(new Counter32(8L));
        return statsReportMsgBuilder.build();
    }
}
