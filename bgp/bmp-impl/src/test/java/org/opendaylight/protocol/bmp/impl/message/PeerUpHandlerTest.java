...
public class PeerUpHandlerTest {
    ...
    private final byte[] serializedPeerUpNotificationData = {
        /*
         * 03 <- bmp version
    ...
    public void testSerializePeerUpNotification() throws BMPDeserializationException {
        ...
        assertArrayEquals(serializedPeerUpNotificationData, serializedNotif);
    }

    @Ignore
    @Test
    public void testParsePeerUPNotification() throws BMPDeserializationException {
        final ByteBuf dataWithoutHeader = Unpooled.copiedBuffer(serializedPeerUpNotificationData).skipBytes(InitiationHandlerTest.HEADER_LENGTH);
        final PeerUpNotification parsedPeerDownNotif = (PeerUpNotification) peerUpHandler.parseMessageBody(dataWithoutHeader);
