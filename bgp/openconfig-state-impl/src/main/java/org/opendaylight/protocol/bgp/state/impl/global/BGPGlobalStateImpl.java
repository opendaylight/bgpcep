...
public final class BGPGlobalStateImpl implements BGPGlobalState, BGPStateConsumer {
    ...
    private void createAfiSafiCounter(final String ribId, final Class<? extends AfiSafiType> afiSafiType) {
        this.totalPaths.put(afiSafiType, new UnsignedInt32Counter("Total Paths installed on Global " + ribId + " " + "AFI-SAFI"));
        this.totalPrefixes.put(afiSafiType, new UnsignedInt32Counter("Total Prefixes installed on Global " + ribId + " AFI-SAFI"));
    }
    ...
    @Override
    public synchronized void incrementAfiSafiPrefixes(final Class<? extends AfiSafiType> afiSafiType) {
        this.totalPrefixes.get(afiSafiType).incrementCount();
    }

    @Override
    public synchronized void incrementAfiSafiPaths(final Class<? extends AfiSafiType> afiSafiType) {
        this.totalPaths.get(afiSafiType).incrementCount();
    }

    @Override
    public synchronized void decrementAfiSafiPrefixes(final Class<? extends AfiSafiType> afiSafiType) {
        this.totalPrefixes.get(afiSafiType).decrementCount();
    }

    @Override
    public synchronized void decrementAfiSafiPaths(final Class<? extends AfiSafiType> afiSafiType) {
        this.totalPaths.get(afiSafiType).decrementCount();
    }
