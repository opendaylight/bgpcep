...
public final class BGPNeighborStateImpl implements BGPNeighborState {
    ...
    @Override
    public synchronized void increasePrefixesInstalled(final Class<? extends AfiSafiType> afiSafiType) {
        this.prefixesInstalled.get(afiSafiType).increaseCount();
    }

    @Override
    public synchronized void decreasePrefixesInstalled(final Class<? extends AfiSafiType> afiSafiType) {
        this.prefixesInstalled.get(afiSafiType).decreaseCount();
    }

    @Override
    public synchronized void increasePrefixesSent(final Class<? extends AfiSafiType> afiSafiType) {
        this.prefixesSent.get(afiSafiType).increaseCount();
    }

    @Override
    public synchronized void increasePrefixesReceived(final Class<? extends AfiSafiType> afiSafiType, final long numberOfPrefixesInstalled) {
        this.prefixesReceived.get(afiSafiType).increaseCount(numberOfPrefixesInstalled);
    }
