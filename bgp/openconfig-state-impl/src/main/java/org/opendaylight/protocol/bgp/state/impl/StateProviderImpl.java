...
public final class StateProviderImpl implements BGPStateProvider, TransactionChainListener, AutoCloseable {
    ...
    @GuardedBy("this")
    private List<AbstractRegistration> registrations = new ArrayList<>();
    ...
    public synchronized void close() throws Exception {
        ...
        for (final AbstractRegistration registration : this.registrations) {
            registration.close();
        }
    ...
    public synchronized AbstractRegistration registerBGPState(final BGPStateConsumer bgpStateConsumer) {
        ...
        final AbstractRegistration registration = new AbstractRegistration() {
            @Override
            protected void removeRegistration() {
                synchronized (StateProviderImpl.this) {
                    ...
                    wTx.submit();
                    StateProviderImpl.this.registrations.remove(this);
                }
            }
        };
