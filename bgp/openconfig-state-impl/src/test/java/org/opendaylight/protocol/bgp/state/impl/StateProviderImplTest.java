...
public class StateProviderImplTest extends AbstractStateTest {
    ...
    public void testStateProvider() throws ReadFailedException {
        final BGPGlobalStateImpl bgpGlobalState = new BGPGlobalStateImpl(this.afiSafisAdvertized, this.ribId, this.bgpId,
            this.as, this.bgpInstanceIdentifier);
        ...
        bgpGlobalState.increaseAfiSafiPaths(IPV4UNICAST.class);
        bgpGlobalState.increaseAfiSafiPrefixes(IPV4UNICAST.class);
        ...
        bgpGlobalState.decreaseAfiSafiPaths(IPV4UNICAST.class);
        bgpGlobalState.decreaseAfiSafiPrefixes(IPV4UNICAST.class);
        ...
        neighborState.setAfiSafiGracefulRestartState(this.restartTime, this.peerRestarting, this.localRestarting,
            BgpAfiSafiGracefulRestartState.Mode.BILATERAL);
        neighborState.setCapabilities(this.holdTimer, this.localPort, this.remote, this.remotePort, true, true, true,
            true, true);
        neighborState.setState(BgpNeighborState.SessionState.ESTABLISHED);
        neighborState.increaseErroneousUpdateReceived();
        neighborState.increaseNotificationSent();
        neighborState.increaseUpdateReceived();
        neighborState.increaseUpdateSent();
        neighborState.increaseNotificationReceived();
        neighborState.increasePrefixesInstalled(IPV4UNICAST.class);
        neighborState.increasePrefixesReceived(IPV4UNICAST.class, 2L);
        neighborState.increasePrefixesSent(IPV4UNICAST.class);
        ...
        ReadDataBrokerUtil.readData(getDataBroker(), this.bgpInstanceIdentifier, bgpRib -> {
            ...
            Assert.assertNotNull(neighbors);
            final Neighbor neighbor = neighbors.getNeighbor().get(0);
            final Long timerTicks = neighbor.getTimers().getState().getAugmentation(NeighborTimersStateAugmentation.class).getUptime()
                .getValue();
