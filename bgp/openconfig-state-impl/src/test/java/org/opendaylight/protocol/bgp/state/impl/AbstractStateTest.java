...
abstract class AbstractStateTest extends AbstractDataBrokerTest {
    ...
    final int holdTimer = 10;
    final int localPort = 1790;
    final int remotePort = 179;
    final int restartTime = 15;
    final boolean peerRestarting = true;
    final boolean localRestarting = true;
    ...
    Neighbor buildNeighborExpected(final IpAddress neighborAddress, final boolean afiSafiActive,
        ...
        if (afiSafiActive) {
            errorMessages = 1L;
            gracefulAugmentation.setLocalRestarting(this.localRestarting)
                .setMode(BgpAfiSafiGracefulRestartState.Mode.BILATERAL)
                .setPeerRestarting(this.peerRestarting).setPeerRestartTime(this.restartTime);
        ...
        final Transport transport = new TransportBuilder().setState(new org.opendaylight.yang.gen.v1.http.openconfig
            ...
            .addAugmentation(NeighborTransportStateAugmentation.class,
                new NeighborTransportStateAugmentationBuilder().setLocalPort(new PortNumber(localPort))
                    .setRemotePort(new PortNumber(remotePort))
