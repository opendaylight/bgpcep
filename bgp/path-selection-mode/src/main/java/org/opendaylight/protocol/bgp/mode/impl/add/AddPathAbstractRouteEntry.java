...
public abstract class AddPathAbstractRouteEntry<C extends Routes & DataObject & ChoiceIn<Tables>,
    ...
    public final int addRoute(final RouterId routerId, final Uint32 remotePathId, final MapEntryNode route) {
        ...
        if (offset < 0) {
            final RouteKeyOffsets newOffsets = offsets.with(key);
            offset = newOffsets.offsetOf(key);
            final MapEntryNode[] newRoute = newOffsets.expand(offsets, values, offset);
            final Uint32[] newPathsId = newOffsets.expand(offsets, pathsId, offset);
            values = newRoute;
            offsets = newOffsets;
            pathsId = newPathsId;
