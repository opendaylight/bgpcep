...
public abstract class AddPathAbstractRouteEntry extends AbstractRouteEntry<AddPathBestPath> {
    ...
    @Override
    public void initializeBestPaths(final RouteEntryDependenciesContainer entryDependencies, final PeerId destPeer,
            final NodeIdentifierWithPredicates routeId, final YangInstanceIdentifier rootPath,
        ...
        if (this.bestPath != null) {
            ...
            this.bestPath.stream().filter(path -> filterRoutes(path.getPeerId(), destPeer, peerPT, localTk,
                    ...
                    .forEach(path -> writeRoutePath(destPeer, routeId, peerPT, peerGroup, destPeerSupAddPath,
                            path, rootPath, localTk, ribSup, tx));
        }
    }
    ...
    private void writeRoutePath(final PeerId destPeer, final NodeIdentifierWithPredicates routeId,
        ...
        final ContainerNode effectiveAttributes
                = peerGroup.effectiveAttributes(getRoutePeerIdRole(peerPT, path.getPeerId()), path.getAttributes());
