...
abstract class BaseAbstractRouteEntry implements RouteEntry {
    ...
    @Override
    public void updateRoute(final DOMDataWriteTransaction tx, final RouteUpdateKey routeUpdateKey) {
        if (this.removedBestPath.isPresent()) {
            removePathFromDataStore(this.removedBestPath.get(), routeUpdateKey, tx);
            this.removedBestPath = Optional.empty();
        }

        if (this.bestPath.isPresent()) {
            addPathToDataStore(this.bestPath.get(), routeUpdateKey, tx);
        }
    }
    ...
    private void removePathFromDataStore(final BestPath path, final RouteUpdateKey routeUpdateKey, final DOMDataWriteTransaction tx) {
        LOG.trace("Best Path removed {}", path);
        fillLocRib(tx, null);
        fillAdjRibsOut(tx, null, null, routeUpdateKey.getRouteId(), path.getPeerId());
    }

    private void addPathToDataStore(final BestPath path, final RouteUpdateKey routeUpdateKey, final DOMDataWriteTransaction tx) {
        final NormalizedNode<?, ?> value = createValue(routeUpdateKey.getRouteId(), path);
        LOG.trace("Selected best value {}", value);
        fillLocRib(tx, value);
        fillAdjRibsOut(tx, path.getAttributes(), value, routeUpdateKey.getRouteId(), path.getPeerId());
    }
