...
public interface RouteEntry {
     ...
     *
     * <p>Unlike {@link #addRoute(RouterId, Uint32, MapEntryNode)}, this method signals that the route node was
     * reported as {@code SUBTREE_MODIFIED} and therefore needs to be re-advertised if it remains a best path, even
     * when its path-selection attributes are unchanged.
     *
     * @param routerId     router ID in unsigned integer format from an Ipv4Address
     * @param remotePathId remote path Id received
