...
public final class BgpConditionsRegistry {
    ...
    private boolean matchNextHopIn(final CNextHop cNextHop, final List<IpAddress> nextHopIn) {
        if (cNextHop == null || nextHopIn == null || cNextHop instanceof EmptyNextHopCase) {
            return true;
        }

        IpAddress global;
        if (cNextHop instanceof Ipv4NextHopCase) {
            global = new IpAddress(((Ipv4NextHopCase) cNextHop).getIpv4NextHop().getGlobal());
        } else {
            global = new IpAddress(((Ipv6NextHopCase) cNextHop).getIpv6NextHop().getGlobal());
        }
