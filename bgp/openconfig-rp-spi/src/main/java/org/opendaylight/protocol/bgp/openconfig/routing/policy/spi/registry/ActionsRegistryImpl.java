...
final class ActionsRegistryImpl {
    @GuardedBy("this")
    private final Map<Class<? extends Augmentation<IgpActions>>, IgpActionsPolicy> igpActionsRegistry = new HashMap<>();
    @GuardedBy("this")
    private final Map<Class<? extends Augmentation<Actions>>, ActionsPolicy> actionsRegistry = new HashMap<>();
    @GuardedBy("this")
    ...
    AbstractRegistration registerActionPolicy(
        ...
        synchronized (this.actionsRegistry) {
            final ActionsPolicy prev = this.actionsRegistry.putIfAbsent(actionPolicyClass, actionPolicy);
    ...
    AbstractRegistration registerIGPActionPolicy(final Class<? extends Augmentation<IgpActions>> igpActionPolicyClass,
        ...
        synchronized (this.igpActionsRegistry) {
            final IgpActionsPolicy prev = this.igpActionsRegistry.putIfAbsent(igpActionPolicyClass, igpActionPolicy);
            Preconditions.checkState(prev == null, "Action Policy %s already registered %s",
    ...
    Attributes applyExportAction(
        ...
        attributesUpdated = applyExportIGPActions(routeEntryInfo, routeEntryExportParameters,
                attributesUpdated, actions.getIgpActions());
    ...
    private Attributes applyExportAugActions(
        ...
        for (final Map.Entry<Class<? extends Augmentation<?>>, Augmentation<?>> entry : conditionsAug.entrySet()) {
            final ActionsPolicy handler = this.actionsRegistry.get(entry.getKey());
            if (attributesUpdated == null) {
                return null;
    ...
    private Attributes applyExportBGPActions(
        ...
        final SetCommunity setCommunityAction =  bgpActions.getSetCommunity();
        final SetExtCommunity setExtCommunityAction =  bgpActions.getSetExtCommunity();
    ...
    @SuppressWarnings("unchecked")
    private Attributes applyExportIGPActions(
            final RouteEntryBaseAttributes routeEntryInfo,
            final BGPRouteEntryExportParameters routeEntryExportParameters,
            @Nonnull final Attributes attributes,
        ...
        if (conditionsAug != null) {
            for (final Map.Entry<Class<? extends Augmentation<?>>, Augmentation<?>> entry : conditionsAug.entrySet()) {
                final IgpActionsPolicy handler = this.igpActionsRegistry.get(entry.getKey());
                if (handler == null) {
                    continue;
                } else if (attributesUpdated == null) {
                    return null;
    ...
    Attributes applyImportAction(
        ...
        Attributes attributesUpdated = attributes;
        attributesUpdated = applyImportIGPActions(routeEntryInfo, routeBaseParameters, attributesUpdated,
    ...
    @SuppressWarnings("unchecked")
    private Attributes applyImportIGPActions(
            final RouteEntryBaseAttributes routeEntryInfo,
            final BGPRouteEntryImportParameters routeBaseParameters,
            @Nonnull final Attributes attributes,
            final IgpActions igpActions) {
