...
public abstract class AbstractBmpPerPeerMessageParser extends AbstractBmpMessageParser {
    ...
    private void serializePerPeerHeader(final PeerHeader peerHeader, final ByteBuf output) {
        Preconditions.checkArgument(peerHeader != null, "Per-peer header cannot be null.");
        ByteBufWriteUtil.writeUnsignedByte((short) peerHeader.getType().getIntValue(), output);
