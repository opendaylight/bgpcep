...
import org.opendaylight.protocol.bmp.parser.BMPParsingException;
...
public class SimpleBmpMessageRegistry implements BmpMessageRegistry {
    ...
    @Override
    public Notification parseMessage(final ByteBuf buffer) throws BMPDeserializationException, BMPParsingException {
    ...
    @Override
    public void serializeMessage(final Notification message, final ByteBuf buffer) throws BMPDeserializationException {
    ...
    private int parseMessageHeader(final ByteBuf buffer) throws BMPParsingException {
        ...
        Preconditions.checkState(messageVersion == BMP_VERSION);
        ...
        if (messageLength - COMMON_HEADER_LENGTH != buffer.readableBytes()) {
            throw new BMPParsingException("Size doesn't match size specified in header. Passed: "
                + buffer.readableBytes() + "; Expected: " + (messageLength - COMMON_HEADER_LENGTH) + ". ");
        }
        return msgType;
    }
