...
public interface RIBSupport<C extends Routes & DataObject & ChoiceIn<Tables>, S extends ChildOf<? super C>> {
     ...
     *
     * <p>An in-place modification is one whose list key is unchanged. A withdrawal is required for AFI/SAFIs whose
     * on-wire NLRI identity is derived from route content beyond the route-key, notably BGP Labeled Unicast, where
     * the label stack is part of the NLRI. For such routes, replacing the content while keeping the route-key stable
     * (e.g. an application-rib route updated with a new label stack) changes the advertised NLRI, so the superseded
     * NLRI must be withdrawn or the peer would retain it as a separate route.
     *
