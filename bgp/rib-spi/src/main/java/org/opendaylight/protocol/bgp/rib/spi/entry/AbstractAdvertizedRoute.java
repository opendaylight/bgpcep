...
public abstract class AbstractAdvertizedRoute<C extends Routes & DataObject & ChoiceIn<Tables>,
     ...
     * update is fanned out to. Build it once and reuse it for equal attributes, so all peers' AdjRibsOut tables
     * share the same node instances instead of holding per-peer copies, which would multiply heap usage by the
     * number of peers.
     *
