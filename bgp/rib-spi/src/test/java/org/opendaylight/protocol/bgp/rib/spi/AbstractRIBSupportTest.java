...
public abstract class AbstractRIBSupportTest {
    ...
    protected final ArrayList<YangInstanceIdentifier> routesYiiPutList;
    protected final ArrayList<NormalizedNode<?, ?>> routesPutList;
    protected final ArrayList<YangInstanceIdentifier> routesYiiDelList;
    protected final BindingToNormalizedNodeCodec mappingService;
    protected final RIBSupport ribSupport;

    @Mock
    protected DOMDataWriteTransaction tx;

    protected AbstractRIBSupportTest(final RIBSupport ribSupport) throws Exception {
     ...
     * @return Route Key
     */
    protected abstract Object routeKey();

    /**
     * Build Reach Destination
     */
    protected abstract DestinationType createDestReach();

    /**
     * Build Unreach Destination
     */
    protected abstract DestinationType createDestUnreach();
     ...
     */
    protected AttributesBuilder createAttributes(final CNextHop hop) {
        ...
        final MpReachNlri reach = buildReach(createDestReach(), hop);
        final MpUnreachNlri unreach = buildUnreach(createDestUnreach());
    ...
    protected final ContainerNode createNlriWithDrawnRoute() {
        final InstanceIdentifier TABLES_IID = InstanceIdentifier.create(Update.class).child(Attributes.class);
        final InstanceIdentifier<MpUnreachNlri> routesIId = TABLES_IID.child(Attributes2.class).child(MpUnreachNlri.class);
        final MpUnreachNlri mpReach = new MpUnreachNlriBuilder().setWithdrawnRoutes(new WithdrawnRoutesBuilder().setDestinationType(createDestUnreach()).build()).build();

        final Map.Entry result = this.mappingService.toNormalizedNode(routesIId, mpReach);
        return (ContainerNode) result.getValue();
    }

    protected final ContainerNode createNlriAdvertiseRoute() {
        final InstanceIdentifier TABLES_IID = InstanceIdentifier.create(Update.class).child(Attributes.class);
        final InstanceIdentifier<MpReachNlri> routesIId = TABLES_IID.child(Attributes1.class).child(MpReachNlri.class);
        final MpReachNlri mpReach = new MpReachNlriBuilder().setAdvertizedRoutes(new AdvertizedRoutesBuilder().setDestinationType(createDestReach()).build()).build();
    ...
    protected InstanceIdentifier getKeyIId() {
    ...
    protected final <T extends DataObject> void checkEmptyRoute(final InstanceIdentifier<T> path, final T data) {
        final Map.Entry expected = this.mappingService.toNormalizedNode(path, data);
        final NormalizedNode<?, ?> temp = this.routesPutList.get(0);
        final SingletonSet emptyRoutes = (SingletonSet) temp.getValue();
