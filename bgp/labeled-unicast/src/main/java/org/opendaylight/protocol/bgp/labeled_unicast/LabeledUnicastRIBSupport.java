...
final class LabeledUnicastRIBSupport extends AbstractRIBSupport {
    ...
    private void processDestination(final DOMDataWriteTransaction tx, final YangInstanceIdentifier routesPath,
        ...
        LOG.debug("DOMDataWriteTransaction {}", tx);
        LOG.debug("YangInstanceIdentifier routesPath {}", routesPath);
        LOG.debug("ContainerNode destination {}", destination);
        LOG.debug("ContainerNode attributes {}", attributes);
        LOG.debug("ApplyRoute function {}", function);
        if (destination != null) {
            final Optional<DataContainerChild<? extends PathArgument, ?>> maybeRoutes = destination.getChild(this.nlriRoutesList);
            LOG.debug("labeled unicast destination {}", destination.getChild(this.nlriRoutesList));
            if (maybeRoutes.isPresent()) {
                LOG.debug("routes {}",maybeRoutes.get());
                final DataContainerChild<? extends PathArgument, ?> routes = maybeRoutes.get();
                if (routes instanceof UnkeyedListNode) {
                    LOG.debug("route {}",routesPath.node(routesContainerIdentifier()).node(this.route));
