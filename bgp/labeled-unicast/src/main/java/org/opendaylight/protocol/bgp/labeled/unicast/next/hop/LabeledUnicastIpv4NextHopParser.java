...
public class LabeledUnicastIpv4NextHopParser implements NextHopParser, NextHopSerializer {
    @Override
    public CNextHop parseNextHop(final ByteBuf buffer) throws BGPParsingException {
        ...
        return new LabeledUnicastIpv4NextHopCaseBuilder().setIpv4NextHop(new Ipv4NextHopBuilder().setGlobal(Ipv4Util
            .addressForByteBuf(buffer)).build()).build();
    }

    @Override
    public void serializeNextHop(final CNextHop cNextHop, final ByteBuf byteAggregator) {
        Preconditions.checkArgument(cNextHop instanceof LabeledUnicastIpv4NextHopCase, "cNextHop is not a Link " +
            "State NextHop Case object.");
        final Ipv4Address nextHop = ((LabeledUnicastIpv4NextHopCase) cNextHop).getIpv4NextHop().getGlobal();
