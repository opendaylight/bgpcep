...
final class LabeledUnicastRIBSupport extends AbstractRIBSupport {
    ...
    private MpReachNlriBuilder setReachAfi(CLabeledUnicastDestination sampleDest){
        final MpReachNlriBuilder builder = new MpReachNlriBuilder();
        if(sampleDest.getPrefixType().getIpv4Prefix() != null){
            builder.setAfi(Ipv4AddressFamily.class);
        } else {
            builder.setAfi(Ipv6AddressFamily.class);
        }
        return builder;
    }

    private MpUnreachNlriBuilder setUnreachAfi(CLabeledUnicastDestination sampleDest){
    ...
    private NodeIdentifierWithPredicates createRouteKey(final UnkeyedListEntryNode labeledUnicast) {
        ...
        final CLabeledUnicastDestination dest = LUNlriParser.extractCLabeledUnicastDestination(labeledUnicast);
        dests.add(dest);
        final MpReachNlriBuilder builder = new MpReachNlriBuilder();
        if (dests.size() >= 1) {
            final CLabeledUnicastDestination sampleDest = dests.get(0);
            setReachAfi(sampleDest);
        }
        LUNlriParser.serializeNlri(dests, buffer, builder.getAfi());
