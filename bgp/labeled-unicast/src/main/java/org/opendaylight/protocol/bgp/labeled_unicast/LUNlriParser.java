...
public class LUNlriParser implements NlriParser, NlriSerializer{
    ...
    static final NodeIdentifier LV_NID = new NodeIdentifier(QName.create("urn:opendaylight:params:xml:ns:yang:bgp-labeled-unicast","2015-08-11","label-value"));
    ...
    public static CLabeledUnicastDestination extractCLabeledUnicastDestination(final DataContainerNode<? extends PathArgument> route) {
        final CLabeledUnicastDestinationBuilder builder = new CLabeledUnicastDestinationBuilder();
        final Optional<DataContainerChild<? extends PathArgument, ?>> prefixType = route.getChild(PREFIX_TYPE_NID);
        if(prefixType.isPresent()){
            builder.setPrefixType((IpPrefix)prefixType.get().getValue());
        }
