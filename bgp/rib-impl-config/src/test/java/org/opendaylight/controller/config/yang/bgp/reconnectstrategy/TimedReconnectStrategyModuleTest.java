...
public class TimedReconnectStrategyModuleTest extends AbstractConfigTest {
	...
	public void testValidationException1()
		...
		try {
			ConfigTransactionJMXClient transaction = configRegistryClient
					.createTransaction();
			createInstance(transaction, instanceName, 500, 100L, null, 500L, 10L, 10000L);
	...
	public void testValidationException2()
		...
		try {
			ConfigTransactionJMXClient transaction = configRegistryClient
					.createTransaction();
			createInstance(transaction, instanceName, 500, 100L, new BigDecimal(0.5), 500L, 10L, 10000L);
			transaction.validateConfig();
			fail();
		} catch (ValidationException e) {
	...
	public void testValidationException3()
		...
		try {
			ConfigTransactionJMXClient transaction = configRegistryClient
					.createTransaction();
			createInstance(transaction, instanceName, null, 100L, new BigDecimal(1.0), 500L, 10L, 10000L);
			transaction.validateConfig();
			fail();
		} catch (ValidationException e) {
	...
	public void testValidationException4()
		...
		try {
			ConfigTransactionJMXClient transaction = configRegistryClient
					.createTransaction();
			createInstance(transaction, instanceName, -1, 100L, new BigDecimal(1.0), 500L, 10L, 10000L);
			transaction.validateConfig();
			fail();
		} catch (ValidationException e) {
	...
	public void testValidationException6()
		...
		try {
			ConfigTransactionJMXClient transaction = configRegistryClient
					.createTransaction();
			createInstance(transaction, instanceName, 100, 300L, new BigDecimal(1.0), 100L, 10L, 10000L);
			transaction.validateConfig();
			fail();
		} catch (ValidationException e) {
			assertTrue(e.getMessage().contains("value 300 is greter than MaxSleep 100"));
		}
	}
	...
	public void testCreateBean() throws Exception {
		ConfigTransactionJMXClient transaction = configRegistryClient
				.createTransaction();
		createInstance(transaction, instanceName, 500, 100L, new BigDecimal(1.0), 500L, 10L, 10000L);
		transaction.validateConfig();
	...
	public void testReconfigure() throws InstanceAlreadyExistsException,
		...
		ConfigTransactionJMXClient transaction = configRegistryClient
				.createTransaction();
		createInstance(transaction, instanceName, 500, 100L, new BigDecimal(1.0), new Long(500), new Long(10), new Long(10000));
		transaction.commit();
		transaction = configRegistryClient.createTransaction();
		assertBeanCount(1, factory.getImplementationName());
	...
	private ObjectName createInstance(final ConfigTransactionJMXClient transaction,
			...
			final Long minSleep, final BigDecimal sleepFactor, final Long maxSleep,
