...
import org.opendaylight.yang.gen.v1.http.openconfig.net.yang.bgp.rev151009.State3;
import org.opendaylight.yang.gen.v1.http.openconfig.net.yang.bgp.rev151009.State3Builder;
...
public final class NeighborStateUtil {
    ...
    public static List<Neighbor> buildNeighbors(@Nonnull final List<BGPNeighborState> neighbors) {
        final List<Neighbor> neighborStates = neighbors.stream().map(NeighborStateUtil::buildNeighbor)
    ...
    public static List<AfiSafi> buildAfisSafisState(
        ...
        return afiSafisAdvertized.stream().map(
            announced -> buildAfiSafi(announced, prefixesInstalled, prefixesReceived, prefixesSent,
            afiSafisReceived.contains(announced), afiSafisGracefulAdvertized.contains(announced),
            afiSafisGracefulReceived.contains(announced))).collect(Collectors.toList());
    }

    private static AfiSafi buildAfiSafi(final Class<? extends AfiSafiType> afiSafiAdvertized,
        ...
        final boolean afiSafiSupported, final boolean gracefulRestartAdvertized,
        final boolean gracefulRestartReceived) {

        return new AfiSafiBuilder()
            .setAfiSafiName(afiSafiAdvertized)
            .setState(buildAfiSafiState(afiSafiAdvertized, prefixesInstalled, prefixesReceived, prefixesSent,
                afiSafiSupported))
    ...
    private static org.opendaylight.yang.gen.v1.http.openconfig.net.yang.bgp.multiprotocol.rev151009.bgp.common.afi.
        ...
        final State3Builder builder = new State3Builder();
        ...
        return new org.opendaylight.yang.gen.v1.http.openconfig.net.yang.bgp.multiprotocol.rev151009.bgp.common.afi
            .safi.list.afi.safi.StateBuilder().addAugmentation(State3.class, builder.build()).build();
    }
