            ...
            if (value[ISO_SYSTEM_ID_LENGTH] == 0) {
                ...
                return new IsisPseudonodeCaseBuilder().setIsisPseudonode(
                        new IsisPseudonodeBuilder().setIsIsRouterIdentifier(iri).setPsn(
                                (short) UnsignedBytes.toInt(value[ISO_SYSTEM_ID_LENGTH])).build()
                ).build();
            }
        }
        if (value.length == 4) {
    ...
    public void serializeAttribute(DataObject attribute, ByteBuf byteAggregator) {
        ...
        if (pathAttributes.getAugmentation(PathAttributes1.class) != null) {
            ...
            if (pathAttributes1.getMpReachNlri().getAdvertizedRoutes() != null &&
                ...
                org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev131125.update.path.attributes.mp.reach.nlri.advertized.routes.destination.type.DestinationLinkstateCase
                        linkstateCase = (org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev131125.update.path.attributes.mp.reach.nlri.advertized.routes.destination.type.DestinationLinkstateCase) pathAttributes1.getMpReachNlri().getAdvertizedRoutes().getDestinationType();
