...
public final class PrefixNlriParser implements NlriTypeCaseParser, NlriTypeCaseSerializer {
    ...
    @Override
    public ObjectType parseTypeNlri(ByteBuf nlri, NlriType type, org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev150210.NodeIdentifier localdescriptor)
        ...
        if (type.equals(NlriType.Ipv4Prefix)) {
            prefdesc = parsePrefixDescriptors(nlri, true);
        }

        else if (type.equals(NlriType.Ipv6Prefix)) {
            prefdesc = parsePrefixDescriptors(nlri, false);
