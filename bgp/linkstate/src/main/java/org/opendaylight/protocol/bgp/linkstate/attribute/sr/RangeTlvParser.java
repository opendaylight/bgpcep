...
public class RangeTlvParser {
    ...
    public static SrRange parseSrRange(final ByteBuf buffer, final ProtocolId protocol) {
        ...
        range.setSubTlvs(parseRangeSubTlvs(buffer, protocol));
        return null;
    }

    private static List<SubTlvs> parseRangeSubTlvs(final ByteBuf buffer, final ProtocolId protocol) {
        ...
        while (buffer.isReadable()) {
            ...
            switch (type) {
                case PrefixAttributesParser.PREFIX_SID:
                    subTlvCase = new PrefixSidTlvCaseBuilder(SrPrefixAttributesParser.parseSrPrefix(buffer.readSlice(length), protocol)).build();
                    break;
                case PrefixAttributesParser.BINDING_SID:
                    subTlvCase = new BindingSidTlvCaseBuilder(BindingSidLabelParser.parseBindingSidLabel(buffer.readSlice(length), protocol)).build();
                    break;
