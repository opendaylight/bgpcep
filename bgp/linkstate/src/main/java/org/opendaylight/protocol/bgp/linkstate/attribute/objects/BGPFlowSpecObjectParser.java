...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.te.lsp.rev150706.flow.spec.object.FlowSpecObjectBuilder;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.ieee754.rev130819.Float32;

public final class BGPFlowSpecObjectParser extends AbstractBGPObjectParser {
    ...
    protected TeLspObject localParseObject(final ByteBuf byteBuf) throws BGPParsingException {
        ...
        byteBuf.readInt();
        final short serviceHeader = byteBuf.readUnsignedByte();
        builder.setServiceHeader(serviceHeader);
        //skip reserved
        byteBuf.readByte();
    ...
    public void localSerializeObject(final TeLspObject teLspObject, final ByteBuf output) {
        ...
        final int sHeader = flowObj.getServiceHeader();
        if (sHeader == 2) {
            serializeAttributeHeader(BODY_SIZE_REQUESTING, flowObj.getClassNum(), flowObj.getCType(), output);
            output.writeZero(SHORT_SIZE);
            output.writeShort(REQUESTING_OVERALL_LENGTH);
        } else {
            serializeAttributeHeader(BODY_SIZE_CONTROLLED, flowObj.getClassNum(), flowObj.getCType(), output);
            output.writeZero(SHORT_SIZE);
        ...
        writeFloat32(flowObj.getTokenBucketRate(), output);
        writeFloat32(flowObj.getTokenBucketSize(), output);
        writeFloat32(flowObj.getPeakDataRate(), output);
        writeUnsignedInt(flowObj.getMinimumPolicedUnit(), output);
        writeUnsignedInt(flowObj.getMaximumPacketSize(), output);
