...
public final class NodeNlriParser {
    ...
    private static IsisNodeCase serializeIsisNode(final ContainerNode isis) {
        ...
        if (isis.getChild(ISIS_ROUTER_NID).isPresent()) {
            final ContainerNode isisRouterNid = (ContainerNode) isis.getChild(ISIS_ROUTER_NID).get();
            if (isisRouterNid.getChild(ISO_SYSTEM_NID).isPresent()) {
                isisBuilder.setIsoSystemId(new IsoSystemIdentifier((byte[]) isisRouterNid.getChild(ISO_SYSTEM_NID).get().getValue()));
            }
        }
        return new IsisNodeCaseBuilder().setIsisNode(isisBuilder.build()).build();
    }
