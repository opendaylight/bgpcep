		...
		for (final Entry<Short, ByteList> entry : attributes.entrySet()) {
			...
			for (final byte[] value : entry.getValue().getBytes()) {
				...
				case TlvCode.FORWARDING_ADDRESS:
					Ipv4Address fwdAddress = null;
					if (value.length == Ipv4Util.IP4_LENGTH) {
						fwdAddress = new Ipv4Address(Ipv4Util.addressForBytes(value));
					} else {
						LOG.info("Ignoring unsupported forwarding address length {}", value.length);
					}
