...
public final class LinkNlriParser extends AbstractNlriTypeCodec {
    ...
    protected ObjectType parseObjectType(final ByteBuf buffer) {
        final LocalNodeDescriptors localDescriptor = SimpleNlriTypeRegistry.getInstance().parseTlv(buffer);
        final RemoteNodeDescriptors remoteDescriptor = SimpleNlriTypeRegistry.getInstance().parseTlv(buffer);
    ...
    protected void serializeObjectType(final ObjectType objectType, final ByteBuf buffer) {
        ...
        SimpleNlriTypeRegistry.getInstance().serializeTlv(LocalNodeDescriptors.QNAME, linkNlri.getLocalNodeDescriptors(), buffer);
        SimpleNlriTypeRegistry.getInstance().serializeTlv(RemoteNodeDescriptors.QNAME, linkNlri.getRemoteNodeDescriptors(), buffer);
    ...
    private void serializeLinkDescriptor(final LinkDescriptors linkDescriptor, final ByteBuf body) {
        if (linkDescriptor.getLinkLocalIdentifier() != null && linkDescriptor.getLinkRemoteIdentifier() != null) {
            SimpleNlriTypeRegistry.getInstance().serializeTlv(LinkLrIdentifiers.QNAME, linkDescriptor, body);
        }
        SimpleNlriTypeRegistry.getInstance().serializeTlv(Ipv4InterfaceTlvParser.IPV4_IFACE_ADDRESS_QNAME, linkDescriptor.getIpv4InterfaceAddress(), body);
        SimpleNlriTypeRegistry.getInstance().serializeTlv(Ipv4NeighborTlvParser.IPV4_NEIGHBOR_ADDRESS_QNAME, linkDescriptor.getIpv4NeighborAddress(), body);
        SimpleNlriTypeRegistry.getInstance().serializeTlv(Ipv6InterfaceTlvParser.IPV6_IFACE_ADDRESS_QNAME, linkDescriptor.getIpv6InterfaceAddress(), body);
        SimpleNlriTypeRegistry.getInstance().serializeTlv(Ipv6NeighborTlvParser.IPV6_NEIGHBOR_ADDRESS_QNAME, linkDescriptor.getIpv6NeighborAddress(), body);
        SimpleNlriTypeRegistry.getInstance().serializeTlv(MultiTopoIdTlvParser.MULTI_TOPOLOGY_ID_QNAME, linkDescriptor.getMultiTopologyId(), body);
    }
