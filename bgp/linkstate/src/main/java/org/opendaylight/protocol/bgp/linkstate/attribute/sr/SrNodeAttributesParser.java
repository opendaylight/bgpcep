...
public final class SrNodeAttributesParser {
    ...
    private static boolean parseLoose(final byte b) {
    ...
    private static List<SubTlvs> parseSidSubtlvs(final ByteBuf buffer) {
        ...
        while (buffer.isReadable()) {
            ...
            case ERO_METRIC:
                ...
                break;
            case ERO_IPV4:
                final Ip4EroCaseBuilder ero4 = new Ip4EroCaseBuilder().setLoose(parseLoose(value.readByte()));
                ero4.setAddress(new IpAddress(Ipv4Util.addressForByteBuf(value)));
                sub = ero4.build();
                break;
            case ERO_IPV6:
                final Ipv6EroCaseBuilder ero6 = new Ipv6EroCaseBuilder().setLoose(parseLoose(value.readByte()));
                ero6.setAddress(new IpAddress(Ipv6Util.addressForByteBuf(value)));
                sub = ero6.build();
                break;
            case UNNUMBERED_ERO:
                final UnnumberedEroCaseBuilder un = new UnnumberedEroCaseBuilder().setLoose(parseLoose(value.readByte()));
                ...
                break;
            case BACKUP_IPV4:
                final Ip4EroBackupCaseBuilder erob4 = new Ip4EroBackupCaseBuilder().setLoose(parseLoose(value.readByte()));
                erob4.setAddress(new IpAddress(Ipv4Util.addressForByteBuf(value)));
                sub = erob4.build();
                break;
            case BACKUP_IPV6:
                final Ipv6EroBackupCaseBuilder erob6 = new Ipv6EroBackupCaseBuilder().setLoose(parseLoose(value.readByte()));
                erob6.setAddress(new IpAddress(Ipv6Util.addressForByteBuf(value)));
                sub = erob6.build();
                break;
            case UNNUMBERED_BACKUP_ERO:
                final UnnumberedEroBackupCaseBuilder unb = new UnnumberedEroBackupCaseBuilder().setLoose(parseLoose(value.readByte()));
                if (value.readableBytes() == UNNUMBERED_4_SIZE) {
