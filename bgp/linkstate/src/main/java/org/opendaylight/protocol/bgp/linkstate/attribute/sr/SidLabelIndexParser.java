...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.network.concepts.rev131125.LocalLabel;
...
public final class SidLabelIndexParser {
    ...
    static SidLabelIndex parseSidLabelIndex(final Size length, final ByteBuf buffer) {
        switch (length) {
        case LABEL:
            return new LocalLabelCaseBuilder().setLocalLabel(new LocalLabel((long) buffer.readUnsignedMedium() >> OFFSET)).build();
        case SID:
