...
public final class LinkAttributesParser {
    ...
    private static final int ADJ_TYPE = 31;
    ...
    static LinkStateAttribute parseLinkAttributes(final Multimap<Integer, ByteBuf> attributes) {
        ...
        for (final Entry<Integer, ByteBuf> entry : attributes.entries()) {
            ...
            case LINK_NAME:
                ...
                break;
            case SR_ADJ_ID:
                value.readShort(); // reads Type and Length of Adj-segment-id
    ...
    static void serializeLinkAttributes(final LinkAttributesCase linkAttributesCase, final ByteBuf byteAggregator) {
        ...
        if (linkAttributes.getSrAdjId() != null) {
            TlvUtil.writeTLV(SR_ADJ_ID, writeAdj(SrLinkAttributesParser.serializeAdjacencySegmentIdentifier(linkAttributes.getSrAdjId())), byteAggregator);
        }
        ...
        if (linkAttributes.getPeerSid() != null) {
            TlvUtil.writeTLV(PEER_SID_CODE, writeAdj(SrLinkAttributesParser.serializeAdjacencySegmentIdentifier(linkAttributes.getPeerSid())), byteAggregator);
        }
        if (linkAttributes.getPeerSetSid() != null) {
            TlvUtil.writeTLV(PEER_SET_SID_CODE, writeAdj(SrLinkAttributesParser.serializeAdjacencySegmentIdentifier(linkAttributes.getPeerSetSid())), byteAggregator);
        }
        LOG.trace("Finished serializing Link Attributes");
    }

    private static ByteBuf writeAdj(final ByteBuf adjValue) {
        final ByteBuf adj = Unpooled.buffer();
        adj.writeByte(ADJ_TYPE);
        adj.writeByte(adjValue.writerIndex());
        adj.writeBytes(adjValue);
        return adj;
    }
