...
public final class CrouterIdTlvParser implements LinkstateTlvParser<CRouterIdentifier>, LinkstateTlvSerializer<CRouterIdentifier> {
    ...
    @Override
    public void serializeTlvBody(CRouterIdentifier tlv, ByteBuf body, final QName qName) {
        if (tlv != null) {
            final ByteBuf routerIdBuf = Unpooled.buffer();
            SimpleNlriTypeRegistry.getInstance().serializeRouterId(tlv, routerIdBuf);
            TlvUtil.writeTLV(IGP_ROUTER_ID, routerIdBuf, body);
        }
    }

    @Override
    public CRouterIdentifier parseTlvBody(ByteBuf value) throws BGPParsingException {
