...
abstract class AbstractPrefixNlriParser extends AbstractNlriTypeCodec {
    ...
    private void serializePrefixDescriptor(final PrefixDescriptors tlv, final ByteBuf buffer) {
        SimpleNlriTypeRegistry.getInstance().serializeTlv(MultiTopoIdTlvParser.MULTI_TOPOLOGY_ID_QNAME, tlv.getMultiTopologyId(), buffer);
        SimpleNlriTypeRegistry.getInstance().serializeTlv(OspfRouteTlvParser.OSPF_ROUTE_TYPE_QNAME, tlv.getOspfRouteType(), buffer);
        SimpleNlriTypeRegistry.getInstance().serializeTlv(ReachTlvParser.IP_REACHABILITY_QNAME, tlv.getIpReachabilityInformation(), buffer);
    }
