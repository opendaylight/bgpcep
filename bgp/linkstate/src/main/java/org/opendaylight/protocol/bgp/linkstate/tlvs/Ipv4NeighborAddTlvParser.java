...
public final class Ipv4NeighborAddTlvParser implements LinkstateTlvParser<Ipv4InterfaceIdentifier>, LinkstateTlvSerializer<Ipv4InterfaceIdentifier> {

    private static final Logger LOG = LoggerFactory.getLogger(Ipv4NeighborAddTlvParser.class);

    public static final int IPV4_NEIGHBOR_ADDRESS = 259;
    ...
    public void serializeTlvBody(Ipv4InterfaceIdentifier tlv, ByteBuf body, final QName qName) {
        if (tlv != null) {
            TlvUtil.writeTLV(IPV4_NEIGHBOR_ADDRESS, Ipv4Util.byteBufForAddress(tlv), body);
        }
    }
