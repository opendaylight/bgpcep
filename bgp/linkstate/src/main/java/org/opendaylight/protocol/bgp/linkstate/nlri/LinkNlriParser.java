...
public final class LinkNlriParser implements NlriTypeCaseParser, NlriTypeCaseSerializer {
    ...
    @Override
    public ObjectType parseTypeNlri(final ByteBuf buffer, final NlriType type, final org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.linkstate.rev150210.NodeIdentifier localdescriptor) throws BGPParsingException {

        final int nodetype = buffer.readUnsignedShort();
        final int length = buffer.readUnsignedShort();

        RemoteNodeDescriptors remoteDescriptors = null;

        if (nodetype == REMOTE_NODE_DESCRIPTORS_TYPE) {
            remoteDescriptors = (RemoteNodeDescriptors) NodeNlriParser.parseNodeDescriptors(buffer.readSlice(length), type, false);
        }
        LinkDescriptors linkdescriptor = parseLinkDescriptors(buffer.slice());
