...
public class BGPBandwidthObjectType1ParserTest {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPBandwidthObjectType1Parser parser = new BGPBandwidthObjectType1Parser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_BANDWIDTH_1, 4, TELspAttributeUtil.TE_LSP_BANDWIDTH_1
                .length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_BANDWIDTH_1, ByteArray.getAllBytes(output));
    }
}