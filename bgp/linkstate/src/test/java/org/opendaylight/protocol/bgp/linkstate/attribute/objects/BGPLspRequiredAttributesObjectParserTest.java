...
public class BGPLspRequiredAttributesObjectParserTest {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPLspRequiredAttributesObjectParser parser = new BGPLspRequiredAttributesObjectParser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_REQUIRED_ATTRIBUTES, 4, TELspAttributeUtil.TE_LSP_REQUIRED_ATTRIBUTES.length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_REQUIRED_ATTRIBUTES, ByteArray.getAllBytes(output));
    }
}