...
public class LinkstateAttributeParserTest {
    ...
    private final LinkstateAttributeParser parser = new LinkstateAttributeParser(false,
        ServiceLoaderBGPExtensionProviderContext.getSingletonInstance().getTeLspRegistry());
    ...
    public void testPositiveTELspAttribute() throws BGPParsingException {
        ...
        this.parser.parseAttribute(Unpooled.copiedBuffer(TELspAttributeUtil.TE_LSP_ATTR), builder);

        final Attributes1 attrs = builder.getAugmentation(Attributes1.class);
        final TeLspAttributes te = ((TeLspAttributesCase) attrs.getLinkStateAttribute()).getTeLspAttributes();
        assertNotNull(te);
        final SenderTspecObject tSpec = te.getSenderTspecObject();
        assertNotNull(tSpec);
        assertEquals(new Short("12"), tSpec.getClassNum());
        ...
        assertEquals(new Float32(new byte[]{(byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x02}), te
            .getSenderTspecObject().getTokenBucketSize());
        ...
        final ByteBuf buff = Unpooled.buffer();
        this.parser.serializeAttribute(builder.build(), buff);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_ATTR,ByteArray.getAllBytes(buff));
        assertTrue(Arrays.equals(TELspAttributeUtil.TE_LSP_ATTR, ByteArray.getAllBytes(buff)));
    }
