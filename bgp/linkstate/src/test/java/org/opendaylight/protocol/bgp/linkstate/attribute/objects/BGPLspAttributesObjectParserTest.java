...
public class BGPLspAttributesObjectParserTest {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPLspAttributesObjectParser parser = new BGPLspAttributesObjectParser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_ATTRIBUTES, 4, TELspAttributeUtil.TE_LSP_ATTRIBUTES.length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_ATTRIBUTES, ByteArray.getAllBytes(output));
    }
}