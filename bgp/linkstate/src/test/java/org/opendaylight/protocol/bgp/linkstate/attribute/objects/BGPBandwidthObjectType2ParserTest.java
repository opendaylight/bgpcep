...
public class BGPBandwidthObjectType2ParserTest extends TestCase {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPBandwidthObjectType2Parser parser = new BGPBandwidthObjectType2Parser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_BANDWIDTH_2, 4, TELspAttributeUtil.TE_LSP_BANDWIDTH_2
                .length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_BANDWIDTH_2, ByteArray.getAllBytes(output));
    }
}