...
public class LinkstateNextHopParserTest {
    ...
    public void testSerializeIpv6NextHopCase() throws BGPParsingException {
        ...
        assertTrue(parsedHop instanceof LinkstateIpv6NextHopCase);
        final Ipv6NextHop nextHopIpv6 = ((LinkstateIpv6NextHopCase) parsedHop).getIpv6NextHop();
        assertEquals(ipv6, nextHopIpv6.getGlobal());
        assertNull(nextHopIpv6.getLinkLocal());
    }

    @Test
    public void testSerializeLinkstateIpv6NextHopCase() throws BGPParsingException {
        hop = new LinkstateIpv6NextHopCaseBuilder().setIpv6NextHop(new org.opendaylight.yang.gen.v1.urn.opendaylight
            .params.xml.ns.yang.bgp.linkstate.rev150210.update.attributes.mp.reach.nlri.c.next.hop.linkstate.ipv6
            .next.hop._case.Ipv6NextHopBuilder().setGlobal(ipv6).setLinkLocal(ipv6l).build()).build();
        buffer.clear();
        linkstateNextHopParser.serializeNextHop(hop, buffer);
        assertArrayEquals(ipv6lB, ByteArray.readAllBytes(buffer));

        final CNextHop parsedHop = linkstateNextHopParser.parseNextHop(Unpooled.wrappedBuffer(ipv6lB));
        assertEquals(hop, parsedHop);
    }
    ...
    @Test
    public void testParseLinkstateIpv6NextHop() throws BGPParsingException {
        hop = linkstateNextHopParser.parseNextHop(Unpooled.wrappedBuffer(NextHopUtilTest.ipv6B));
        assertTrue(hop instanceof LinkstateIpv6NextHopCase);
        final Ipv6NextHop nextHopIpv6 = ((LinkstateIpv6NextHopCase) hop).getIpv6NextHop();
        assertEquals(NextHopUtilTest.ipv6, nextHopIpv6.getGlobal());
        assertNull(nextHopIpv6.getLinkLocal());
    }

    @Test
    public void testParseLinkstateIpv6LinkNextHop() throws BGPParsingException {
        hop = linkstateNextHopParser.parseNextHop(Unpooled.wrappedBuffer(NextHopUtilTest.ipv6lB));
        assertTrue(hop instanceof LinkstateIpv6NextHopCase);
        final Ipv6NextHop nextHopIpv6 = ((LinkstateIpv6NextHopCase) hop).getIpv6NextHop();
        assertEquals(NextHopUtilTest.ipv6, nextHopIpv6.getGlobal());
        assertEquals(NextHopUtilTest.ipv6l, nextHopIpv6.getLinkLocal());
    }*/
}