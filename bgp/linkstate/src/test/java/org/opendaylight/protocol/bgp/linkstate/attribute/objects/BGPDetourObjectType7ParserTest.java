...
public class BGPDetourObjectType7ParserTest{
    @Test
    public void testParser() throws BGPParsingException {
        final BGPDetourObjectType7Parser parser = new BGPDetourObjectType7Parser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_DETOUR7, 4, TELspAttributeUtil.TE_LSP_DETOUR7.length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_DETOUR7, ByteArray.getAllBytes(output));
    }
}