...
public class BGPSessionAttributeObjectType1ParserTest {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPSessionAttributeObjectType1Parser parser = new BGPSessionAttributeObjectType1Parser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_SESSION_C1, 4, TELspAttributeUtil.TE_LSP_SESSION_C1
                .length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_SESSION_C1, ByteArray.getAllBytes(output));
    }
}