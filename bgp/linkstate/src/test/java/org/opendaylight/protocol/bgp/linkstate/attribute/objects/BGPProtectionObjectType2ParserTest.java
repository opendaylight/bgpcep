...
public class BGPProtectionObjectType2ParserTest {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPProtectionObjectType2Parser parser = new BGPProtectionObjectType2Parser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_PROTECTION_C2, 4, TELspAttributeUtil.TE_LSP_PROTECTION_C2.length - 4)));
        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_PROTECTION_C2, ByteArray.getAllBytes(output));
    }
}