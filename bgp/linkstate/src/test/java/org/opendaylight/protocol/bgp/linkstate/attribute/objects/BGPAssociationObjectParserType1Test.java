...
public class BGPAssociationObjectParserType1Test {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPAssociationObjectParserType1 parser = new BGPAssociationObjectParserType1();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_ASSOCIATION_1, 4, TELspAttributeUtil.TE_LSP_ASSOCIATION_1.length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_ASSOCIATION_1, ByteArray.getAllBytes(output));
    }
}