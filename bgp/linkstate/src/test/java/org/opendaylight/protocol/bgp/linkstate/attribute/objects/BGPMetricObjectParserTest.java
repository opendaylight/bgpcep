...
public class BGPMetricObjectParserTest {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPMetricObjectParser parser = new BGPMetricObjectParser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_METRIC, 4, TELspAttributeUtil.TE_LSP_METRIC.length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_METRIC, ByteArray.getAllBytes(output));
    }
}