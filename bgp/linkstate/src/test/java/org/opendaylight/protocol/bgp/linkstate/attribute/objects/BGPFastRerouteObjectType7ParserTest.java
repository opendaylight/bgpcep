...
public class BGPFastRerouteObjectType7ParserTest {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPFastRerouteObjectType7Parser parser = new BGPFastRerouteObjectType7Parser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_FAST_REROUTE7, 4, TELspAttributeUtil.TE_LSP_FAST_REROUTE7.length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_FAST_REROUTE7, ByteArray.getAllBytes(output));
    }
}