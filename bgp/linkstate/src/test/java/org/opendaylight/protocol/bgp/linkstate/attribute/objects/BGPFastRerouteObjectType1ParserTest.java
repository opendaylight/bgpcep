...
public class BGPFastRerouteObjectType1ParserTest {
    @Test
    public void testParser() throws BGPParsingException {
        final BGPFastRerouteObjectType1Parser parser = new BGPFastRerouteObjectType1Parser();
        final TeLspObject obj = parser.parseObject(Unpooled.copiedBuffer(
            ByteArray.subByte(TELspAttributeUtil.TE_LSP_FAST_REROUTE1, 4, TELspAttributeUtil.TE_LSP_FAST_REROUTE1.length - 4)));

        final ByteBuf output = Unpooled.buffer();
        parser.serializeObject(obj, output);
        assertArrayEquals(TELspAttributeUtil.TE_LSP_FAST_REROUTE1, ByteArray.getAllBytes(output));
    }
}