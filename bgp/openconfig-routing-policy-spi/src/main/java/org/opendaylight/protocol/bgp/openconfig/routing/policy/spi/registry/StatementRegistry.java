...
public final class StatementRegistry implements StatementRegistryConsumer, StatementRegistryProvider {
    ...
    @Override
    public ContainerNode applyExportStatement(final OpenconfigPolicyConsumer policyConsumer, final long localAs,
        ...
        final ContainerNode attributes, final Statement statement, boolean conditionSatisfied) {
        ...
        if (!this.conditionsRegistry.matchExportConditions(policyConsumer, routeKey, fromPeerId, fromPeerRole, toPeer,
            toPeerRole, attributes, statement.getConditions())) {
            return attributes;
        }
        conditionSatisfied = true;
        return this.actionsRegistry.applyExportAction(localAs, originatorId, clusterId, routeKey, fromPeerId,
            fromPeerRole, toPeer, toPeerRole, attributes, actions);
    }

    @Override
    public ContainerNode applyImportStatement(final OpenconfigPolicyConsumer policyConsumer, final long localAs,
        final Ipv4Address originatorId, final ClusterIdentifier clusterId, final PathArgument routeKey,
        final PeerId fromPeerId, final PeerRole fromPeerRole, final ContainerNode attributes,
        final Statement statement, boolean conditionSatisfied) {
        if (!this.conditionsRegistry.matchImportConditions(policyConsumer, routeKey, fromPeerId, fromPeerRole,
            attributes, statement.getConditions())) {
            return attributes;
        }
        conditionSatisfied = true;
        return this.actionsRegistry.applyImportAction(localAs, originatorId, clusterId, routeKey, fromPeerId,
            fromPeerRole, attributes, statement.getActions());
    }
