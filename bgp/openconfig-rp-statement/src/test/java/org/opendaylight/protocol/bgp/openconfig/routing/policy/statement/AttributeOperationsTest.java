...
import static org.opendaylight.protocol.bgp.openconfig.routing.policy.statement.ExportAttributeTestUtil.ATTRS_EXTENSION_Q;
import static org.opendaylight.protocol.bgp.openconfig.routing.policy.statement.ExportAttributeTestUtil.SEGMENTS_NID;
import static org.opendaylight.protocol.bgp.openconfig.routing.policy.statement.ExportAttributeTestUtil.SEQ_SEGMENT;
import static org.opendaylight.protocol.bgp.openconfig.routing.policy.statement.ExportAttributeTestUtil.SET_SEGMENT;
...
import org.opendaylight.protocol.bgp.openconfig.routing.policy.statement.actions.AttributeOperations;
...
public class AttributeOperationsTest {
    ...
    static final NodeIdentifier ORIGIN_NID
            = new NodeIdentifier(QName.create(ATTRS_EXTENSION_Q, Origin.QNAME.getLocalName()).intern());
    static final NodeIdentifier ORIGIN_VALUE_NID
            = new NodeIdentifier(QName.create(ATTRS_EXTENSION_Q, "value").intern());
    static final NodeIdentifier AS_PATH_NID
            = new NodeIdentifier(QName.create(ATTRS_EXTENSION_Q, AsPath.QNAME.getLocalName()).intern());
    static final NodeIdentifier ATOMIC_NID
            = new NodeIdentifier(QName.create(ATTRS_EXTENSION_Q, AtomicAggregate.QNAME.getLocalName()));
    static final NodeIdentifier CLUSTER_C_NID
            = new NodeIdentifier(QName.create(ATTRS_EXTENSION_Q, ClusterId.QNAME.getLocalName()));
    static final NodeIdentifier CLUSTER_NID
            = new NodeIdentifier(QName.create(ATTRS_EXTENSION_Q, "cluster"));
    static final NodeIdentifier ORIGINATOR_C_NID
            = new NodeIdentifier(QName.create(ATTRS_EXTENSION_Q, OriginatorId.QNAME.getLocalName()));
    static final NodeIdentifier ORIGINATOR_NID
            = new NodeIdentifier(QName.create(ATTRS_EXTENSION_Q, "originator"));

    private final Ipv4Address originatorId = new Ipv4Address("127.0.0.2");
    private final ClusterIdentifier clusterId = new ClusterIdentifier("10.10.10.10");
    private final long ourAs = 72L;

    @Mock
    private RouteEntryBaseAttributes baseAtt;
    ...
    private static LeafSetNode<?> checkFirstLeafList(final ContainerNode exportedAttributes) {
        ...
        final UnkeyedListNode segments
                = (UnkeyedListNode) NormalizedNodes.findNode(exportedAttributes, AS_PATH_NID, SEGMENTS_NID).get();
    ...
    public void setUp() {
        ...
        doReturn(this.originatorId).when(this.baseAtt).getOriginatorId();
        doReturn(this.clusterId).when(this.baseAtt).getClusterId();
        doReturn(this.ourAs).when(this.baseAtt).getLocalAs();

    }

    @Test
    public void testExportedAttributesSetFirst() {
        final ContainerNode attributesSetBefore = Builders.containerBuilder()
        ...
        final ContainerNode exportedAttributes = operations.exportedAttributes(attributesSetBefore, this.ourAs);

        // make sure our AS is prepended to the list (ourAs the AS-PATH starts with AS-SET)
        final LeafSetNode<?> list = checkFirstLeafList(exportedAttributes);
        assertEquals(this.ourAs, list.getValue().iterator().next().getValue());
    }

    @Test
    public void testExportedAttributesListFirst() {
        final ContainerNode attributesListBefore = Builders.containerBuilder()
                .withNodeIdentifier(new NodeIdentifier(ATTRS_EXTENSION_Q))
                .addChild(Builders.containerBuilder().withNodeIdentifier(AS_PATH_NID)
                        .addChild(Builders.unkeyedListBuilder().withNodeIdentifier(SEGMENTS_NID)
                                .addChild(SEQ_SEGMENT)
        ...
        final ContainerNode exportedAttributes = operations.exportedAttributes(attributesListBefore, this.ourAs);

        // make sure our AS is appended to the a-list (ourAs the AS-PATH starts with A-LIST)
        final LeafSetNode<?> list = checkFirstLeafList(exportedAttributes);
        final Iterator<?> iter = list.getValue().iterator();
        assertEquals(this.ourAs, ((LeafSetEntryNode<?>) iter.next()).getValue());
    ...
    public void testExportedAttributesEmptyWithTransitive() {
        ...
        // Origin should be within exportedAttributes ourAs it is Transitive
    ...
    public void testReflectedAttributesOriginatorAndClusterNotPresent() {
        final ContainerNode attributes = Builders.containerBuilder()
                ...
                .addChild(Builders.containerBuilder().withNodeIdentifier(ORIGIN_NID)
                        .addChild(Builders.leafBuilder()
                                .withNodeIdentifier(ORIGIN_VALUE_NID).withValue(BgpOrigin.Egp).build())
                        .build())
                .build();
        final AttributeOperations operations = AttributeOperations.getInstance(attributes);
        final ContainerNode reflectedAttributes = operations.reflectedAttributes(attributes, this.baseAtt);

        // Origin should be within reflectedAttributes ourAs part of original attributes
        ...
        assertEquals(originatorId.getValue(), ((ContainerNode) reflectedAttributes
                .getChild(ORIGINATOR_C_NID).get()).getChild(ORIGINATOR_NID).get().getValue());
    }
    ...
    public void testReflectedAttributesOriginatorAndClusterPresent() {
        ...
        final ContainerNode attributes = Builders.containerBuilder()
                ...
                .addChild(Builders.containerBuilder().withNodeIdentifier(ORIGINATOR_C_NID)
                        .addChild(Builders.leafBuilder()
                                .withNodeIdentifier(ORIGINATOR_NID).withValue("127.0.0.2").build()).build())
                .addChild(Builders.containerBuilder().withNodeIdentifier(CLUSTER_C_NID)
                        .addChild(Builders.orderedLeafSetBuilder().withNodeIdentifier(CLUSTER_NID)
                                .addChild(Builders.leafSetEntryBuilder().withNodeIdentifier(
                                        new NodeWithValue<>(CLUSTER_NID.getNodeType(), clusterId1.getValue()))
                                        .withValue(clusterId1.getValue()).build())
                                .addChild(Builders.leafSetEntryBuilder().withNodeIdentifier(
                                        new NodeWithValue<>(CLUSTER_NID.getNodeType(), clusterId2.getValue()))
                                        .withValue(clusterId2.getValue()).build()).build()).build()).build();
        final AttributeOperations operations = AttributeOperations.getInstance(attributes);
        doReturn(ourClusterId).when(this.baseAtt).getClusterId();
