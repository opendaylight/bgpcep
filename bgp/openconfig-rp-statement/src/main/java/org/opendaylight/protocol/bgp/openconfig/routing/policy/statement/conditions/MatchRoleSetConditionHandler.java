...
public final class MatchRoleSetConditionHandler implements BGPConditionPolicy {
    ...
    public MatchRoleSetConditionHandler(final DataBroker dataBroker) {
        this.dataBroker = requireNonNull(dataBroker);
    }

    public boolean matchRoleCondition(final MatchRoleSet roleMatch, final PeerRole role) {
        final String peerSetKey = StringUtils
                .substringBetween(roleMatch.getRoleSet(), "=\"", "\"");
        final List<PeerRole> roles = this.roleSets.getUnchecked(peerSetKey);
    ...
    public boolean matchImportCondition(
            ...
            final Augmentation<BgpConditions> conditions) {
        Preconditions.checkArgument(conditions instanceof MatchRoleSetCondition,
                "The condition augmentation %s is not MatchRoleSetCondition type.", conditions);

        final List<MatchRoleSet> neighCond = ((MatchRoleSetCondition) conditions).getMatchRoleSet();
