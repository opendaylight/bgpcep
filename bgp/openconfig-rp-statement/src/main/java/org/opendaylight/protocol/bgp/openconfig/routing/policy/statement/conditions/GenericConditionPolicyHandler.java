...
public class GenericConditionPolicyHandler implements GenericConditionPolicy {
    ...
    public GenericConditionPolicyHandler(final DataBroker databroker) {
    ...
    private boolean matchPrefix(
        ...
        final List<Prefix> prefixSet = prefixSets.getUnchecked(matchPrefixSet.getPrefixSet());
        final boolean found = prefixSet.stream().map(Prefix::getIpPrefix).collect(Collectors.toList()).contains(prefix);
        if (matchPrefixSet.getMatchSetOptions().equals(ANY)) {
            return found;
        } else {
            //INVERT CASE
            return !found;
