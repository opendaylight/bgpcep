...
public final class AttributeOperations {
    ...
    private static final LoadingCache<QNameModule, AttributeOperations> ATTRIBUTES_CACHE = CacheBuilder.newBuilder()
    ...
    private AttributeOperations(final QNameModule namespace) {
        ...
        this.clusterListContainer = new NodeIdentifier(QName.create(namespace, ClusterId.QNAME.getLocalName())
                .intern());
        ...
        this.originatorIdContainer = new NodeIdentifier(QName.create(namespace, OriginatorId.QNAME.getLocalName())
                .intern());
    ...
    public static AttributeOperations getInstance(final ContainerNode attributes) {
        return ATTRIBUTES_CACHE.getUnchecked(attributes.getNodeType().getModule());
    }

    private static void addOtherClusterEntries(
            final Optional<NormalizedNode<?, ?>> maybeClusterList,
            final ListNodeBuilder<Object,
        ...
        if (clusterList instanceof LeafSetNode) {
            for (final LeafSetEntryNode<?> n : ((LeafSetNode<?>) clusterList).getValue()) {
                ...
                @SuppressWarnings("unchecked") final LeafSetEntryNode<Object> child = (LeafSetEntryNode<Object>) n;
                clb.addChild(child);
            }
        } else {
    ...
    public ContainerNode exportedAttributes(final ContainerNode attributes, final Long localAs) {
        ...
        final CollectionNodeBuilder<UnkeyedListEntryNode, UnkeyedListNode> segmentsBuilder
                = Builders.unkeyedListBuilder();
        segmentsBuilder.withNodeIdentifier(this.asPathSegments);

        final Optional<NormalizedNode<?, ?>> maybeOldAsSegments
                = NormalizedNodes.findNode(attributes, this.asPathContainer, this.asPathSegments);
        if (maybeOldAsSegments.isPresent() && !((UnkeyedListNode) maybeOldAsSegments.get()).getValue().isEmpty()) {
            ...
            final ListNodeBuilder<Object, LeafSetEntryNode<Object>> asSequenceBuilder
                    = Builders.orderedLeafSetBuilder();
            // add local AS
            asSequenceBuilder.withNodeIdentifier(this.asPathSequence).addChild(Builders.leafSetEntryBuilder()
                    .withNodeIdentifier(new NodeWithValue(this.asNumberQname, localAs)).withValue(localAs).build());

            final Iterator<UnkeyedListEntryNode> oldAsSegments
                    = ((UnkeyedListNode) maybeOldAsSegments.get()).getValue().iterator();
            ...
            if (reusableAsSeq != null) {
                for (final LeafSetEntryNode<?> child : reusableAsSeq.getValue()) {
                    asSequenceBuilder.withChild(Builders.leafSetEntryBuilder()
                            .withNodeIdentifier(new NodeWithValue(this.asNumberQname, child.getValue()))
                            .withValue(child.getValue()).build());
                }
            }
            // Add the new first segment
            segmentsBuilder.withChild(Builders.unkeyedListEntryBuilder().withNodeIdentifier(this.asPathSegments)
                    .withChild(asSequenceBuilder.build()).build());
            ...
            // a single entry
            segmentsBuilder.withChild(Builders.unkeyedListEntryBuilder().withNodeIdentifier(this.asPathSegments)
                    .withChild(Builders.orderedLeafSetBuilder().withNodeIdentifier(this.asPathSequence)
                            .addChild(Builders.leafSetEntryBuilder()
                                    .withNodeIdentifier(new NodeWithValue(this.asNumberQname, localAs))
                                    .withValue(localAs).build()).build()).build());
        }

        containerBuilder.withChild(Builders.containerBuilder().withNodeIdentifier(this.asPathContainer)
                .withChild(segmentsBuilder.build()).build());
     ...
     */
    public ContainerNode reflectedAttributes(final ContainerNode attributes) {
        final DataContainerNodeAttrBuilder<NodeIdentifier, ContainerNode> attributesContainer
                = Builders.containerBuilder(attributes);

        // if there was a CLUSTER_LIST attribute, add it
        final Optional<NormalizedNode<?, ?>> maybeClusterList
                = NormalizedNodes.findNode(attributes, this.clusterListPath);
        if (maybeClusterList.isPresent()) {
            ...
            final DataContainerNodeAttrBuilder<NodeIdentifier, ContainerNode> clusterListCont
                    = Builders.containerBuilder();
    ...
    public ContainerNode reflectedAttributes(
        ...
        clusterBuilder.withChild(Builders.leafSetEntryBuilder()
                .withNodeIdentifier(new NodeWithValue(this.clusterQname, clusterId.getValue()))
        ...
        final Optional<NormalizedNode<?, ?>> maybeOriginatorId
                = NormalizedNodes.findNode(attributes, this.originatorIdPath);
        if (!maybeOriginatorId.isPresent()) {
            final DataContainerNodeAttrBuilder<NodeIdentifier, ContainerNode> originatorIDBuilder
                    = Builders.containerBuilder();
    ...
    private boolean spliceTransitives(
            final DataContainerNodeAttrBuilder<NodeIdentifier, ContainerNode> target,
            final ContainerNode attributes) {
     ...
     */
    public ContainerNode transitiveAttributes(final ContainerNode attributes) {
