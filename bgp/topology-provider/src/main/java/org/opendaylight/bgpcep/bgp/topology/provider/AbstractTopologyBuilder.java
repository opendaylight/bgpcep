    ...
    public synchronized void onDataTreeChanged(final Collection<DataTreeModification<T>> changes) {
        ...
        for (final DataTreeModification<T> change : changes) {
            try {
                ...
                // we should avoid rebuilding the whole listener as there might be huge overhead when the topology is big
                // here we retry the #onDataTreeChanged() with changes that succeed
                final List<DataTreeModification<T>> succeedChange = new ArrayList<>();
                succeedChange.addAll(changes);
                succeedChange.remove(change);
                resetTransactionChain();
                if (!succeedChange.isEmpty()) {
                    onDataTreeChanged(succeedChange);
                }
                return;
            }
        }
