...
public abstract class AbstractTopologyBuilder<T extends Route> implements AutoCloseable, DataTreeChangeListener<T>, TopologyReference, TransactionChainListener {
     ...
     */
    public final synchronized void registerDataChangeListener() {
     ...
     */
    public final synchronized void unregisterDataChangeListener() {
        if (this.listenerRegistration != null) {
            ...
            try {
                ...
                LOG.error("Error encountered when unregistering listener {} on topology {}", this, this.getInstanceIdentifier(), e);
            }
        }
    }
    ...
    public synchronized void onDataTreeChanged(final Collection<DataTreeModification<T>> changes) {
        ...
        for (final DataTreeModification<T> change : changes) {
            try {
                ...
                // trans.cancel() is not supported by PingPongTransactionChain, so we reset the listener instead
                resetListener();
                return;
            }
        }
     ...
     */
    private synchronized void destroyOperationalTopology() throws TransactionCommitFailedException {
        Preconditions.checkNotNull(this.chain, "A valid transaction chain must be provided.");
    ...
    private synchronized void resetListener() {
        ...
        try {
            ...
            LOG.error("Unable to reset operational topology {}", this.topology, e);
        } finally {
            initOperationalTopology();
        }
