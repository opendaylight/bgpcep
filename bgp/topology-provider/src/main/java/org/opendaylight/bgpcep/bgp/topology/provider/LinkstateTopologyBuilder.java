...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.topology.sr.rev130819.sr.node.attributes.segments.segment.specification.NodeCaseBuilder;
...
public class LinkstateTopologyBuilder extends AbstractTopologyBuilder<LinkstateRoute> {
    ...
    private static final TopologyTypes SR_AWARE_LINKSTATE_TOPOLOGY_TYPE =
    ...
    private final class NodeHolder {
        ...
        private Long srgbFirstValue;
        private Integer srgbRangeSize;
        private IpPrefix nodePrefix;
        private Long sidIndex;
        private Long nodeSid;
        private Segments nodeSegment;
        ...
        void addSrgb(final WriteTransaction trans, final boolean updateNode, final Long srgbFirstVal,
            ...
            if (this.sidIndex != null) {
                if (this.sidIndex >= this.srgbRangeSize) {
                    LOG.warn("Node SID index {} is outside the SRGB range of {} for node {}", this.sidIndex,
                            this.srgbRangeSize, this.nb.getNodeId().getValue());
                    return;
                }
                this.nodeSid = this.srgbFirstValue + this.sidIndex;
                handleNodeSidChange(trans, updateNode);
            }
        }
        ...
        void removeSrgb(final WriteTransaction trans) {
            ...
            this.nodeSid = null;
            handleNodeSidChange(trans, false);
        }

        void addSidIndex(final WriteTransaction trans, final boolean updateNode, final IpPrefix ippfx,
                final Long sidIdx) {
            this.nodePrefix = ippfx;
            this.sidIndex = sidIdx;
        ...
        void removeSidIndex(final WriteTransaction trans, final IpPrefix ippfx) {
            if (!Objects.equals(this.nodePrefix, ippfx)) {
                return;
            }
            this.nodePrefix = null;
            this.sidIndex = null;
            this.nodeSid = null;
            handleNodeSidChange(trans, true);
        }

        @SuppressWarnings("checkstyle:LineLength")
        void handleNodeSidChange(final WriteTransaction trans, final boolean updateNode) {
            if (this.nodeSid != null) {
                LOG.trace("Adding node SID {} for node {}", this.nodeSid, this.nb.getNodeId().getValue());
                final SegmentId segmentId = new SegmentId(Uint32.valueOf(this.nodeSid));
                final SegmentsBuilder sb = new SegmentsBuilder();
                sb.setSegmentId(segmentId);
                sb.withKey(new SegmentsKey(segmentId));
                sb.setSegmentSpecification(new NodeCaseBuilder().setNode(
                        new org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.topology.sr.rev130819.sr.node.attributes.segments.segment.specification.node._case.NodeBuilder()
                                .setNode(getNodeId()).build())
                        .build());
                this.nodeSegment = sb.build();
                this.segments.add(this.nodeSegment);
                addSegment(trans, updateNode, this.nodeSegment);
            } else if (this.nodeSegment != null) {
                LOG.trace("Removing node SID for node {}", this.nb.getNodeId().getValue());
                this.segments.remove(this.nodeSegment);
                removeSegment(trans, updateNode, this.nodeSegment);
                this.nodeSegment = null;
            }
        }
        ...
        void removeAdjacencySid(final WriteTransaction trans, final LinkId linkId) {
            ...
            LOG.trace("Removing adjacency SID for link {} on node {}", linkId.getValue(),
                    this.nb.getNodeId().getValue());
            final Segments adjSegment = this.adjSegments.get(linkId);
            this.adjSegments.remove(linkId);
    ...
    private void createPrefix(final WriteTransaction trans, final UriBuilder base,
        ...
        Long nodeSidIndex = null;
        if (pa != null) {
            if (pa.getPrefixMetric() != null) {
                pb.setMetric(pa.getPrefixMetric().getValue());
            }
            if (pa.getSrPrefix() != null) {
                final SrPrefix srPrefix = pa.getSrPrefix();
                if (isAssociatedWithNodeSid(ippfx, srPrefix)) {
                    nodeSidIndex = getSegmentIdValue(srPrefix.getSidLabelIndex());
                }
            }
        }
        ...
        if (nh == null) {
            nh = getNode(node);
            nh.addPrefix(pfx);
            if (nodeSidIndex != null) {
                nh.addSidIndex(trans, false, ippfx, nodeSidIndex);
            }
            putNode(trans, nh);
        } else {
            nh.addPrefix(pfx);
            if (nodeSidIndex != null) {
                nh.addSidIndex(trans, true, ippfx, nodeSidIndex);
            }
            final InstanceIdentifier<Node> nid = getNodeInstanceIdentifier(new NodeKey(nh.getNodeId()));
    ...
    private boolean isAssociatedWithNodeSid(final IpPrefix ippfx, final SrPrefix srPrefix) {
