...
public class LinkstateGraphBuilder extends AbstractTopologyBuilder<LinkstateRoute> {
    ...
    private void createVertex(final LinkstateRoute value, final NodeCase nodeCase, final Attributes attributes) {
        ...
        if (vertex.getRouterId().getIpv4Address() != null) {
            builder.setPrefix(new IpPrefix(new Ipv4Prefix(vertex.getRouterId().getIpv4Address().getValue() + "/32")));
        }
        if (vertex.getRouterId().getIpv6Address() != null) {
            builder.setPrefix(new IpPrefix(new Ipv6Prefix(vertex.getRouterId().getIpv6Address().getValue() + "/128")));
        }
