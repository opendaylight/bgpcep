...
public class LinkstateGraphBuilder extends AbstractTopologyBuilder<LinkstateRoute> {
    ...
    private void createEdge(final LinkstateRoute value, final LinkCase linkCase, final Attributes attributes) {
        ...
        /*
         * Add corresponding Prefix for the Local Address. Remote address will
         * be added with the remote Edge
         */
        ...
        if (edge.getEdgeAttributes().getLocalAddress().getIpv4Address() != null) {
            prefBuilder.setPrefix(new IpPrefix(
                    new Ipv4Prefix(edge.getEdgeAttributes().getLocalAddress().getIpv4Address().getValue())));
        }
        if (edge.getEdgeAttributes().getLocalAddress().getIpv6Address() != null) {
            prefBuilder.setPrefix(new IpPrefix(
                    new Ipv6Prefix(edge.getEdgeAttributes().getLocalAddress().getIpv6Address().getValue())));
        }
    ...
    private void createVertex(final LinkstateRoute value, final NodeCase nodeCase, final Attributes attributes) {
        ...
        /* Add corresponding Prefix for the router ID */
        PrefixBuilder builder = new PrefixBuilder().setVertexId(vertexId);
        if (vertex.getRouterId().getIpv4Address() != null) {
            builder.setPrefix(new IpPrefix(new Ipv4Prefix(vertex.getRouterId().getIpv4Address().getValue())));
        }
        if (vertex.getRouterId().getIpv6Address() != null) {
            builder.setPrefix(new IpPrefix(new Ipv6Prefix(vertex.getRouterId().getIpv6Address().getValue())));
        }
        Prefix prefix = builder.build();

        /* Add the Connected Vertex and associated Vertex in the Graph */
        LOG.info("Add Vertex {} and associated Prefix {} in TED[{}]", vertex.getName(), prefix.getPrefix(), cgraph);
    ...
    private Uint64 getVertexId(CRouterIdentifier routerID) {
        ...
        if (routerID instanceof IsisNodeCase) {
            ...
            rid = ByteBuffer.wrap(isis.getIsoSystemId().getValue()).getLong();
        }
        if (routerID instanceof OspfNodeCase) {
            final OspfNode ospf = ((OspfNodeCase) routerID).getOspfNode();
            rid = ospf.getOspfRouterId().longValue();
        }
