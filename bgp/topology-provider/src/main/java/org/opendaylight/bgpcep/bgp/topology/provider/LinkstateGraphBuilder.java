...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.graph.rev191125.graph.topology.Graph.GraphType;
...
public class LinkstateGraphBuilder extends AbstractTopologyBuilder<LinkstateRoute> {
    ...
    private ListenerRegistration<LinkstateGraphBuilder> listenerRegistration = null;
    ...
    public LinkstateGraphBuilder(final DataBroker dataProvider, final RibReference locRibReference,
        ...
        this.cgraph = provider.createConnectedGraph("ted://" + configuration.getTopologyId().getValue(),
                GraphType.IntraDomain);
     ...
     *
     * @param linkCase
     *            The Link part of the Linkstate route
    ...
    /**
     * Determine the Source Edge Key from the link descriptor. There is several
     * case: IPv4, IPv6 address or Unnumbered Interface.
     *
     * @param linkCase
     *            The Link part of the Linkstate route
     * @return Unique key
     ...
     *
     * @param value
     *            The complete Linkstate route information
     * @param linkCase
     *            The Link part of the Linkstate route
     * @param attributes
     *            Link attributes
    ...
    private void createEdge(final LinkstateRoute value, final LinkCase linkCase, final Attributes attributes) {
        ...
        LOG.info("Add Edge {} in TED[{}]", edge.getName(), cgraph);
        cgraph.addEdge(edge);
    }
    ...
    private EdgeAttributes createEdgeAttributes(LinkAttributes la, final LinkDescriptors linkDesc) {
        ...
        ArrayList<BigDecimal> unRsvBw = new ArrayList<BigDecimal>(MAX_PRIORITY);
        ...
        if (linkDesc.getIpv4NeighborAddress() != null) {
            builder.setLocalAddress(new IpAddress(new Ipv4Address(linkDesc.getIpv4NeighborAddress())));
        }
        if (linkDesc.getIpv6NeighborAddress() != null) {
            builder.setLocalAddress(new IpAddress(new Ipv6Address(linkDesc.getIpv6NeighborAddress())));
        }
        ...
        if (la.getMaxLinkBandwidth() != null) {
            builder.setMaxLinkBandwidth(bandwithToBigDecimal(la.getMaxLinkBandwidth()));
        }
        if (la.getMaxReservableBandwidth() != null) {
            builder.setMaxResvLinkBandwidth(bandwithToBigDecimal(la.getMaxReservableBandwidth()));
        }
        if (la.getUnreservedBandwidth() != null) {
            int upperBound = Math.min(la.getUnreservedBandwidth().size(), MAX_PRIORITY);
            unRsvBw.clear();
            for (int i = 0; i < upperBound; i++) {
                unRsvBw.add(i, bandwithToBigDecimal(la.getUnreservedBandwidth().get(i).getBandwidth()));
            }
        ...
        if (la.getLinkDelay() != null) {
            builder.setDelay(new Delay(la.getDelayVariation().getValue()));
        }
        ...
        if (la.getAvailableBandwidth() != null) {
            builder.setAvailableBandwidth(bandwithToBigDecimal(la.getAvailableBandwidth()));
        }
        if (la.getResidualBandwidth() != null) {
            builder.setResidualBandwidth(bandwithToBigDecimal(la.getResidualBandwidth()));
        }
        if (la.getUtilizedBandwidth() != null) {
            builder.setUtilizedBandwidth(bandwithToBigDecimal(la.getUtilizedBandwidth()));
        }
     ...
     *
     * @param attributes
     *            The attribute part from the Link State route
     ...
     *            Node Attributes
     * @param cvertex
     *            Connected Vertex associated to this Vertex
     * @param as
     *            As number
     * @return New Vertex
     */
    private Vertex createVertex(NodeAttributes na, Uint64 id, int as) {
        ...
        if (na.getSrCapabilities() != null) {
            if (na.getSrCapabilities().getSidLabelIndex() instanceof SidCase) {
                final SidCase sidcase = (SidCase) na.getSrCapabilities().getSidLabelIndex();
                builder.setSrgb(new SrgbBuilder().setLowerBound(sidcase.getSid())
                        .setRangeSize(na.getSrCapabilities().getRangeSize().getValue()).build());
            }
        }
     ...
     *            The complete Linkstate route information
     * @param nodeCase
     *            The node part of the Linkstate route
     * @param attributes
     *            The node attributes
     */
    private void createVertex(final LinkstateRoute value, final NodeCase nodeCase, final Attributes attributes) {
        ...
        Vertex vertex = createVertex(na, vertexId, asNumber);

        // Update corresponding prefix
        // graph.addPrefix(32, cvertex.getRID(), cvertex);
     ...
     *            The complete Linkstate route information
     * @param prefixCase
     *            The Prefix part of the Linkstate route
     * @param attributes
    ...
    private void createPrefix(final LinkstateRoute value, final PrefixCase prefixCase, final Attributes attributes) {
        ...
        if (pa.getSrPrefix() != null && pa.getSrPrefix().getSidLabelIndex() instanceof SidCase) {
            builder.setPrefixSid(Uint32.valueOf(((SidCase) pa.getSrPrefix().getSidLabelIndex()).getSid().intValue()));
     ...
     * @return Vertex in the Traffic Engineering Graph that corresponds to this
     *         Router ID. Vertex is created if not found
     */
    private Uint64 getVertexId(CRouterIdentifier routerID) {
        ...
        if (routerID instanceof IsisNodeCase) {
            ...
            byte[] id = isis.getIsoSystemId().getValue();
            rid = ((((0xFF & id[0]) << 40) | (0xFF & id[1] << 32) | (0xFF & id[2]) << 24) | ((0xFF & id[3]) << 16)
                    | ((0xFF & id[4]) << 8) | (0xFF & id[5]));
        }
        if (routerID instanceof OspfNodeCase) {
            final OspfNode ospf = ((OspfNodeCase) routerID).getOspfNode();
            rid = ospf.getOspfRouterId().intValue();
        }
    ...
    private BigDecimal bandwithToBigDecimal(Bandwidth bw) {
        return BigDecimal.valueOf(ByteBuffer.wrap(bw.getValue()).getFloat());
    }
    ...
    private long ipv6ToKey(String str) {
        ...
        return (((0xFF & ip[0]) << 56) | ((0xFF & ip[1]) << 48) | ((0xFF & ip[2]) << 40) | ((0xFF & ip[3]) << 32)
                | ((0xFF & ip[4]) << 24) | ((0xFF & ip[5]) << 16) | ((0xFF & ip[6]) << 8) | (0xFF & ip[7]));
    }
