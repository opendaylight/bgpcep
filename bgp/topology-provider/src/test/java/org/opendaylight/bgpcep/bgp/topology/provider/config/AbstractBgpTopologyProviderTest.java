...
public abstract class AbstractBgpTopologyProviderTest extends AbstractConcurrentDataBrokerTest {
    ...
    @Mock
    protected AbstractRegistration serviceRegistration;
    @Mock
    ...
    public void setUp() {
        initMocks(this);
        Mockito.doReturn(singletonServiceRegistration).when(cssp).registerClusterSingletonService(any(ClusterSingletonService.class));
        Mockito.doReturn(registration).when(bundleContext).registerService(any(String[].class), any() ,any());
        Mockito.doReturn(registration).when(bundleContext).registerService(any(String.class), any() ,any());
        bgpTopologyDeployer = new BgpTopologyDeployerImpl(bundleContext, getDataBroker(), cssp);
        Mockito.doNothing().when(serviceRegistration).close();
        Mockito.doNothing().when(registration).unregister();
    }
}
