...
public class LinkstateGraphBuilderTest {
    ...
    @Before
    public void setUp() {
        connectedGraph = mock(ConnectedGraph.class);
        final var ribReference = mock(RibReference.class);
        final var connectedGraphProvider = mock(ConnectedGraphProvider.class);
        final var dataBroker = mock(DataBroker.class);
        when(connectedGraphProvider.createConnectedGraph(eq("ted://" + TOPOLOGY_ID.getValue()),
            eq(Graph.DomainScope.IntraDomain))).thenReturn(connectedGraph);
    ...
    public void testCreateEdgeNoPrefix() {
        ...
        final LinkstateRoute route = createLinkstateRoute(descriptorsBuilder.build());
