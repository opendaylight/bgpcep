...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.flowspec.rev150807.update.attributes.mp.reach.nlri.c.next.hop.FlowspecIpv4NextHopCaseBuilder;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.bgp.flowspec.rev150807.update.attributes.mp.reach.nlri.c.next.hop.flowspec.ipv4.next.hop._case.Ipv4NextHopBuilder;
...
public class FlowspecIpv4NextHopParser implements NextHopParser, NextHopSerializer {
    @Override
    public CNextHop parseNextHop(final ByteBuf buffer) throws BGPParsingException {
        ...
        return new FlowspecIpv4NextHopCaseBuilder().setIpv4NextHop(new Ipv4NextHopBuilder().setGlobal(Ipv4Util
            .addressForByteBuf(buffer)).build()).build();
    }

    @Override
    public void serializeNextHop(final CNextHop cNextHop, final ByteBuf byteAggregator) {
        Preconditions.checkArgument(cNextHop instanceof FlowspecIpv4NextHopCase, "cNextHop is not a Link " +
            "State NextHop Case object.");
        final Ipv4Address nextHop = ((FlowspecIpv4NextHopCase) cNextHop).getIpv4NextHop().getGlobal();
