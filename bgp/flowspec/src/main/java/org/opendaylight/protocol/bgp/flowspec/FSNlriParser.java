...
public class FSNlriParser implements NlriParser, NlriSerializer {
    ...
    private static NumericOperand createNumericOperand(final Set<String> opValues) {
        return new NumericOperand(opValues.contains("and-bit"), opValues.contains("end-of-list"), opValues.contains("equals"), opValues.contains("greater-than"), opValues.contains("less-than"));
    }

    private static BitmaskOperand createBitmaskOperand(final Set<String> opValues) {
        return new BitmaskOperand(opValues.contains("and-bit"), opValues.contains("end-of-list"), opValues.contains("match"), opValues.contains("not"));
    }

    private static Fragment createFragment(final Set<String> data) {
        return new Fragment(data.contains("do-not"), data.contains("first"), data.contains("is-a"), data.contains("last"));
    }

    private static List<ProtocolIps> createProtocolsIps(final UnkeyedListNode protocolIps) {
        final ProtocolIpsBuilder ipsBuilder = new ProtocolIpsBuilder();

        for (final UnkeyedListEntryNode node : protocolIps.getValue()) {
        ...
        return Lists.<ProtocolIps>newArrayList(ipsBuilder.build());
    }

    private static List<Ports> createPorts(final UnkeyedListNode ports) {
        final PortsBuilder portsBuilder = new PortsBuilder();

        for (final UnkeyedListEntryNode node : ports.getValue()) {
            if (node.getNodeType().getLocalName().equals("op")) {
                Collection<DataContainerChild<? extends PathArgument, ?>> oppNode = node.getValue();
                for (DataContainerChild<? extends PathArgument, ?> leafNode : oppNode) {
        ...
        return Lists.<Ports>newArrayList(portsBuilder.build());
    }

    private static List<DestinationPorts> createDestinationPorts(final UnkeyedListNode ports) {
        ...
        return Lists.<DestinationPorts>newArrayList(destPortsBuilder.build());
    }

    private static List<SourcePorts> createSourcePorts(final UnkeyedListNode ports) {
        ...
        return Lists.<SourcePorts>newArrayList(sourcePortsBuilder.build());
    }

    private static List<Types> createTypes(final UnkeyedListNode ports) {
        ...
        return Lists.<Types>newArrayList(typesBuilder.build());
    }

    private static List<Codes> createCodes(final UnkeyedListNode ports) {
        ...
        return Lists.<Codes>newArrayList(codesBuilder.build());
    }

    private static List<TcpFlags> createTcpFlags(final UnkeyedListNode ports) {
        ...
        return Lists.<TcpFlags>newArrayList(tcpFlagsBuilder.build());
    }

    private static List<PacketLengths> createPacketLengths(final UnkeyedListNode ports) {
        ...
        return Lists.<PacketLengths>newArrayList(packetLengthsBuilder.build());
    }

    private static List<Dscps> createDscpsLengths(final UnkeyedListNode ports) {
        final DscpsBuilder packetLengthsBuilder = new DscpsBuilder();
        ...
        for (final UnkeyedListEntryNode node : ports.getValue()) {
            ...
            if (node.getNodeType().getLocalName().equals("value")) {
                Collection<DataContainerChild<? extends PathArgument, ?>> valueNode = node.getValue();
                for ( DataContainerChild<? extends PathArgument, ?> leafNode : valueNode) {
                    packetLengthsBuilder.setValue(new Dscp((Short)leafNode.getValue()));
                }
            }
        }

        return Lists.<Dscps>newArrayList(packetLengthsBuilder.build());
    }

    private static List<Fragments> createFragments(final UnkeyedListNode ports) {
        ...
        for (final UnkeyedListEntryNode node : ports.getValue()) {
            ...
            if (node.getNodeType().getLocalName().equals("value")) {
                Collection<DataContainerChild<? extends PathArgument, ?>> valueNode = node.getValue();
                for ( DataContainerChild<? extends PathArgument, ?> leafNode : valueNode) {
                    final Set<String> fragments = (Set<String>) leafNode.getValue();
                    fragmentsBuilder.setValue(createFragment(fragments));
                }
            }
        }

        return Lists.<Fragments>newArrayList(fragmentsBuilder.build());
    }
