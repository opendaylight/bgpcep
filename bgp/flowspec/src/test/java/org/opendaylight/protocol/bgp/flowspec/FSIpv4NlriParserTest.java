...
public class FSIpv4NlriParserTest {
    ...
    public void testParseMpReachNlri() throws BGPParsingException {
        ...
        assertEquals("all packets to 10.0.1.0/32 AND from 1.2.3.4/32 AND where IP protocol equals to 6 AND where port is greater than or equal to 137 and is less than or equal to 139 or equals to 8080 AND where destination port is greater than 4089 or equals to 179 AND where source port equals to 8080 ", FS_PARSER.stringNlri(flows));
    }
    ...
    public void testParseMpUnreachNlri() throws BGPParsingException {
        ...
        assertEquals("all packets where ICMP type is less than 2 or is less than 3 AND where ICMP code is less than is greater than 4 or 5 AND where TCP flags is not 1025 or does match 22193 AND where packet length is less than is greater than 57005 AND where DSCP is less than is greater than 42 AND where fragment does match 'IS FIRST' 'IS LAST' 'IS A' ", FS_PARSER.stringNlri(flows));
    }
    ...
    public void testExtractFlowspecDestPrefix() {
        ...
        entry.withChild(Builders.unkeyedListBuilder()
            .withNodeIdentifier(AbstractFSNlriParser.FLOWSPEC_NID)
            .withChild(Builders.unkeyedListEntryBuilder()
                .withNodeIdentifier(AbstractFSNlriParser.FLOWSPEC_NID)
                .withChild(Builders.choiceBuilder()
                    .withNodeIdentifier(AbstractFSNlriParser.FLOWSPEC_TYPE_NID)
                    .withChild(Builders.leafBuilder().withNodeIdentifier(AbstractFSNlriParser.DEST_PREFIX_NID).withValue("127.0.0.5/32").build()).build()).build()).build());
    ...
    public void testExtractFlowspecSourcePrefix() {
        ...
        entry.withChild(Builders.unkeyedListBuilder()
            ...
            .withChild(Builders.unkeyedListEntryBuilder()
                ...
                .withChild(Builders.choiceBuilder()
                    .withNodeIdentifier(AbstractFSNlriParser.FLOWSPEC_TYPE_NID)
                    .withChild(Builders.leafBuilder().withNodeIdentifier(AbstractFSNlriParser.SOURCE_PREFIX_NID).withValue("127.0.0.6/32").build()).build()).build()).build());
