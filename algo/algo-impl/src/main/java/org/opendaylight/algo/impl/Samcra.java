...
/**
 * This Class implements the Self Adaptive Multiple Constraints Routing
 * Algorithm (SAMCRA) a Path Computation Algorithm. The SAMCRA algorithm take
 * into account the Bandwidth, TE Metric and Delay as composite constraints.
 * Details of SAMCRA algorithm could be found in the article "Concepts of Exact
 * QoS Routing Algorithms", Piet Van Mieghem and Fernando A. Kuipers, IEEE/ACM
 * Transactions on Networking, Volume 12, Number 5, October 2004.
 *
...
public class Samcra extends AbstractPathComputation {
    ...
    /*
     * This class stores the set of paths which has been computed for a given
     * Connected Vertex: - pathCount number of active paths - pathCurrent node
     * path currently in the priority queue (path with minimal length) -
     * pathList list of computed paths
     *
     * Each path is represented by a "CspfPath" class to encompass path
     * predecessor, path status and path length information
     */
     ...
     * Samcra Algo:
     *
     * To limit the modification outside the Samcra method the same set of
     * parameters as the CSPF method is used (related to pseudo code, the path
     * length is computed inside the method based on the individual constraint
     * parameters).
     *
     * On contrast to a simple CSPF algo, with Samcra a connected vertex might
     * be associated to several metric vectors from which different path lengths
     * are computed. However a connected vertex is only present once in the
     * priority queue, associated to the minimal path weight, which is used as
     * key to address the priority queue.
     *
     * For a given metric the path weight is an integer value computed as the
     * entire part of the quantity: 100 * (vector_path_metric/target_metric) The
     * path weight correspond to the maximum length computed from either the
     * delay or TE metric.
     *
     * To maintain the priority queue behavior unchanged, a "SamcraPath" classes
     * is created to manage the set of possible paths associated to a given
     * vertex (see above).
     *
     */
    ...
    public ConstrainedPath computeP2pPath(VertexKey src, VertexKey dst, PathConstraints cts) {
        ...
        /*
         * Exploration of the priority queue: Each connected vertex is
         * represented only once in the priority queue associated to the path
         * with the minimal length (other path are stored in the SamcraPath
         * object). The top of the queue, i.e. the element with the minimal key(
         * path weight), is processed at each loop
         */
        while (priorityQueue.size() != 0) {
            ...
            for (ConnectedEdge edge : edges) {
                /*
                 * Connected Vertex's edges processing: Prune the connected
                 * edges that do not satisfy the constraints (Bandwidth, TE
                 * Metric, Delay, Loss) For each remaining edge process the path
                 * to the remote vertex using the "relaxSamcra" procedure
                 *
                 * If the return path length is positive, the destination is
                 * reached and the obtained route satisfies the requested
                 * constraints. The path length is checked to record only the
                 * optimal route (i.e. the route with the minimal path length)
                 * info obtained from the destination vertex
                 */
            ...
            /*
             * The connected vertex that has been removed from the priority
             * queue may have to be re-inserted with the minimal length
             * non-dominated path associated to the connected vertex if it
             * exists (to be done except for the source). Otherwise, the current
             * path associated to the connected vertex is reset to null to allow
             * the connected vertex addition to the priority queue later on with
             * a new path (refer to "relaxSamcra" for addition of a connected
            ...
            if (!(currentPath.equals(pathSource))) {
                ...
                /*
                 * The list of paths associated to the connected vertex is
                 * retrieved The path used to represent the connected vertex in
                 * the Priority Queue is marked from "selected" to "processed".
                 * The list of paths is analyzed to check if other "active"
                 * path(s) exist(s). If it is the case the shortest length is
                 * used to re-inject the connected vertex in the Priority Queue
                 */
                for (CspfPath testedPath : currentSamcraPath.getPathList()) {
                    LOG.trace("Samcra: priority queue output: testedPath: {} status: {} ", testedPath.getPath(),
                ...
                /*
                 * If a path is found it is marked as "selected", used as
                 * "current path" for the connected vertex and added to the
                 * priority queue
                 */
                if (selectedPath != null) {
                    ...
                    LOG.trace("Samcra priority queue output: add path to the priority queue: {} path count: {} ",
         ...
         * elements have been explored The "ConstrainedPathBuilder" object
         * contains the optimal path if it exists Otherwise an empty path with
    ...
    /*
     * Connected Edge to remote connected vertex processing (on contrast to CSPF
     * algorithm, the already processed connected vertex are not zapped as a
     * connected vertex may be associated to multiple paths). This method
     * computes the TE metric and Delay costs up to the remote end-point
     * connected vertex and checks if the computed values are acceptable
     * according to the end-to-end constraints. If relevant, update the computed
     * path on the remote end-point connected vertex. If the connected vertex
     * has not already been processed, the corresponding CspfPath object is
     * created.
     */
    private float relaxSamcra(ConnectedEdge edge, CspfPath currentPath, CspfPath source) {
        ...
        /*
         * Connected Vertex's paths management using SamcraPath object. The
         * predecessor connected vertex is checked to avoid unnecessary
         * processing.
         */
        ...
        /*
         * Connected Vertex's paths management using CspfPath object. The paths
         * list is explored and the paths dominated by the new path are marked
         * as dominated. The new path is also check and if it is dominated by an
         * existing path it is omitted.
         */
        ...
        /*
         * If the new path is not dominated by an already existing path, a new
         * "CspfPath" object is created with predecessor set to connected
         * vertex, path length and path status information, marked as "active"
         * and added to the connected vertex's path list. The weight attribute,
         * used as classification key by the priority queue, is an integer value
         * computed from the TE and delay length.
         */
        CspfPath newPath = createNonDominatedPath(edge, nextVertexPath.getVertex(), currentPath);

        /*
         * The new path is check versus the path currently representing the
         * connected vertex in the priority queue. If there is not yet a path
         * for the connected vertex or if the new path length is shorter than
         * the length of the path currently selected, the new path is used as
         * current path, marked as "selected" and is added to the priority
         * queue. The previously current path status is changed from "selected"
         * to "active" and can be re-selected later on. If the new path is
         * associated to the destination connected vertex it is not added to the
         * priority queue.
         */
        ...
        if (currentSamcraPath == null) {
            ...
            samcraPath.getPathList().stream().filter(path -> path.getPathStatus() == CspfPath.SELECTED)
                    .forEach(path -> path.setPathStatus(CspfPath.ACTIVE));

            /*
             * It is not possible to directly update the CspfPath in the
             * Priority Queue. Indeed, if we modify the path weight, the
             * Priority Queue must be re-ordered. So, we need fist to remove the
             * CspfPath if it is present in the Priority Queue, then, update the
             * Path Weight, and finally (re-)insert it in the Priority Queue.
             */
            if (!(newPath.equals(pathDestination))) {
        ...
        /*
         * In all cases the new path is added to the list of paths associated to
         * the vertex
         */
        ...
        LOG.trace("relaxSamcra: current Path {}  predecessor {}", samcraPath.getCurrentPath(),
                samcraPath.getCurrentPath().getPredecessor());
        samcraPaths.put(samcraPath.getVertex().getKey(), samcraPath);

        /*
         * If the destination is reached, return the computed path length 0
         * otherwise
         */
    ...
    /**
     * Evaluate if the current path is dominated by an all one or dominates all
        ...
        for (CspfPath testedPath : samcraPath.getPathList()) {
            ...
            if (testedPath.getPathStatus() != CspfPath.DOMINATED) {
                ...
                if ((((teMetric != null) && (pathCostDominated)) && ((pathDelayDominated) || (delay == null)))
                    ...
                    /*
                     * Old Path is dominated by the new path. Mark it as
                     * Dominated and decrement number of valid Path
                     */
    ...
    private CspfPath createNonDominatedPath(ConnectedEdge edge, ConnectedVertex vertex, CspfPath cspfPath) {
        ...
        CspfPath newPath = new CspfPath(vertex).setCost(teCost).setDelay(delayCost).setKey((int) (100 * pathLength))
                .setPathStatus(CspfPath.ACTIVE).setPathLength(pathLength).setPredecessor(cspfPath.getVertexKey())
                .replacePath(cspfPath.getPath()).addConnectedEdge(edge);

        LOG.debug("Creation of a new Path: {} path length: {} predecessor connected vertex {}", newPath, pathLength,
                newPath.getPredecessor());
