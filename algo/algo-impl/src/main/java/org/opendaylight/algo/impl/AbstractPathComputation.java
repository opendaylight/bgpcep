...
public abstract class AbstractPathComputation implements PathComputationAlgorithm {
    ...
    protected ConstrainedPathBuilder initializePathComputation(VertexKey src, VertexKey dst) {
        ConstrainedPathBuilder cpathBuilder = new ConstrainedPathBuilder();
    ...
    protected boolean pruneEdge(ConnectedEdge edge, CspfPath path) {
        ...
        /* Check that Edge meet Bandwidth constraints */
        Long bandwidth = constraints.getBandwitdh().longValue();
        if (attributes.getUnreservedBandwidth().get(constraints.getClassType().intValue()).longValue() < bandwidth
                || attributes.getMaxLinkBandwidth().longValue() < bandwidth
                || attributes.getMaxResvLinkBandwidth().longValue() < bandwidth) {
            return true;
        }

        /* Check that Edge belongs to the requested address family */
        switch (constraints.getAddressFamily().getIntValue()) {
            case 0:
