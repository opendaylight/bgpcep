...
public abstract class AbstractPathComputation implements PathComputationAlgorithm {
    ...
    private ConstrainedPath mergePath(ConstrainedPath cp1, ConstrainedPath cp2) {
        ConstrainedPathBuilder cpb = new ConstrainedPathBuilder(cp1);
        ArrayList<PathDescription> mergePathDesc = new ArrayList<PathDescription>();
        mergePathDesc.addAll(cp1.getPathDescription());
        mergePathDesc.addAll(cp1.getPathDescription());
        cpb.setPathDescription(mergePathDesc);
        if (!cp1.getStatus().equals(cp2.getStatus())) {
            cpb.setStatus(cp2.getStatus());
        }
        return cpb.build();
    }
