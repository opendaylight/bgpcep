    ...
    protected @Nullable MplsLabel getIpv4NodeSid(ConnectedVertex cvertex) {
        ...
        for (Prefix prefix : cvertex.getPrefixes()) {
            ...
            if (prefix.isNodeSid() && prefix.getPrefix().getIpv4Prefix() != null) {
                return new MplsLabel(Uint32.valueOf(
                        cvertex.getVertex().getSrgb().getLowerBound().intValue() + prefix.getPrefixSid().intValue()));
            }
        }
        return null;
    }
        ...
        for (ConnectedEdge edge : edges) {
            ...
            switch (constraints.getAddressFamily()) {
                ...
                case Ipv6:
                    ...
                    break;
                case SrIpv4:
                    pathDesc = new PathDescriptionBuilder().setLabel(getIpv4NodeSid(edge.getDestination())).build();
                    break;
                case SrIpv6:
                    pathDesc = new PathDescriptionBuilder().setLabel(getIpv6NodeSid(edge.getDestination())).build();
                    break;
                default:
                    break;
            }
