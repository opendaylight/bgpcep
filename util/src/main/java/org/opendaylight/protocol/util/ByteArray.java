...
public final class ByteArray {
    ...
    public static byte[] subByte(final byte[] bytes, final int startIndex, final int length) {
        Preconditions.checkArgument(checkLength(bytes, length) && checkStartIndex(bytes, startIndex, length), "Cannot create subByte, invalid arguments: Length: " + length + " startIndex: " + startIndex);
    ...
    public static byte[] cutBytes(final byte[] bytes, final int count) {
        Preconditions.checkArgument(bytes.length != 0 && count <= bytes.length && count > 0, "Cannot cut bytes, invalid arguments: Count: " + count + " bytes.length: " + bytes.length);
