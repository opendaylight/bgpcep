...
public final class BitArray {
    ...
    private BitArray(final byte[] backingArray) {
        Preconditions.checkNotNull(backingArray, "Byte Array cannot be null");
        this.backingArray = backingArray;
        this.size = backingArray.length * Byte.SIZE;
    }
    ...
    public void set(final int index, final Boolean value) {
        ...
        this.backingArray[pos] = (byte) (UnsignedBytes.toInt(b) | mask(index, pos));
    }
    ...
    public boolean get(final int index) {
        ...
        final int pos = calculatePosition(index);
        final byte b = this.backingArray[pos];
        return ((byte) (b & mask(index, pos))) == 0 ? false : true;
    }
     ...
     */
    private static int calculateBytes(final int bits) {
        return bits/Byte.SIZE + (bits % Byte.SIZE == 0 ? 0 : 1);
    }
     ...
     */
    private static int calculatePosition(final int index) {
       if (index % Byte.SIZE == 0) {
            return calculateBytes(index);
        } else {
            return calculateBytes(index) - 1;
        }
    }
     ...
     * @param bytePos byte index within byte array
     * @return byte with one bit set
     */
    private static byte mask(final int index, final int bytePos) {
        return (byte)Math.pow(2, index - (Byte.SIZE * bytePos));
    }
    ...
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + Arrays.hashCode(this.backingArray);
    ...
    public boolean equals(final Object obj) {
        ...
        if (!Arrays.equals(this.backingArray, other.backingArray)) {
            return false;
        }
