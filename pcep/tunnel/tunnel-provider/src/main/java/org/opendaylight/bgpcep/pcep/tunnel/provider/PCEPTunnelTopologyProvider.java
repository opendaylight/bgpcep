...
public final class PCEPTunnelTopologyProvider implements AutoCloseable {
    ...
    private PCEPTunnelTopologyProvider(final InstanceIdentifier<Topology> dst,
            final ListenerRegistration<NodeChangedListener> reg) {
    ...
    public static PCEPTunnelTopologyProvider create(final DataBroker dataProvider,
        ...
        final NodeChangedListener ncl = new NodeChangedListener(dataProvider, sourceTopology.firstKeyOf(Topology.class)
                .getTopologyId(), dst);

        final InstanceIdentifier<Node> src = sourceTopology.child(Node.class);
        final ListenerRegistration<NodeChangedListener> reg = dataProvider
