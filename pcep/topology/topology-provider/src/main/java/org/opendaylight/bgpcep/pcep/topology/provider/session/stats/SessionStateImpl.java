...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.controller.pcep.stats.rev171113.pcep.session.state.PeerCapabilities;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.controller.pcep.stats.rev171113.pcep.session.state.PeerCapabilitiesBuilder;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.controller.pcep.stats.rev171113.reply.time.grouping.ReplyTime;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.controller.pcep.stats.rev171113.reply.time.grouping.ReplyTimeBuilder;
...
public final class SessionStateImpl implements PcepSessionState {
    ...
    private boolean lspUpdateCapability;
    private boolean initiationCapability;
    ...
    public synchronized void setStatefulCapabilities(final Stateful stateful) {
        this.stateful = true;
        if (stateful.isLspUpdateCapability() != null) {
            this.lspUpdateCapability = stateful.isLspUpdateCapability();
        }
        final Stateful1 stateful1 = stateful.getAugmentation(Stateful1.class);
        if (stateful1 != null && stateful1.isInitiation() != null) {
            this.initiationCapability = stateful1.isInitiation();
        }
    }

    public synchronized boolean isInitiationCapability() {
        return this.initiationCapability;
    }
    ...
    public synchronized Boolean isSynchronized() {
        return this.session.isSynchronized();
    }
    ...
    private synchronized StatefulCapabilitiesStatsAug createStatefulCapabilities() {
        return new StatefulCapabilitiesStatsAugBuilder()
                .setActive(this.lspUpdateCapability)
                .setInstantiation(isInitiationCapability())
