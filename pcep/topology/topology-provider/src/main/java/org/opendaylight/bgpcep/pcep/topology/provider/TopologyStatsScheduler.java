...
final class TopologyStatsScheduler {
    ...
    TopologyStatsScheduler(final Timer timer, final Executor executor, final long period, final TimeUnit unit) {
        this.timer = requireNonNull(timer);
        this.executor = requireNonNull(executor);
        setPeriod(period, unit);
    }

    void setPeriod(final long period, final TimeUnit unit) {
        final long newPeriodNanos = unit.toNanos(period);
        checkArgument(newPeriodNanos >= 0, "Negative period %s %s", period, unit);
