...
public class PCEPTopologyDeployerImpl implements ClusteredDataTreeChangeListener<Topology>, AutoCloseable {
    ...
    public PCEPTopologyDeployerImpl(final BlueprintContainer container,
        ...
        networTopology = InstanceIdentifier.builder(NetworkTopology.class).build();
    }

    public synchronized void init() {
        LOG.info("PCEP Topology Deployer initialized");
        listenerRegistration = dataBroker.registerDataTreeChangeListener(
            DataTreeIdentifier.create(LogicalDatastoreType.CONFIGURATION, networTopology.child(Topology.class)), this);
    }
    ...
    private void updateTopologyProvider(final Topology topology) {
        ...
        final PCEPTopologyProviderBean previous = pcepTopologyServices.remove(topology.getTopologyId());
    ...
    private void createTopologyProvider(final Topology topology) {
        ...
        final TopologyId topologyId = topology.getTopologyId();
        if (pcepTopologyServices.containsKey(topology.getTopologyId())) {
