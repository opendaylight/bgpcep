...
final class TopologyNodeState implements AutoCloseable, TransactionChainListener {
        ...
        if (retrieveNode) {
            Futures.addCallback(readOperationalData(this.nodeId), new FutureCallback<Optional<Node>>() {
                ...
                public void onSuccess(@Nonnull final Optional<Node> result) {
                    synchronized (TopologyNodeState.this) {
                        if (!result.isPresent()) {
                            putTopologyNode();
                        } else {
    ...
    public synchronized void storeNode(final InstanceIdentifier<Node1> topologyAugment, final Node1 ta,
        ...
        final WriteTransaction trans = this.chain.newWriteOnlyTransaction();
        trans.put(LogicalDatastoreType.OPERATIONAL, topologyAugment, ta, true);

        // All set, commit the modifications
        Futures.addCallback(trans.submit(), new FutureCallback<Void>() {
            @Override
            public void onSuccess(final Void result) {
                LOG.trace("Node stored for session {} updated successfully", session);
            }

            @Override
            public void onFailure(final Throwable throwable) {
                LOG.error("Failed to store node for session {}, terminating it", session, throwable);
