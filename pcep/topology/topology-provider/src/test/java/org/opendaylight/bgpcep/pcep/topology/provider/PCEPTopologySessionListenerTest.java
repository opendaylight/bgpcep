...
public class PCEPTopologySessionListenerTest extends AbstractPCEPSessionTest {
    ...
    public void testPCEPTopologySessionListener() throws Exception {
        ...
        // add-lsp
        rpcMap.getInstance(AddLsp.class).invoke(createAddLspInput());
        ...
        final org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.topology.pcep.rev220730.update.lsp.args
                ...
                .topology.pcep.rev220730.update.lsp.args.ArgumentsBuilder();
        ...
        rpcMap.getInstance(UpdateLsp.class).invoke(update);
        ...
        // ensure-operational
        final org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.topology.pcep.rev220730.ensure.lsp
        ...
        final EnsureLspOperationalInput ensure = new EnsureLspOperationalInputBuilder().setArguments(ensureArgs.build())
                .setName(tunnelName).setNetworkTopologyRef(new NetworkTopologyRef(TOPO_IID))
                .setNode(nodeId).build();
        final OperationResult result = rpcMap.getInstance(EnsureLspOperational.class).invoke(ensure).get().getResult();
        //check result
        ...
        rpcMap.getInstance(RemoveLsp.class).invoke(remove);
    ...
    public void testOnErrorMessage() throws InterruptedException, ExecutionException {
        ...
        listener.onSessionUp(session);
        final Future<RpcResult<AddLspOutput>> futureOutput = rpcMap.getInstance(AddLsp.class)
            .invoke(createAddLspInput());
    ...
    public void testOnServerSessionManagerRestartAndSessionRecovery() throws Exception {
        ...
        // send request
        manager.setRpcTimeout((short) 0);
        final AddLsp addLspRpc = rpcMap.getInstance(AddLsp.class);
    ...
    public void testUpdateUnknownLsp() throws InterruptedException, ExecutionException {
        ...
        final UpdateLspInput update = new UpdateLspInputBuilder().setArguments(updArgsBuilder.build())
                ...
                .build();
        final UpdateLspOutput result = rpcMap.getInstance(UpdateLsp.class).invoke(update).get().getResult();
    ...
    public void testUpdateEmptyLsp() throws InterruptedException, ExecutionException {
        // create LSP
        listener.onSessionUp(session);
        final var addLsp = rpcMap.getInstance(AddLsp.class).invoke(createAddLspInput());
        assertEquals(1, receivedMsgs.size());
        final var pcinitiate =  receivedMsgs.get(0);
        assertThat(pcinitiate, instanceOf(Pcinitiate.class));
        ...
        final var input = new UpdateLspInputBuilder().setArguments(builder.build())
                .setName(tunnelName).setNetworkTopologyRef(new NetworkTopologyRef(TOPO_IID)).setNode(nodeId)
                .build();
        final var result = rpcMap.getInstance(UpdateLsp.class).invoke(input).get().getResult();
        assertEquals(FailureType.Unsent, result.getFailure());
        assertEquals(1, result.getError().size());
        final ErrorObject errorObject = result.getError().get(0).getErrorObject();
        assertNotNull(errorObject);
    ...
    public void testRemoveUnknownLsp() throws InterruptedException, ExecutionException {
        ...
        final OperationResult result = rpcMap.getInstance(RemoveLsp.class).invoke(remove).get().getResult();
        assertEquals(FailureType.Unsent, result.getFailure());
        assertEquals(1, result.getError().size());
        final ErrorObject errorObject = result.getError().get(0).getErrorObject();
        assertNotNull(errorObject);
    ...
    public void testAddAlreadyExistingLsp() throws InterruptedException, ExecutionException {
        listener.onSessionUp(session);
        final var addLspRpc = rpcMap.getInstance(AddLsp.class);
        addLspRpc.invoke(createAddLspInput());
        assertEquals(1, receivedMsgs.size());
        final var pcinitiate =  receivedMsgs.get(0);
        assertThat(pcinitiate, instanceOf(Pcinitiate.class));
    ...
    public void testPccResponseTimeout() throws Exception {
        listener.onSessionUp(session);
        final Future<RpcResult<AddLspOutput>> addLspResult = rpcMap.getInstance(AddLsp.class)
            .invoke(createAddLspInput());
