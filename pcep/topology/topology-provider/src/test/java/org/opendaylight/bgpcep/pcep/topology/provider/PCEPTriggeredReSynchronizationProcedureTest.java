...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.topology.pcep.rev250602.TriggerSyncInputBuilder;
            ...
            .setTlvs(new org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.object.rev250930.lsp
                .object.lsp.TlvsBuilder()
                    .setLspIdentifiers(new LspIdentifiersBuilder()
                        ...
                        .build())
                    .setSymbolicPathName(
                        new SymbolicPathNameBuilder().setPathName(new SymbolicPathName("test".getBytes()))
                        .build())
                    .setLspDbVersion(new LspDbVersionBuilder().setLspDbVersionValue(Uint64.ONE).build())
                    .build())
            .setLspFlags(new LspFlagsBuilder()
                .setSync(true).setRemove(false)
                .setOperational(OperationalStatus.Active)
                .build())
            .build(),
            Optional.empty(), createPath(Collections.emptyList()));
    }
}
