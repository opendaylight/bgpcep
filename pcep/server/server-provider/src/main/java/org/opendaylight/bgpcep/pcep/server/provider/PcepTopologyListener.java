...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.server.rev210720.managed.path.managed.node.te.path.intended.path.ConstraintsBuilder;
...
public final class PcepTopologyListener implements DataTreeChangeListener<Node>, AutoCloseable {
    ...
    public PcepTopologyListener(final DataBroker dataBroker, KeyedInstanceIdentifier<Topology, TopologyKey> topology,
        ...
        LOG.info("Registered PCE Server listener {} for PCEP Topology {}", listenerRegistration,
    ...
    public void close() {
        if (this.listenerRegistration != null) {
            LOG.info("Unregistered PCE Server listener {} for PCEP Topology", listenerRegistration);
        ...
        for (DataObjectModification<? extends DataObject> lsp : lspMod) {
            ...
            switch (lsp.getModificationType()) {
                case DELETE:
                    ...
                    pathManager.unregisterTePath(nodeId, new TePathKey(rptLsp.getName()));
                    break;
                case SUBTREE_MODIFIED:
                case WRITE:
                    rptLsp = (ReportedLsp) lsp.getDataAfter();
                    LOG.debug("Register Managed TE Path {}", rptLsp.getName());
                    pathManager.registerTePath(nodeId,  getTePath(rptLsp));
                    break;
                default:
                    break;
            }
    ...
    /**
     * Translate Segment Routing Path a.k.a. ERO into Path Description.
     *
    ...
    /**
     * Translate RSVP-TE Path a.k.a. ERO into Path Description.
     *
     ...
     * @return      new TE Path.
     */
    private static TePath getTePath(ReportedLsp rl) {
        ...
        if (pathDesc != null) {
            apb.setPathDescription(pathDesc);
        }

        /* Finally build TE Path */
        return new TePathBuilder()
                .setName(rl.getName())
                .setPathType(ptype)
