...
public class ManagedTePath {
    ...
    public PathStatus checkReportedPath(final TePath tePath) {
        ...
        /*
         * Check constraints ...
         * Note that TE metric and Delay are excluded as, at least, they are not supported by Juniper Routers.
         */
    ...
    public ListenableFuture<RpcResult<RemoveLspOutput>> removePath(final NetworkTopologyPcepService ntps) {
        ...
        sent = true;
        final NodeId id = teNode.getId();
        final ListenableFuture<RpcResult<RemoveLspOutput>> enforce = ntps
                .removeLsp(new RemoveLspInputBuilder().setNode(id).setName(path.getName()).build());
     ...
     *
     * @param nodeId    Node ID which this TE Path belongs to
     */
