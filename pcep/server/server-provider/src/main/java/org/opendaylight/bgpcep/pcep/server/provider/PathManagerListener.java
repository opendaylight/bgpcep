...
public final class PathManagerListener
    ...
    @Inject
    @Activate
    public PathManagerListener(@Reference final DataBroker dataBroker, @Reference final PceServerProvider pceServer) {
        // this.dataBroker = requireNonNull(dataBroker);
        requireNonNull(pceServer);
        this.pathMan = requireNonNull(pceServer.getPathManager());
        ...
        LOG.info("Registered listener {} for Managed Path at {}", this, MANAGED_NODE_IDENTIFIER);
    }
        ...
        for (DataTreeModification<ManagedNode> change : changes) {
            ...
            switch (root.getModificationType()) {
                case DELETE:
                    ...
                    pathMan.deleteManagedNode(node);
                    break;
                case SUBTREE_MODIFIED:
                case WRITE:
                    ...
                    if (!pathMan.existManagedNode(node.getNodeId())) {
                        LOG.info("Create new Managed Node {}", node.getNodeId());
                        pathMan.addManagedNode(node);
                    } else {
