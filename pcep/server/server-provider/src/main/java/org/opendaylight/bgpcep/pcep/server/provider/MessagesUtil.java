...
public final class MessagesUtil {
     ...
     */
    public static final int IGP_METRIC = 0;
    public static final int TE_METRIC = 1;
    ...
    private static PathsBuilder buildPath(ConstrainedPath cpath) {
        ...
        for (PathDescription path : cpath.getPathDescription()) {
            ...
            if (path.getIpv4() != null) {
                final Ipv4Prefix ipv4Pref = new Ipv4Prefix(path.getIpv4().getValue());
            ...
            if (path.getIpv6() != null) {
                final Ipv6Prefix ipv6Pref = new Ipv6Prefix(path.getIpv6().getValue());
                ipPref = new IpPrefixBuilder().setIpPrefix(
                        new org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.inet.types.rev130715.IpPrefix(
                                ipv6Pref))
                        .build();
            ...
            if (ipPref != null) {
                ...
                sb = new SubobjectBuilder()
                        .setSubobjectType(ipPrefCase)
                        .setLoose(false)
                        .build();
            }
            if (labelCase != null) {
                sb = new SubobjectBuilder()
                        .setSubobjectType(labelCase)
                        .setLoose(false)
                        .build();
            }
            if (sb != null) {
        ...
        pathBuilder.setEro(eroBuilder.build());
        ...
        if (cpath.getMetric() != null) {
            ...
            metricBuilder.setComputed(true);
            metricBuilder.setMetricType(Uint8.valueOf(IGP_METRIC));
            metricBuilder.setValue(new Float32(
        ...
        if (cpath.getTeMetric() != null) {
            final MetricBuilder metricBuilder = new MetricBuilder();
            metricBuilder.setComputed(true);
            metricBuilder.setMetricType(Uint8.valueOf(TE_METRIC));
            metricBuilder.setValue(new Float32(
        ...
        if (cpath.getDelay() != null) {
            final MetricBuilder metricBuilder = new MetricBuilder();
            metricBuilder.setComputed(true);
            metricBuilder.setMetricType(Uint8.valueOf(PATH_DELAY));
            metricBuilder.setValue(new Float32(
                    ByteBuffer.allocate(4).putFloat(Float.valueOf(cpath.getDelay().getValue().floatValue())).array()));
            metrics.add(new MetricsBuilder().setMetric(metricBuilder.build()).build());
        }
        if (!metrics.isEmpty()) {
        ...
        if (cpath.getBandwidth() != null) {
            ...
            bwBuilder.setBandwidth(new Bandwidth(new Float32(
                    ByteBuffer.allocate(4).putFloat(Float.valueOf(cpath.getBandwidth().floatValue())).array())));
    ...
    public static Pcrep createNoPathMessage(Rp rp, byte reason) {
        ...
        npBuilder.setNatureOfIssue(Uint8.ZERO);
        npBuilder.setUnsatisfiedConstraints(true);
        final NoPathVectorBuilder npvBuilder = new NoPathVectorBuilder();
        ...
        npvBuilder.setFlags(flags);
        final TlvsBuilder tlvsBuilder = new TlvsBuilder();
        tlvsBuilder.setNoPathVector(npvBuilder.build());
        npBuilder.setTlvs(tlvsBuilder.build());
        final NoPath noPath = npBuilder.build();

        /* Prepare Reply */
        final RepliesBuilder replyBuilder = new RepliesBuilder();
        replyBuilder.setRp(rp);
        replyBuilder.setResult(new FailureCaseBuilder().setNoPath(noPath).build());
    ...
    public static Pcerr createErrorMsg(@NonNull final PCEPErrors pcepErrors, final Uint32 reqID) {
        ...
        return new PcerrBuilder()
                .setPcerrMessage(
                        msgBuilder
                                .setErrorType(new RequestCaseBuilder().setRequest(new RequestBuilder()
                                        .setRps(Lists.newArrayList(new RpsBuilder()
                                                .setRp(new RpBuilder().setProcessingRule(false).setIgnore(false)
                                                        .setRequestId(new RequestId(reqID)).build())
                                                .build()))
                                        .build()).build())
                                .setErrors(Collections.singletonList(new ErrorsBuilder()
                                        .setErrorObject(new ErrorObjectBuilder().setType(pcepErrors.getErrorType())
                                                .setValue(pcepErrors.getErrorValue()).build())
                                        .build()))
                                .build())
                .build();
    }
