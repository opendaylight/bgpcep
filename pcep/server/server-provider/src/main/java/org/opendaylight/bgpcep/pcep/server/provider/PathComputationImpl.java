            ...
            for (Metrics metric : metrics) {
                ...
                switch (metric.getMetric().getMetricType().intValue()) {
                    case MessagesUtil.IGP_METRIC:
                        ...
                        LOG.info("Read IGP Metric value {} from {} and convert it to {}", convert, metric,
                                ctsBuilder.getMetric());
                        break;
                    case MessagesUtil.TE_METRIC:
                        ctsBuilder.setTeMetric(Uint32.valueOf(convert.longValue()));
                        LOG.info("Read TE Metric value {} from {} and convert it to {}", convert, metric,
                                ctsBuilder.getTeMetric());
                        break;
                    case MessagesUtil.PATH_DELAY:
                        ctsBuilder.setDelay(new Delay(Uint32.valueOf(convert.longValue())));
                        LOG.info("Read Delay Metric value {} from {} and convert it to {}", convert, metric,
                                ctsBuilder.getDelay());
                        break;
                    default:
                        LOG.info("Metric {} is not handle by Path Computation Constraints", metric);
                        break;
                }
            }
        }
