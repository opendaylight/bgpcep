...
public class SimpleVendorInformationObjectRegistry implements VendorInformationObjectRegistry {
    ...
    @Override
    public Optional<? extends Object> parseVendorInformationObject(final long enterpriseNumber, final ObjectHeader header, final ByteBuf buffer)
            throws PCEPDeserializerException {
        Preconditions.checkArgument(enterpriseNumber >= 0);
        final ObjectParser parser = this.handlers.getParser(Ints.checkedCast(enterpriseNumber));
        if (parser == null) {
