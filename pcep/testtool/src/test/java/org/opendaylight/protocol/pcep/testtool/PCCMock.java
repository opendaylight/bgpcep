...
public class PCCMock<M extends ProtocolMessage, S extends ProtocolSession<M>, L extends SessionListener<M, ?, ?>> extends
	...
	public PCCMock(final SessionNegotiatorFactory<M, S, L> negotiatorFactory, final L listener, final ProtocolHandlerFactory<?> factory,
			final DefaultPromise<PCEPSessionImpl> defaultPromise) {
		this.negotiatorFactory = Preconditions.checkNotNull(negotiatorFactory);
		this.listenerFactory = new SessionListenerFactory<L>() {

			@Override
			public L getSessionListener() {
				return listener;
			}
		};
	...
	public static void main(final String[] args) throws Exception {
		...
		final PCCMock<PCEPMessage, PCEPSessionImpl, PCEPSessionListener> pcc = new PCCMock<>(snf, new SimpleSessionListener(), new PCEPHandlerFactory(), new DefaultPromise<PCEPSessionImpl>(GlobalEventExecutor.INSTANCE));

		pcc.createClient(new InetSocketAddress("127.0.0.3", 12345), new NeverReconnectStrategy(GlobalEventExecutor.INSTANCE, 2000)).get();
	}
}
