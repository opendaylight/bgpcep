...
public class PCCEndPointIpv4ObjectParserTest {
    ...
    private final static String IP_1 = "1.2.3.4";
    private final static String IP_2 = "1.2.3.5";
    ...
    public void testParseObject() throws PCEPDeserializerException {
        final ObjectHeader header = new ObjectHeaderImpl(false, false);
        final ByteBuf bytes = Unpooled.buffer();
        bytes.writeBytes(Ipv4Util.bytesForAddress(new Ipv4Address(IP_1)));
        bytes.writeBytes(Ipv4Util.bytesForAddress(new Ipv4Address(IP_2)));
        final EndpointsObj output = (EndpointsObj) new PCCEndPointIpv4ObjectParser().parseObject(header, bytes);

        assertEquals(IP_1, ((Ipv4Case) output.getAddressFamily()).getIpv4().getSourceIpv4Address().getValue());
        assertEquals(IP_2, ((Ipv4Case) output.getAddressFamily()).getIpv4().getDestinationIpv4Address().getValue());
