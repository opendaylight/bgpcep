...
public final class MsgBuilderUtil {
    ...
    public static Path updToRptPath(final org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.message
            .rev250930.pcupd.message.pcupd.message.updates.Path path) {
    ...
    public static Tlvs createLspTlvs(final Uint32 lspId, final boolean symbolicPathName, final String tunnelEndpoint,
        ...
        lspDBVersion.ifPresent(dbVersion -> {
            tlvs.setLspDbVersion(new LspDbVersionBuilder().setLspDbVersionValue(dbVersion).build());
        });
        return tlvs.build();
    }
