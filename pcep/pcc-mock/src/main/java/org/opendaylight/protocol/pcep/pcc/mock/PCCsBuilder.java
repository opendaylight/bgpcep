...
final class PCCsBuilder {
    ...
    public PCCsBuilder(@Nonnull final int lsps, @Nonnull final boolean pcError, @Nonnull final int pccCount, @Nonnull final InetSocketAddress localAddress,
                       @Nonnull final List<InetSocketAddress> remoteAddress, @Nonnull final short keepAlive, @Nonnull final short deadTimer,
                       @Nonnull final String password, @Nonnull final long reconnectTime, @Nonnull final int redelegationTimeout, @Nonnull final int stateTimeout,
                       @Nonnull final BigInteger syncOptDBVersion, @Nonnull final StateSyncOpt stateSyncOpt, @Nonnull final Integer disconnectAfter,
                       @Nonnull final Integer reconnectAfter) {
        this.lsps = lsps;
    ...
    private void createPCCs(@Nonnull final BigInteger initialDBVersion) throws InterruptedException, ExecutionException {
        ...
        if (this.stateSyncOpt.equals(StateSyncOpt.AvoidanceProcedure) || this.stateSyncOpt.equals(StateSyncOpt.IncrementalProcedure)) {
    ...
    private void createPCC(@Nonnull final InetSocketAddress localAddress, @Nonnull final PCCTunnelManager tunnelManager,
                           @Nonnull final BigInteger initialDBVersion) throws InterruptedException, ExecutionException {
    ...
    private PCEPSessionNegotiatorFactory<PCEPSessionImpl> getSessionNegotiatorFactory() {
        Boolean includeDbVersion = Boolean.FALSE;
        Boolean incrementalSync = Boolean.FALSE;
        if (this.stateSyncOpt.equals(StateSyncOpt.AvoidanceProcedure)) {
            includeDbVersion = Boolean.TRUE;
        } else if(this.stateSyncOpt.equals(StateSyncOpt.IncrementalProcedure)) {
            includeDbVersion = Boolean.TRUE;
            incrementalSync = Boolean.TRUE;
        }
        PCEPCapability STATEFUL_CAPABILITY = new PCEPStatefulCapability(true, true, true, false, false, incrementalSync , includeDbVersion);
