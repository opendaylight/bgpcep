...
public class PCCTunnelManagerImpl implements PCCTunnelManager {
    ...
    @GuardedBy("this")
    private Map<PlspId, PCCTunnel> tunnels = new HashMap<>();
    ...
    public synchronized void onSessionUp(final PCCSession session) {
        ...
        if (this.syncOptimization.isSyncAvoidanceEnabled() && !dbV.equals(BigInteger.ONE)) {
            this.tunnels = PCCTunnelBuilder.createTunnels(this.address, dbV.intValue());
        } else {
            this.tunnels = PCCTunnelBuilder.createTunnels(this.address, this.lspsCount);
        }
    ...
    public void onMessagePcupd(@Nonnull final Updates update, @Nonnull final PCCSession session) {
        if (isTriggeredSync(update.getLsp())) {
            lspReport(session);
        } else if (this.syncOptimization.isTriggeredReSyncEnabled() && update.getLsp().isSync()) {
            reportLsp(update.getLsp().getPlspId(), update.getSrp().getOperationId(), session);
    ...
    private void reportMissedLsp(final PCCSession session) {
        for (long missedLsp = this.syncOptimization.getRemoteLspDbVersionValue().longValue() + 1;
            ...
            final PlspId plspId = new PlspId(Long.valueOf(missedLsp));
            final PCCTunnel tunnel = this.tunnels.get(plspId);
            ...
            final Tlvs tlvs = createLspTlvs(missedLsp, true, destinationAddress, this.address, this.address, Optional.of(tunnel.getPathName()),
                this.syncOptimization.incrementLspDBVersion());

            Lsp lsp = createLsp(missedLsp, this.syncOptimization.isSyncNeedIt(), Optional.fromNullable(tlvs), delegation, false);
    ...
    private void reportAllKnownLsp(final PCCSession session) {
        ...
        for (final Entry<PlspId, PCCTunnel> entry : this.tunnels.entrySet()) {
           ...
           /* if (synchronizing && this.syncOptimization.isSyncAvoidanceEnabled()) {
            ...
            Lsp lsp = createLsp(plspId, this.syncOptimization.isSyncNeedIt(), Optional.fromNullable(tlvs), delegation, false);
            final Pcrpt pcrtp = createPcRtpMessage(lsp, NO_SRP, tunnel.getLspState());
            session.sendReport(pcrtp);
        }
    }
    ...
    private void reportLsp(final PlspId plspId, final SrpIdNumber operationId, final PCCSession session) {
        ...
        final String destinationAddress = getDestinationAddress(tunnel.getLspState().getEro().getSubobject(), this.address);
        final Optional<BigInteger> lspDBV = this.syncOptimization.incrementLspDBVersion(); //???List of lsp
        final Tlvs tlvs = createLspTlvs(plspId.getValue(), true, destinationAddress, this.address, this.address, Optional.of(tunnel.getPathName()), lspDBV);

        Lsp lsp = createLsp(plspId.getValue(), false, Optional.fromNullable(tlvs), delegation, false);

        final Srp srp = new SrpBuilder().setOperationId(operationId).build();
        final Pcrpt pcrtp = createPcRtpMessage(lsp, Optional.of(srp), tunnel.getLspState());
        session.sendReport(pcrtp);
    }
                ...
                for (int i = 1; i < PCCTunnelManagerImpl.this.sessions.size(); i++) {
                    ...
                    if (nextSession != null) {
                        ...
                        final Tlvs tlvs = createLspTlvs(plspId.getValue(), true,
                            ...
                            syncOptimization.incrementLspDBVersion());
