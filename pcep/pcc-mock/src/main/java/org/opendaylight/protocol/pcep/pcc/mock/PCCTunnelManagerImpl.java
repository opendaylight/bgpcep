...
public class PCCTunnelManagerImpl implements PCCTunnelManager {
    ...
    public synchronized void onSessionUp(final PCCSession session) {
        ...
        if (!this.tunnels.isEmpty() && !this.syncOptimization.isSyncAvoidanceEnabled()) {
            ...
            handleAvoidSynchronization(session);
        }
    }

    private void handleAvoidSynchronization(final PCCSession session) {
        if(this.syncOptimization.isDeltaSyncEnabled() && !this.syncOptimization.doesLspDbMatch()) {
    ...
    private void reportMissedLsp(final PCCSession session) {
        for (long key = this.syncOptimization.getRemoteLspDbVersionValue().longValue();
