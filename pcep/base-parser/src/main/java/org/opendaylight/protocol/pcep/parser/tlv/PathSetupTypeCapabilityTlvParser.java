...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev250602.path.setup.type.capability.tlv.path.setup.type.capability.SubTlvs;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev250602.path.setup.type.capability.tlv.path.setup.type.capability.SubTlvsBuilder;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev250602.sr.pce.capability.tlv.SrPceCapability;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev250602.srv6.pce.capability.tlv.Srv6PceCapability;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev250602.srv6.pce.capability.tlv.srv6.pce.capability.Msds;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev250602.srv6.pce.capability.tlv.srv6.pce.capability.MsdsBuilder;
...
public class PathSetupTypeCapabilityTlvParser implements TlvParser, TlvSerializer {
    ...
    public void serializeTlv(final Tlv tlv, final ByteBuf buffer) {
        ...
        body.writeByte(pstCapability.getPstNumber().byteValue());
        pstCapability.getPsts().forEach(pst -> body.writeByte(pst.getIntValue()));
        // Write padding if necessary
        body.writeZero(TlvUtil.getPadding(pstCapability.getPstNumber().intValue(), TlvUtil.PADDED_TO));
        if (pstCapability.getSubTlvs() != null) {
            if (pstCapability.getSubTlvs().getSrPceCapability() != null) {
                serializeSrPceCapability(pstCapability.getSubTlvs().getSrPceCapability(), body);
            }
            if (pstCapability.getSubTlvs().getSrv6PceCapability() != null) {
                serializeSrv6PceCapability(pstCapability.getSubTlvs().getSrv6PceCapability(), body);
            }
        }
    ...
    public Tlv parseTlv(final ByteBuf buffer) throws PCEPDeserializerException {
        ...
        return new PathSetupTypeCapabilityBuilder()
            .setPstNumber(Uint8.valueOf(numPST))
            .setPsts(psts)
            .setSubTlvs(parsePstSubTlvs(buffer, psts))
            .build();
    }

    private SubTlvs parsePstSubTlvs(final ByteBuf buffer, final List<PsType> psts) throws PCEPDeserializerException {
        final var builder = new SubTlvsBuilder();
        while (buffer.isReadable()) {
            final int type = buffer.readUnsignedShort();
            final int length = buffer.readUnsignedShort();
            switch (type) {
                case SR_PCE -> builder.setSrPceCapability(parseSrPceCapability(buffer, psts));
                case SRV6_PCE -> builder.setSrv6PceCapability(parseSrv6PceCapability(buffer, psts, length));
