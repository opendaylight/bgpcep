...
final class Stateful07TopologySessionListener extends AbstractTopologySessionListener<SrpIdNumber, PlspId> {
    ...
    public synchronized ListenableFuture<OperationResult> updateLsp(final UpdateLspArgs input) {
        ...
        if (inputLsp != null) {
            rb.setLsp(new LspBuilder().setPlspId(reportedLsp.getPlspId()).setDelegate(inputLsp.isDelegate()).setTlvs(inputLsp.getTlvs()).setAdministrative(inputLsp.isAdministrative()).build());
        } else {
