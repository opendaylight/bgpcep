...
public abstract class AbstractTopologySessionListener<S, L> implements PCEPSessionListener, TopologySessionListener, ListenerStateRuntimeMXBean {
    ...
    private synchronized void tearDown(final PCEPSession session) {
        ...
        // do not send close message when close the session listener
        // the message may have been sent before or we are not able to send it
        close(false);
    }
     ...
     *
     * @param closeSession Whether to send close message to PCEP peer actively when closing
     */
    private final synchronized void close(final boolean closeSession) {
        unregister();
    ...
    private final synchronized void unregister() {
        if (this.registration != null) {
            ...
            LOG.debug("PCEP session {} is unregistered successfully.", getPeerId());
            this.registration = null;
        } else {
            LOG.debug("PCEP session {} was not registered.", getPeerId());
        }
    }
    ...
    private final synchronized void register() {
        ...
        if (runtimeReg != null) {
            ...
            LOG.debug("PCEP session {} is successfully registered.", getPeerId());
        }
    }
