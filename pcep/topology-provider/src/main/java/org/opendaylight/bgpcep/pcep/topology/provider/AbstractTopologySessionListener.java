...
public abstract class AbstractTopologySessionListener<S, L> implements PCEPSessionListener, TopologySessionListener, ListenerStateRuntimeMXBean {
    ...
    public final synchronized void onSessionUp(final PCEPSession session) {
        ...
        if (state == null) {
            LOG.error("Unable to fetch topology node state for PCEP session. Terminating it to reset session status. {}", session);
            session.terminate(TerminationReason.UNKNOWN);
            return;
        }

        if (this.session != null || this.nodeState != null) {
            LOG.error("PCEP session is already up. Terminating it to reset session status. {}", session);
            session.terminate(TerminationReason.UNKNOWN);
            return;
        }
        ...
        if (this.registration == null) {
            LOG.error("PCEP session fails to register. Terminating it to reset session status. {}", session);
            session.terminate(TerminationReason.UNKNOWN);
            return;
        }
