...
public class Stateful07TopologySessionListenerTest extends AbstractPCEPSessionTest<Stateful07TopologySessionListenerFactory> {
    ...
    public void testStateful07TopologySessionListener() throws Exception {
        ...
        final Tlvs tlvs = createLspTlvs(req.getLsp().getPlspId().getValue(), true,
            TEST_ADDRESS, TEST_ADDRESS, TEST_ADDRESS, Optional.<byte[]>absent(), Optional.<BigInteger>absent());
        final Pcrpt pcRpt = MsgBuilderUtil.createPcRtpMessage(
            new LspBuilder(req.getLsp()).setTlvs(tlvs).setPlspId(new PlspId(1L))
                .setSync(false).setRemove(false).setOperational(OperationalStatus.Active).build(),
        ...
        final Tlvs tlvs2 = createLspTlvs(upd.getLsp().getPlspId().getValue(), false,
                NEW_DESTINATION_ADDRESS, TEST_ADDRESS, TEST_ADDRESS, Optional.<byte[]>absent(), Optional.<BigInteger>absent());
        final Pcrpt pcRpt2 = MsgBuilderUtil.createPcRtpMessage(new LspBuilder(upd.getLsp()).setTlvs(tlvs2).setSync(true).setRemove(false).
            setOperational(OperationalStatus.Active).build(), Optional.of(MsgBuilderUtil.createSrp(srpId2)),
            MsgBuilderUtil.createPath(upd.getPath().getEro().getSubobject()));
