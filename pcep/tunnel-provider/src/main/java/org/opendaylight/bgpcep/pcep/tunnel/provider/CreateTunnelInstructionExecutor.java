...
public final class CreateTunnelInstructionExecutor extends AbstractInstructionExecutor {
    private final DataBroker dataProvider;
    private final NetworkTopologyPcepService topologyService;
    private final PcepCreateP2pTunnelInput input;

    protected CreateTunnelInstructionExecutor(final PcepCreateP2pTunnelInput input, final DataBroker dataProvider, final NetworkTopologyPcepService topologyService) {
        super(input);
        this.input = input;
    ...
    private AddLspInput createAddLspInput(final ReadOnlyTransaction transaction) {
        ...
        final TpReader dr = new TpReader(transaction, tii, input.getDestination());
        final TerminationPoint dp = Preconditions.checkNotNull(dr.getTp());

        final TpReader sr = new TpReader(transaction, tii, input.getSource());
    ...
    private void checkLinkIsnotExistent(final InstanceIdentifier<Topology> tii, final AddLspInputBuilder ab, final ReadOnlyTransaction t) {
    ...
    private Arguments buildArguments(final PcepCreateP2pTunnelInput input, final TerminationPoint sp, final TerminationPoint dp) {
        ...
        args.setEro(TunelProgrammingUtil.buildEro(input.getExplicitHops()));
