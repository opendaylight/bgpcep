...
public final class PCEPTlvParser implements TlvParser {
	 ...
	 */
	private enum PCEPTlvType {
		NO_PATH_VECTOR(1), OVERLOADED_DURATION(2), REQ_MISSING(3), OF_LIST_TLV(4), ORDER_TLV(5), P2MP_CAPABILITY(6), PCE_STATEFUL_CAPABILITY(
				16), LSP_SYMBOLIC_NAME(17), LSP_IDENTIFIER_IPV4(18), LSP_IDENTIFIER_IPV6(19), LSP_UPDATE_ERROR(20), RSVP_ERROR_SPEC_IPV4(21), RSVP_ERROR_SPEC_IPV6(
				22), LSP_STATE_DB_VERSION(23),
		// TODO: use IANA defined number - for now has been used first unused
		// number
		NODE_IDENTIFIER(24), LSP_CLEANUP_TLV(26);
	...
	public static Tlv parse(final byte[] bytes) throws PCEPDeserializerException {
		if (bytes == null)
	...
	public static byte[] put(final PCEPTlv objToSerialize) {
		...
		if (objToSerialize instanceof PCEStatefulCapabilityTlv) {
			...
			System.arraycopy(ByteArray.longToBytes(((ReqMissingTlv) objToSerialize).getRequestID()), Long.SIZE / Byte.SIZE - REQ_ID_LENGTH,
					valueBytes, 0, REQ_ID_LENGTH);
