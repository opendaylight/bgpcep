...
public class PCEPOpenMessageParser extends AbstractMessageParser {
	...
	public PCEPOpenMessageParser(HandlerRegistry registry) {
		super(registry);
	}
	...
	public void serializeMessage(Message message, ByteBuf buffer) {
		if (!(message instanceof OpenMessage))
			throw new IllegalArgumentException("Wrong instance of Message. Passed instance " + message.getClass()
					+ ". Nedded OpenMessage.");
		org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev131005.open.message.OpenMessage open = ((OpenMessage) message).getOpenMessage();
	...
	@Override
	public OpenMessage parseMessage(byte[] buffer) throws PCEPDeserializerException {
		if (buffer == null || buffer.length == 0) {
			throw new PCEPDeserializerException("Open message doesn't contain OPEN object.");
		}
		List<Object> objs = parseObjects(buffer);
			
		return new org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.message.rev131007.OpenBuilder().setOpenMessage(validate(objs)).build();
	}
	
	private org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev131005.open.message.OpenMessage validate(final List<Object> objects) throws PCEPDeserializerException {
		if (objects == null)
		...
		final org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev131005.open.message.OpenMessage msg = new OpenMessageBuilder().setOpen((Open) objects.get(0)).build();
