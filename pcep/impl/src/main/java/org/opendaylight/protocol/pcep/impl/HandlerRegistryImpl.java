...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.rsvp.rev130820.Subobject;
...
public class HandlerRegistryImpl implements HandlerRegistry {
	...
	static {
		...
		reg.registerObjectParser(PCEPEndPointsObjectParser.CLASS, PCEPEndPointsObjectParser.TYPE,
				new PCEPEndPointsObjectParser(reg));
		reg.registerObjectParser(PCEPEndPointsObjectParser.CLASS_6, PCEPEndPointsObjectParser.TYPE_6,
				new PCEPEndPointsObjectParser(reg));
		...
		reg.registerObjectParser(PCEPExcludeRouteObjectParser.CLASS, PCEPExcludeRouteObjectParser.TYPE, new PCEPExcludeRouteObjectParser(reg));
	...
	private final Map<Class<? extends Subobject>, SubobjectSerializer> subobjectSerializers = new ConcurrentHashMap<>();
	...
	@Override
	public SubobjectSerializer getSubobjectSerializer(final Subobject subobject) {
		final Class<? extends Subobject> c = subobject.getClass();
		for (final Map.Entry<Class<? extends Subobject>, SubobjectSerializer> s : this.subobjectSerializers.entrySet()) {
			if (s.getKey().isAssignableFrom(c)) {
				return s.getValue();
			}
		}
	...
	@Override
	public AutoCloseable registerSubobjectSerializer(final Class<? extends Subobject> subobjectClass, final SubobjectSerializer serializer) {
		Preconditions.checkArgument(!this.tlvSerializers.containsKey(subobjectClass), "Subobject class %s already registered", subobjectClass);
	...
	private synchronized void unregisterSubobjectSerializer(final Class<? extends Subobject> subobjectClass) {
	...
	public AutoCloseable registerSubobjectParser(final int subobjectType, final SubobjectParser parser) {
		Preconditions.checkArgument(subobjectType >= 0 && subobjectType <= 65535);
		Preconditions.checkArgument(!this.subobjectParsers.containsKey(subobjectType), "Subobject type %s already registered", subobjectType);
