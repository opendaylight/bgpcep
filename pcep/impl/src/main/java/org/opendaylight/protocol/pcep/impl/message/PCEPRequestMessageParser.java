...
public class PCEPRequestMessageParser extends AbstractMessageParser {
    ...
    private SvecState insertObject(SvecState state, final Object obj, final SvecBuilder builder, final List<Metrics> metrics, final List<VendorInformationObject> viObjects) {
        switch (state) {
        case INIT:
            if (obj instanceof Of) {
                builder.setOf((Of) obj);
