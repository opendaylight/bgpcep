...
public class PCEPNotificationMessageParser extends AbstractMessageParser {
	...
	public PCEPNotificationMessageParser(HandlerRegistry registry) {
		super(registry);
	}
	...
	public void serializeMessage(Message message, ByteBuf buffer) {
		if (!(message instanceof PcntfMessage))
			throw new IllegalArgumentException("Wrong instance of Message. Passed instance of " + message.getClass()
					+ ". Needed PcntfMessage.");
		org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev131005.pcntf.message.PcntfMessage msg = ((PcntfMessage) message).getPcntfMessage();
		
		for (org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev131005.pcntf.message.pcntf.message.Notifications n : msg.getNotifications()) {
			...
			if (n.getNotifications() == null || n.getNotifications().isEmpty()) {
				...
				for (Notifications not : n.getNotifications()) {
	...
	@Override
	public Message parseMessage(byte[] buffer) throws PCEPDeserializerException {
	...
	public Message validate(final List<Object> objects) throws PCEPDeserializerException {
		...
		while (!objects.isEmpty()) {
			...
			try {
				...
				b.setErrors(Arrays.asList(new ErrorsBuilder().setType(maping.getFromErrorsEnum(e.getError()).type).setValue(maping.getFromErrorsEnum(e.getError()).value).build()));
	...
	private static org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.pcep.types.rev131005.pcntf.message.pcntf.message.Notifications getValidNotificationComposite(final List<Object> objects) throws PCEPDocumentedException {
		...
		while (!objects.isEmpty()) {
			...
			case 2:
				if (obj instanceof NotificationObject) {
					NotificationObject n = (NotificationObject) obj;
