...
//TODO use capital letter in variables names - see PCEPEROSubobjectParserTest.java for example
    ...
    public void testXROIp4PrefixSubobject() throws PCEPDeserializerException {
        ...
        assertEquals(
            subs.build(), parser.parseSubobject(Unpooled.wrappedBuffer(ByteArray.cutBytes(ip4PrefixBytes, 2)), false));
    ...
    public void testXROIp6PrefixSubobject() throws PCEPDeserializerException {
        ...
        assertEquals(
            subs.build(), parser.parseSubobject(Unpooled.wrappedBuffer(ByteArray.cutBytes(ip6PrefixBytes, 2)), true));
        final ByteBuf buff = Unpooled.buffer();
        parser.serializeSubobject(subs.build(), buff);
    ...
    public void testXROUnnumberedSubobject() throws PCEPDeserializerException {
        ...
        assertEquals(
            subs.build(), parser.parseSubobject(Unpooled.wrappedBuffer(ByteArray.cutBytes(unnumberedBytes, 2)), true));
        final ByteBuf buff = Unpooled.buffer();
        parser.serializeSubobject(subs.build(), buff);
