...
public final class KeyMapping extends HashMap<InetAddress, byte[]> {
    ...
    public static KeyMapping getKeyMapping(@Nonnull final InetAddress inetAddress, @Nullable final Optional<Rfc2385Key> password){
        if (password.isPresent()) {
            final KeyMapping keyMapping = new KeyMapping();
            keyMapping.put(inetAddress, password.get().getValue().getBytes(Charsets.US_ASCII));
            return keyMapping;
        }
        return null;
    }
