...
public class NetconfClientSession extends NetconfSession {
	...
	public NetconfClientSession(SessionListener sessionListener, Channel channel, long sessionId, Collection<String> capabilities) {
	...
	protected void handleMessage(NetconfMessage netconfMessage) {
		logger.debug("Client Session {} received message {}", toString(), Xml.toString(netconfMessage.getDocument()));
	...
	protected void endOfInput() {
		logger.debug("Client Session {} end of input detected while session was in state {}", toString(), isUp() ? "up" : "initialized");
		if (isUp()) {
