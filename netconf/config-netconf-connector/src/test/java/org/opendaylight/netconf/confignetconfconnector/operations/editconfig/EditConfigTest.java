...
import org.opendaylight.netconf.confignetconfconnector.mapping.config.InstanceConfigElementResolved;
...
import org.opendaylight.netconf.confignetconfconnector.operations.editconfig.EditConfigXmlParser.EditConfigExecution;
...
public class EditConfigTest {
	...
	public void test() throws NetconfDocumentedException {
		EditConfig edit = new EditConfig(yangStoreSnapshot, provider, configRegistry, ValidateTest.NETCONF_SESSION_ID_FOR_REPORTING);
		EditConfigStrategy editStrat = mock(EditConfigStrategy.class);
		doNothing().when(editStrat).executeConfiguration(anyString(), anyString(), anyMap(), any(ConfigTransactionClient.class));
		...
		EditConfigExecution editConfigExecution = new EditConfigExecution(null, cfg, xmlElement, EditConfigXmlParser.TestOption.testThenSet);

		edit.getResponseInternal(Xml.getDocument(), editConfigExecution);
