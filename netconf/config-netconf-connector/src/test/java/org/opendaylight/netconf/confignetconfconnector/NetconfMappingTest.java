...
public class NetconfMappingTest extends AbstractConfigTest {
	...
	public void testConfigNetconf() throws Exception {
		...
		final String responseFromCandidate = Xml.toString(response).replaceAll("\\s+", "");
		// System.out.println(responseFromCandidate);
		response = getConfigRunning();
		final String responseFromRunning = Xml.toString(response).replaceAll("\\s+", "");
		// System.out.println(responseFromRunning);
		assertEquals(responseFromCandidate, responseFromRunning);

		final String expectedResult = XmlFileLoader.fileToString("netconfMessages/editConfig_expectedResult.xml").replaceAll("\\s+", "");
	...
	private void closeSession() throws NetconfDocumentedException, ParserConfigurationException, SAXException, IOException {
		DefaultCloseSession closeOp = new DefaultCloseSession(NETCONF_SESSION_ID);
		executeOp(closeOp, "netconfMessages/closeSession.xml");
	}

	private void edit(String resource) throws ParserConfigurationException, SAXException, IOException, NetconfDocumentedException {
		EditConfig editOp = new EditConfig(yangStoreSnapshot, transactionProvider, configRegistryClient, NETCONF_SESSION_ID);
	...
	private Element getConfigCandidate() throws ParserConfigurationException, SAXException, IOException, NetconfDocumentedException {
		GetConfig getConfigOp = new GetConfig(yangStoreSnapshot, Optional.<String> absent(), transactionProvider, configRegistryClient, NETCONF_SESSION_ID);
		return executeOp(getConfigOp, "netconfMessages/getConfig_candidate.xml");
	}

	private Element getConfigRunning() throws ParserConfigurationException, SAXException, IOException, NetconfDocumentedException {
		GetConfig getConfigOp = new GetConfig(yangStoreSnapshot, Optional.<String> absent(), transactionProvider, configRegistryClient, NETCONF_SESSION_ID);
	...
	public void testSameAttrDifferentNamespaces() throws Exception {
		try {
			...
			assertThat(message, JUnitMatchers.containsString("Element simple-long-2 present multiple times with different namespaces"));
			assertThat(message, JUnitMatchers.containsString("urn:opendaylight:params:xml:ns:yang:controller:test:impl"));
			assertThat(message, JUnitMatchers.containsString(XMLUtil.URN_OPENDAYLIGHT_PARAMS_XML_NS_YANG_CONTROLLER_CONFIG));
			throw e;
		}
	}
	...
	public void testSameAttrDifferentNamespacesList() throws Exception {
		try {
			...
			String message = e.getMessage();
			assertThat(message, JUnitMatchers.containsString("Element binaryLeaf present multiple times with different namespaces"));
			assertThat(message, JUnitMatchers.containsString("urn:opendaylight:params:xml:ns:yang:controller:test:impl"));
			assertThat(message, JUnitMatchers.containsString(XMLUtil.URN_OPENDAYLIGHT_PARAMS_XML_NS_YANG_CONTROLLER_CONFIG));
			throw e;
		}
	...
	private void checkTypeConfigAttribute(Element response) {
		...
		XmlElement modulesElement = XmlElement.fromDomElement(response).getOnlyChildElement("data").getOnlyChildElement("modules");
		...
		for (XmlElement moduleElement : modulesElement.getChildElements("module")) {
			for (XmlElement type : moduleElement.getChildElements("type")) {
				if (type.getAttribute(Xml.XMLNS_ATTRIBUTE_KEY).equals("") == false) {
	...
	public void testConfigNetconfRuntime() throws Exception {
		...
		assertThat(Xml.toString(response), JUnitMatchers.containsString("testarg1".toUpperCase()));

		response = executeOp(netconf, "netconfMessages/rpcInner.xml");
		assertThat(Xml.toString(response), JUnitMatchers.containsString("ok"));

		response = executeOp(netconf, "netconfMessages/rpcInnerInner.xml");
		assertThat(Xml.toString(response), JUnitMatchers.containsString("true"));
	}
	...
	private void registerInner2(final InnerRunningDataRuntimeRegistration reg) {
		class InnerInnerRunningDataRuntimeMXBeanTest implements InnerInnerRunningDataRuntimeMXBean {
			...
			@Override
			public Boolean noArgInnerInner(Boolean aBoolean, Integer integer) {
				return aBoolean;
			}
	...
	private Element executeOp(final NetconfOperation op, final String filename) throws ParserConfigurationException, SAXException,
			IOException, NetconfDocumentedException {

		final Document request = XmlFileLoader.xmlFileToDocument(filename);

		logger.debug("Executing netconf operation\n{}", Xml.toString(request));
	...
	private List<InputStream> getYangs() throws FileNotFoundException {
		...
		for (String path : paths) {
			final InputStream is = Preconditions.checkNotNull(getClass().getResourceAsStream(path), path + " not found");
