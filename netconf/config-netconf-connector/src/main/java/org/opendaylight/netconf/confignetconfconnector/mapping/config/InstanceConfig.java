...
import org.opendaylight.netconf.confignetconfconnector.mapping.attributes.fromxml.AttributeReadingStrategy;
import org.opendaylight.netconf.confignetconfconnector.mapping.attributes.fromxml.ObjectXmlReader;
import org.opendaylight.netconf.confignetconfconnector.mapping.attributes.mapping.AttributeMappingStrategy;
import org.opendaylight.netconf.confignetconfconnector.mapping.attributes.mapping.ObjectMapper;
import org.opendaylight.netconf.confignetconfconnector.mapping.attributes.resolving.AttributeResolvingStrategy;
import org.opendaylight.netconf.confignetconfconnector.mapping.attributes.resolving.ObjectResolver;
import org.opendaylight.netconf.confignetconfconnector.mapping.attributes.toxml.AttributeWritingStrategy;
import org.opendaylight.netconf.confignetconfconnector.mapping.attributes.toxml.ObjectXmlWriter;
...
public final class InstanceConfig {
	...
	private Map<String, Object> getMappedConfiguration(ObjectName on, Services depTracker) {
		...
		Map<String, AttributeMappingStrategy<?, ? extends OpenType<?>>> mappingStrategies = new ObjectMapper(depTracker).prepareMapping(jmxToAttrConfig);
	...
	private void resolveConfiguration(InstanceConfigElementResolved mappedConfig, Services depTracker) {
		...
		Map<String, AttributeResolvingStrategy<?, ? extends OpenType<?>>> resolvingStrategies = new ObjectResolver(depTracker).prepareResolving(yangToAttrConfig);
		...
		for (Entry<String, AttributeConfigElement> configDefEntry : mappedConfig.getConfiguration().entrySet()) {
			try {
				...
				throw new IllegalStateException("Unable to resolve value " + configDefEntry.getValue() + " to attribute "
	...
	public InstanceConfigElementResolved fromXml(XmlElement moduleElement, Services services, String moduleNamespace) {
		...
		XmlElement type = moduleElement.getOnlyChildElementWithSameNamespace(Xml.TYPE_KEY);
		XmlElement name = moduleElement.getOnlyChildElementWithSameNamespace(Xml.NAME_KEY);
		...
		for (Entry<String, AttributeReadingStrategy> readStratEntry : strats.entrySet()) {
			List<XmlElement> configNodes = getConfigNodes(moduleElement, moduleNamespace, readStratEntry.getKey(), recognisedChildren,
					typeAndName);
		...
		String perInstanceEditStrategy = moduleElement.getAttribute(Xml.OPERATION_ATTR_KEY, XMLUtil.URN_IETF_PARAMS_XML_NS_NETCONF_BASE_1_0);

		InstanceConfigElementResolved instanceConfigElementResolved = perInstanceEditStrategy.equals("") ? new InstanceConfigElementResolved(retVal)
				: new InstanceConfigElementResolved(perInstanceEditStrategy, retVal);
	...
	private List<XmlElement> getConfigNodes(XmlElement moduleElement, String moduleNamespace, String name,
		...
		if (foundConfigNodes.isEmpty()) {
			...
			// In case module type or name element is not present in config it would be matched with config type or name
			// We need to remove config type and name from available module config elements
			foundConfigNodes.removeAll(typeAndName);
			logger.debug("Found {} config nodes {} without specified namespace in {}", foundConfigNodes.size(), name, moduleElement);
		} else {
