...
public final class ModuleRpcs {
	...
	public void addNameMapping(RuntimeBeanEntry runtimeEntry) {
		...
		Preconditions.checkState(yangToJavaNames.containsKey(yangName) == false, "RuntimeBean %s found twice in same namespace", yangName);
	...
	public void addRpc(RuntimeBeanEntry runtimeEntry, Rpc rpc) {
		...
		Preconditions.checkState(map.containsKey(rpc.getYangName()) == false, "Rpc %s for runtime bean %s added twice", rpc.getYangName(),
				yangName);
	...
	public String getRbeJavaName(String yangName) {
		...
		Preconditions.checkState(javaName != null, "No runtime bean entry found under yang name %s, available yang names %s", yangName,
