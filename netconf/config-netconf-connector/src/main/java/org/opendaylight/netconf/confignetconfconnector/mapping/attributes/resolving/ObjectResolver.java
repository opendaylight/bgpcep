...
package org.opendaylight.netconf.confignetconfconnector.mapping.attributes.resolving;

import com.google.common.base.Preconditions;
import com.google.common.collect.Maps;
import org.opendaylight.controller.config.yangjmxgenerator.attribute.*;
import org.opendaylight.netconf.confignetconfconnector.mapping.attributes.AttributeIfcSwitchStatement;
...
public class ObjectResolver extends AttributeIfcSwitchStatement<AttributeResolvingStrategy<?, ? extends OpenType<?>>> {
	...
	public Map<String, AttributeResolvingStrategy<?, ? extends OpenType<?>>> prepareResolving(Map<String, AttributeIfc> configDefinition) {
		...
		for (Entry<String, AttributeIfc> attrEntry : configDefinition.entrySet()) {
			strategies.put(attrEntry.getKey(), prepareStrategy(attrEntry.getValue(), attrEntry.getValue().getOpenType()));
		}

		return strategies;
	}

	private AttributeResolvingStrategy<?, ? extends OpenType<?>> prepareStrategy(AttributeIfc attributeIfc, OpenType<?> openType) {
	...
	@Override
	protected AttributeResolvingStrategy<?, ? extends OpenType<?>> caseDependencyAttribute(DependencyAttribute attributeIfc) {
	...
	protected AttributeResolvingStrategy<?, ? extends OpenType<?>> caseTOAttribute(TOAttribute attributeIfc) {
		...
		for (String innerName : compositeType.keySet()) {
			Preconditions.checkState(attributeIfc instanceof TOAttribute, "Unexpected state, " + attributeIfc + " should be instance of "
					+ TOAttribute.class.getName());
			AttributeIfc innerAttributeIfc = attributeIfc.getJmxPropertiesToTypesMap().get(innerName);
			innerMap.put(innerAttributeIfc.getAttributeYangName(), prepareStrategy(innerAttributeIfc, compositeType.getType(innerName)));
		}
