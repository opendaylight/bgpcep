...
public class Config {
	...
	private Map<String, Map<String, Collection<ObjectName>>> getMappedInstances(Set<ObjectName> instancesToMap, Services serviceTracker) {
		...
		for (String namespace : moduleConfigs.keySet()) {
			...
			for (Entry<String, ModuleConfig> mbeEntry : moduleConfigs.get(namespace).entrySet()) {
				...
				// This way all instances will be serialized as all available services when get-config is triggered
	...
	private void addServices(Services serviceTracker, Collection<ObjectName> instances, Collection<String> providedServices) {
	...
	// public Element toXml(Set<ObjectName> instancesToMap, String namespace, Document document) {
	// return toXml(instancesToMap, Optional.of(namespace), document);
	// }

	public Element toXml(Set<ObjectName> instancesToMap, Optional<String> maybeNamespace, Document document, Element dataElement) {
		Services serviceTracker = new Services();

		Map<String, Map<String, Collection<ObjectName>>> moduleToInstances = getMappedInstances(instancesToMap, serviceTracker);

		Element root = dataElement;
		if (maybeNamespace.isPresent()) {
			Xml.addNamespaceAttr(root, maybeNamespace.get());
		}
		...
		for (String moduleNamespace : moduleToInstances.keySet()) {
			for (Entry<String, Collection<ObjectName>> moduleMappingEntry : moduleToInstances.get(moduleNamespace).entrySet()) {
	...
	private void addEmptyModulesCommented(Document document, Element root, String moduleNamespace,
		...
		Element emptyModule = document.createElement(Xml.MODULE_KEY);

		Element typeElement = Xml.createTextElement(document, Xml.TYPE_KEY, moduleMappingEntry.getKey());
		emptyModule.appendChild(typeElement);

		root.appendChild(document.createComment(Xml.toString(emptyModule, false)));
	}

	// TODO refactor, replace string representing namespace with namespace class
	// TODO refactor, replace Map->Multimap with e.g. ConfigElementResolved class
	...
	private List<XmlElement> fromXmlModules(XmlElement xml, List<XmlElement> recognisedChildren) {
		Optional<XmlElement> modulesElement = xml.getOnlyChildElementOptionally(Xml.MODULES_KEY,
				XMLUtil.URN_OPENDAYLIGHT_PARAMS_XML_NS_YANG_CONTROLLER_CONFIG);
		List<XmlElement> moduleElements;
		if (modulesElement.isPresent()) {
			moduleElements = modulesElement.get().getChildElementsWithSameNamespace(Xml.MODULE_KEY);
	...
	private void resolveModule(Map<String, Multimap<String, ModuleElementResolved>> retVal, Services serviceTracker,
		...
		XmlElement typeElement = moduleElement.getOnlyChildElementWithSameNamespace(Xml.TYPE_KEY);
		...
		XmlElement nameElement = moduleElement.getOnlyChildElementWithSameNamespace(Xml.NAME_KEY);
		...
		ModuleElementResolved moduleElementResolved = moduleMapping.fromXml(moduleElement, serviceTracker, instanceName, moduleNamespace);
	...
	private Services fromXmlServices(XmlElement xml, List<XmlElement> recognisedChildren) {
		Optional<XmlElement> servicesElement = xml.getOnlyChildElementOptionally(Xml.SERVICES_KEY,
				XMLUtil.URN_OPENDAYLIGHT_PARAMS_XML_NS_YANG_CONTROLLER_CONFIG);
	...
	private String getFactoryName(String factoryNameWithPrefix, String prefixOrEmptyString) {
		checkState(factoryNameWithPrefix.startsWith(prefixOrEmptyString), format("Internal error: text "
				+ "content '%s' of type node does not start with prefix '%s'", factoryNameWithPrefix, prefixOrEmptyString));
	...
	private ModuleConfig getModuleMapping(String moduleNamespace, String instanceName, String factoryName) {
		...
		Preconditions.checkNotNull(mappingsFromNamespace,
				"Namespace %s, defined in: module %s of type %s not found, available namespaces: %s", moduleNamespace, instanceName,
				factoryName, moduleConfigs.keySet());
