...
public class InstanceRuntime {
	...
	/**
	 * Finds all children runtime beans, same properties and values as current root + any number of additional
	 * properties
	 */
	...
	/**
	 * Finds next level root runtime beans, beans that have the same properties as current root + one additional
	 */
	...
	public Element toXml(ObjectName rootOn, Set<ObjectName> childRbeOns, Document document, String instanceIndex, String keyName) {
		Element xml = document.createElement(keyName == null ? Xml.DATA_KEY : keyName);
		// TODO namespace
		...
		for (Entry<String, InstanceRuntime> childMappingEntry : childrenMappings.entrySet()) {
			Set<ObjectName> innerRootBeans = getRootBeans(childRbeOns, childMappingEntry.getKey(), rootOn.getKeyPropertyList().size());
			...
			for (ObjectName objectName : innerRootBeans) {
				...
				xml.appendChild(childMappingEntry.getValue().toXml(objectName, innerChildRbeOns, document, runtimeInstanceIndex,
						elementName));
			}
		}
