...
public class SimpleAttributeReadingStrategy extends AbstractAttributeReadingStrategy<AttributeIfc> {
	...
	AttributeConfigElement readElementHook(List<XmlElement> configNodes) {
		...
		Preconditions.checkState(configNodes.size() == 1, "This element should be present only once " + xmlElement + " but was "
				+ configNodes.size());
	...
	/**
	 * Wrapper for JavaAttribute inner element attributes (in case JavaAttribute is array)
	 */
