...
public class ModuleConfig {
	...
	public Element toXml(ObjectName instanceON, Services depTracker, Document document, String namespace) {
		...
		Element typeElement = Xml.createPrefixedTextElement(document, Xml.TYPE_KEY, prefix, moduleName);
		Xml.addPrefixedNamespaceAttr(typeElement, prefix, namespace);
		// Xml.addNamespaceAttr(document, typeElement, XMLUtil.URN_OPENDAYLIGHT_PARAMS_XML_NS_YANG_CONTROLLER_CONFIG);
		root.appendChild(typeElement);

		Element nameElement = Xml.createTextElement(document, Xml.NAME_KEY, ObjectNameUtil.getInstanceName(instanceON));
		// Xml.addNamespaceAttr(document, nameElement, XMLUtil.URN_OPENDAYLIGHT_PARAMS_XML_NS_YANG_CONTROLLER_CONFIG);
	...
	private String getPrefix(String namespace) {
		...
		return "prefix";
		// return namespace.substring(namespace.lastIndexOf(':') + 1, namespace.length());

	}

	public ModuleElementResolved fromXml(XmlElement moduleElement, Services depTracker, String instanceName, String moduleNamespace) {
