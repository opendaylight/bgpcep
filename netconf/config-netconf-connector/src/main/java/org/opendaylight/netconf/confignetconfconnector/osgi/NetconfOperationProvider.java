...
final class NetconfOperationProvider {
	...
	NetconfOperationProvider(YangStoreSnapshot yangStoreSnapshot, ConfigRegistryClient configRegistryClient,
		...
		operations = setUpOperations(yangStoreSnapshot, configRegistryClient, transactionProvider, netconfSessionIdForReporting);
	}
	...
	private static Set<NetconfOperation> setUpOperations(YangStoreSnapshot yangStoreSnapshot, ConfigRegistryClient configRegistryClient,
			TransactionProvider transactionProvider, String netconfSessionIdForReporting) {
		Set<NetconfOperation> ops = Sets.newHashSet();

		GetConfig getConfigOp = new GetConfig(yangStoreSnapshot, Optional.<String> absent(), transactionProvider, configRegistryClient, netconfSessionIdForReporting);

		ops.add(getConfigOp);
		ops.add(new EditConfig(yangStoreSnapshot, transactionProvider, configRegistryClient, netconfSessionIdForReporting));
