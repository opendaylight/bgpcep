...
public class Validate extends AbstractConfigNetconfOperation {
	...
	private void checkXml(XmlElement xml) {
		...
		sourceChildNode.checkNamespace(XMLUtil.URN_IETF_PARAMS_XML_NS_NETCONF_BASE_1_0);
		String datastoreValue = sourceChildNode.getName();
		Datastore sourceDatastore = Datastore.valueOf(datastoreValue);

		Preconditions.checkState(sourceDatastore == Datastore.candidate, "Only " + Datastore.candidate + " is supported as source for "
				+ VALIDATE + " but was " + datastoreValue);
	}
	...
	protected Element handle(Document document, XmlElement xml) throws NetconfDocumentedException {
		...
		try {
			...
			errorInfo.put(ErrorTag.operation_failed.name(),
					"Datastore is not present. Use 'get-config' or 'edit-config' before triggering 'validate' operation");
			throw new NetconfDocumentedException(e.getMessage(), e, ErrorType.application, ErrorTag.operation_failed, ErrorSeverity.error, errorInfo);
