	...
	public Optional<Object> parseAttribute(String attrName, Object value) {
		...
		if (innerTypeResolvingStrategy.getOpenType() instanceof CompositeType) {
			...
			try {
				...
				throw new RuntimeException("Unable to locate class for " + getOpenType().getElementOpenType().getClassName(), e);
			}
		...
		logger.debug("Attribute {} : {} parsed to type {} as {}", attrName, value, getOpenType(), toStringArray(parsedArray));
