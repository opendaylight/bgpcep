...
package org.opendaylight.netconf.confignetconfconnector.transactions;
...
public class TransactionProvider implements AutoCloseable {
	...
	public Optional<ObjectName> getTransaction() {
		...
		if (isStillOpenTransaction(transaction) == false) {
			logger.warn("Fixing illegal state: transaction {} was closed in {}", transaction, netconfSessionIdForReporting);
			transaction = null;
			return Optional.absent();
		}
