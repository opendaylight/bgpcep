...
public class ReplaceEditConfigStrategy extends AbstractEditConfigStrategy {
	...
	@Override
	void handleMissingInstance(Map<String, AttributeConfigElement> configuration, ConfigTransactionClient ta, String module, String instance) {
		try {
		...
		for (Entry<String, AttributeConfigElement> configAttributeEntry : configuration.entrySet()) {
			try {
				...
				if (!ace.getResolvedValue().isPresent()) {
					...
					logger.debug("Attribute {} set to default value {} for {}", configAttributeEntry.getKey(), value, on);
				} else {
