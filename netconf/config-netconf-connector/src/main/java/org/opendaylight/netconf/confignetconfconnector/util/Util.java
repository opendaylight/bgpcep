...
package org.opendaylight.netconf.confignetconfconnector.util;
...
public final class Util {
	...
	public static void checkType(final Object value, final Class<?> clazz) {
		Preconditions.checkArgument(clazz.isAssignableFrom(value.getClass()), "Unexpected type " + value.getClass() + " should be " + clazz);
	}

	// TODO: add message and proper error types
	public static YangStoreSnapshot getYangStore(final YangStoreService yangStoreService) throws NetconfDocumentedException {
		try {
			return yangStoreService.getYangStoreSnapshot();
		} catch (final YangStoreException e) {
			throw new NetconfDocumentedException("TODO", e, ErrorType.application, ErrorTag.bad_attribute, ErrorSeverity.error);
		}
	}
