...
public final class NetconfMessageFactory implements ProtocolMessageFactory<NetconfMessage> {
	...
	public byte[] put(NetconfMessage netconfMessage) {
		...
		byte[] bytes = (this.framing == FramingMechanism.EOM) ? this.putEOM(netconfMessage) : this.putChunked(netconfMessage);
	...
	/**
	 * For Hello message the framing is always EOM, but the framing mechanism may change.
	 * 
	 * @param fm new framing mechanism
	 */
