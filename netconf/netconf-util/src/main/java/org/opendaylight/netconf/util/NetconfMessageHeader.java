...
/**
 * Netconf message header is used only when chunked framing mechanism is supported. The header consists of only the
 * length field.
 */
public final class NetconfMessageHeader {
	...
	public NetconfMessageHeader fromBytes(final byte[] bytes) {
		// the length is variable therefore bytes between headerBegin and headerEnd mark the length
		// the length should be only numbers and therefore easily parsed with ASCII
		this.length = Long.parseLong(Charsets.US_ASCII.decode(
				ByteBuffer.wrap(ByteArray.subByte(bytes, headerBegin.length, bytes.length - headerBegin.length - 1))).toString());
