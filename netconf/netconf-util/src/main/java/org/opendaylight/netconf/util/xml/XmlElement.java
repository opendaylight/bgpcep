...
public class XmlElement {
	...
	public static XmlElement fromString(String s) {
		try {
			return new XmlElement(Xml.readXmlToElement(s));
	...
	private static Map<String, String> extractNamespaces(Element typeElement) {
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (attribKey.startsWith(Xml.XMLNS_ATTRIBUTE_KEY)) {
				String prefix;
				if (attribKey.equals(Xml.XMLNS_ATTRIBUTE_KEY)) {
					prefix = "";
				} else {
					Preconditions.checkState(attribKey.startsWith(Xml.XMLNS_ATTRIBUTE_KEY + ":"));
					prefix = attribKey.substring(Xml.XMLNS_ATTRIBUTE_KEY.length() + 1);
				}
	...
	public void checkName(String expectedName) {
		Preconditions.checkArgument(getName().equals(expectedName), "Expected %s xml element but was %s", expectedName, getName());
	}
	...
	public void checkNamespace(String expectedNamespace) {
		Preconditions.checkArgument(getNamespace().equals(expectedNamespace), "Unexpected namespace %s for element %s, should be %s",
	...
	public List<XmlElement> getChildElementsWithinNamespace(final String childName, String namespace) {
		return Lists.newArrayList(Collections2.filter(getChildElementsWithinNamespace(namespace), new Predicate<XmlElement>() {
			@Override
	...
	public XmlElement getOnlyChildElement(final String childName, String namespace) {
		...
		Preconditions.checkState(children.size() == 1, "One element %s:%s expected in %s but was %s", namespace, childName, toString(),
				children.size());
	...
	public XmlElement getOnlyChildElement() {
		...
		Preconditions.checkState(children.size() == 1, "One element expected in %s but was %s", toString(), children.size());
		return children.get(0);
	}
	...
	public String getNamespaceAttribute() {
		String attribute = element.getAttribute(Xml.XMLNS_ATTRIBUTE_KEY);
		Preconditions.checkState(attribute != null && !attribute.equals(""), "Element %s must specify a %s attribute", toString(),
				Xml.XMLNS_ATTRIBUTE_KEY);
		return attribute;
	}
	...
	/**
	 * Search for element's attributes defining namespaces. Look for the one namespace that matches prefix of element's
	 * text content. E.g.
	 * 
	 * <pre>
	 * &lt;type
	 ...
	 * </pre>
	 * 
	 * returns {"th-java","urn:.."}. If no prefix is matched, then default namespace is returned with empty string as
	...
	public Map.Entry<String/*prefix*/, String/*namespace*/> findNamespaceOfTextContent() {
		...
		if (namespaces.containsKey(prefix) == false) {
			throw new IllegalArgumentException("Cannot find namespace for " + element + ". Prefix from content is " + prefix
