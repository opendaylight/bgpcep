...
public class NetconfConfigUtil {
	...
	public static Optional<TLSConfiguration> extractTLSConfiguration(ConfigProvider configProvider) {
		...
		if (address.isPresent()) {
			...
			checkNotNull(keystoreFileName, "Property %s must be defined for tls netconf server", NETCONF_TLS_KEYSTORE_PROP);
	...
	/**
	 * @param configProvider from which properties are being read.
	 * @param infixProp either tcp or tls
	 * @return absent if address is missing, value if address and port are valid.
	 * @throws IllegalStateException if address or port are invalid
	 */
	private static Optional<InetSocketAddress> extractSomeNetconfAddress(ConfigProvider configProvider, InfixProp infixProp) {
		...
		try {
			...
			throw new IllegalStateException("Cannot create " + infixProp + " netconf address from address:" + address + " and port:"
					+ portString, e);
		}
	}
}
