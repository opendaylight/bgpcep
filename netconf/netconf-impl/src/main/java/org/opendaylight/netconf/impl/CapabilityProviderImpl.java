...
import org.opendaylight.netconf.impl.osgi.NetconfOperationServiceSnapshot;
import org.opendaylight.netconf.mapping.api.Capability;
...
public class CapabilityProviderImpl implements CapabilityProvider {
	...
	public synchronized String getSchemaForCapability(String moduleName, Optional<String> revision) {
		...
		Preconditions.checkState(revisionMapRequest != null, "Capability for module %s not present, " + "" + "available modules : %s",
				moduleName, capabilityURIs);

		if (revision.isPresent()) {
			String schema = revisionMapRequest.get(revision.get());

			Preconditions.checkState(schema != null, "Capability for module %s:%s not present, available revisions for module: %s",
					moduleName, revision.get(), revisionMapRequest.keySet());

			return schema;
		} else {
			Preconditions.checkState(revisionMapRequest.size() == 1, "Expected 1 capability for module %s, available revisions : %s",
					moduleName, revisionMapRequest.keySet());
