...
import org.opendaylight.netconf.mapping.api.NetconfOperationFilter;
import org.opendaylight.netconf.mapping.api.NetconfOperationFilterChain;
...
public class DefaultCommit implements NetconfOperationFilter {
	...
	private static Document loadGetConfigMessage() {
		try (InputStream asStream = DefaultCommit.class.getResourceAsStream(GET_CONFIG_CANDIDATE_XML_LOCATION)) {
			return Xml.readXmlToDocument(asStream);
		} catch (Exception e) {
			throw new IllegalStateException("Unable to load getConfig message for notifications from " + GET_CONFIG_CANDIDATE_XML_LOCATION);
		}
	}

	@Override
	public Document doFilter(Document message, NetconfOperationRouter operationRouter, NetconfOperationFilterChain filterChain)
	...
	private boolean canHandle(OperationNameAndNamespace operationNameAndNamespace) {
		if (operationNameAndNamespace.getOperationName().equals("commit") == false)
			return false;
		return operationNameAndNamespace.getNamespace().equals(XMLUtil.URN_IETF_PARAMS_XML_NS_NETCONF_BASE_1_0);
	}
	...
	private boolean isCommitWithoutNotification(Document message) {
		XmlElement xmlElement = XmlElement.fromDomElementWithExpected(message.getDocumentElement(), Xml.RPC_KEY,
	...
	private Element getConfigSnapshot(NetconfOperationRouter opRouter) throws NetconfDocumentedException {
		...
		try {
			XmlElement xmlElement = XmlElement.fromDomElementWithExpected(responseDocument.getDocumentElement(), Xml.RPC_REPLY_KEY,
					XMLUtil.URN_IETF_PARAMS_XML_NS_NETCONF_BASE_1_0);
			dataElement = xmlElement.getOnlyChildElement(Xml.DATA_KEY);
			...
			throw new NetconfDocumentedException(msg, e, NetconfDocumentedException.ErrorType.application, NetconfDocumentedException.ErrorTag.operation_failed, NetconfDocumentedException.ErrorSeverity.error, info);
		}
