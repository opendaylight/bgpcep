...
public class NetconfServerSessionListenerFactory implements SessionListenerFactory<NetconfServerSessionListener> {
	...
	public NetconfServerSessionListener getSessionListener() {
		NetconfOperationServiceSnapshot netconfOperationServiceSnapshot = factoriesListener.getSnapshot(idProvider.getCurrentSessionId());

		CapabilityProvider capabilityProvider = new CapabilityProviderImpl(netconfOperationServiceSnapshot);

		NetconfOperationRouterImpl operationRouter = new NetconfOperationRouterImpl(netconfOperationServiceSnapshot, capabilityProvider, commitNotifier);
