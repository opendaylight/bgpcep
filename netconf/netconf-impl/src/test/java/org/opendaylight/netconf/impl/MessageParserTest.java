...
import org.opendaylight.netconf.util.FramingMechanism;
import org.opendaylight.netconf.util.NetconfMessageFactory;
...
public class MessageParserTest {
	...
	public void testPutChunk() throws IOException, SAXException, ParserConfigurationException {
		...
		assertArrayEquals(NetconfMessageFactory.endOfChunk,
				ByteArray.subByte(bytes, bytes.length - NetconfMessageFactory.endOfChunk.length, NetconfMessageFactory.endOfChunk.length));
	}

	@Test
	public void testParseEOM() throws IOException, SAXException, DeserializerException, DocumentedException, ParserConfigurationException {
		...
		final Document doc = this.parser.parse(ByteArray.subByte(bytes, 0, bytes.length - NetconfMessageFactory.endOfMessage.length)).iterator().next().getDocument();
		assertEquals(XMLUtil.xmlToString(msg), XMLUtil.xmlToString(doc));
		XMLAssert.assertXMLEqual(msg, doc);
	}

	@Test
	public void testParseChunk() throws IOException, SAXException, DeserializerException, DocumentedException, ParserConfigurationException {
		final Document msg = XmlFileLoader.xmlFileToDocument("netconfMessages/client_hello.xml");
		this.parser.setFramingMechanism(FramingMechanism.CHUNK);
		final byte[] bytes = this.parser.put(new NetconfMessage(msg));
		final Document doc = this.parser.parse(ByteArray.subByte(bytes, 6, bytes.length - NetconfMessageFactory.endOfChunk.length - 6)).iterator().next().getDocument();
		assertEquals(XMLUtil.xmlToString(msg), XMLUtil.xmlToString(doc));
		XMLAssert.assertXMLEqual(msg, doc);
	}
